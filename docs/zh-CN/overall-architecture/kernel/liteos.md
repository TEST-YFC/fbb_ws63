...
LiteOS (Huawei LiteOS) 是华为面向物联网领域的轻量级实时操作系统，版本 v208.5.0，为 WS63 的默认内核，代码位于 `kernel/liteos/`。
...
- WS63 应用和普通组件统一使用 OSAL (Operating System Abstraction Layer) ，不直接调用 LiteOS 原生接口。OSAL 位于 LiteOS 之上，用于统一任务、同步、内存、定时器和中断等系统能力的使用方式。
