...
> 基于 `ws63-liteos-app` 当前源码，说明 LiteOS (Huawei LiteOS) 开始调度后系统中的任务组成、执行上下文、通信边界和资源约束
...
平台任务表定义在 `application/ws63/ws63_liteos_application/main.c` 的 `g_app_tasks[]` 中。实际固件只包含当前 target 和 Kconfig 选中的任务，并非下表任务会同时存在。
