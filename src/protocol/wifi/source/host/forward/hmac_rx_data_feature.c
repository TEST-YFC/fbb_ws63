...
#endif
        (void)memcpy_s(msdu->netbuf->data, submsdu_len, (submsdu_hdr + MAC_SUBMSDU_HEADER_LEN), submsdu_len);
...
OAL_STATIC osal_void hmac_parse_dns_answer(osal_u8 *msg, osal_u32 msg_len, osal_u16 qdcount)
{
    ...
    while (buff + 12 < end) { /* 12：长度判断 */
        ...
        (void)memcpy_s(&type, OAL_SIZEOF(type), buff, OAL_SIZEOF(type));
        buff += 8;    // 8：type, class: 2bytes, ttl: 4bytes
        (void)memcpy_s(&rdlen, OAL_SIZEOF(rdlen), buff, OAL_SIZEOF(rdlen));
        buff += 2; /* 2：跳转长度 */
        type = OAL_NTOH_16(type);
        rdlen = OAL_NTOH_16(rdlen);
        if (type == OAL_NS_T_A && rdlen == 4) { /* 4：长度判断 */
            (void)memcpy_s(&ipv4, rdlen, buff, rdlen);
