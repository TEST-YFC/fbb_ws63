...
#if defined(_PRE_WLAN_FEATURE_WS63) && defined(__LITEOS__)
#include "watchdog.h"
#include "los_cpup.h"
...
#if defined(_PRE_WLAN_FEATURE_WS63) && defined(__LITEOS__)
#define HMAC_W2W_MIN_IDLE_RATE 50
OAL_STATIC osal_void hmac_wlan_to_wlan_delay(osal_void)
{
    osal_u32 ret = LOS_HistoryTaskCpuUsage(OsGetIdleTaskId(), CPUP_LAST_ONE_SECONDS);
    if (ret < HMAC_W2W_MIN_IDLE_RATE) {
        uapi_watchdog_kick();
    }
}
#endif
...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_u32 hmac_rx_transmit_virtual_multi_sta_proc(hmac_vap_stru *hmac_vap,
    hmac_user_stru *hmac_user, hmac_msdu_stru *msdu, oal_netbuf_stru *netbuf)
{
    ...
    if (fhook != OSAL_NULL) {
        if (((hmac_bridge_rx_process_cb)fhook)(netbuf, hmac_vap) != OAL_CONTINUE) {
            return OAL_SUCC;
        }
    }
    ...
    hmac_msdu_stru *msdu)
{
    ...
    /* 在SLE_REPEATER流程才会出现满足条件的情况，由内部释放netbuf */
    if (hmac_rx_transmit_virtual_multi_sta_proc(hmac_vap, hmac_user, msdu, netbuf) != OAL_CONTINUE) {
        return;
    }
...
osal_void hmac_rx_process_data_ap_tcp_ack_opt_etc(oal_netbuf_head_stru *netbuf_header)
{
    ...
    while (netbuf_num != 0 && temp_netbuf != OSAL_NULL) {
        ...
        netbuf->data_src = DATA_SRC_SOFTAP;
#endif
        /* 组播帧处理 */
