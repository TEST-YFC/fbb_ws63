...
WIFI_HMAC_TCM_TEXT OSAL_STATIC osal_u8 hmac_rx_data_check_cipher(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    data_type = hmac_get_data_type_from_80211_etc(netbuf, oal_netbuf_rx_header_len(netbuf));
    if ((data_type == MAC_DATA_EAPOL) && (mac_get_eapol_type_etc(netbuf) == OAL_EAPOL_TYPE_KEY) &&
        (hmac_is_rx_ucast_frame_for_current_bss(hmac_vap, (const osal_u8 *)mac_hdr) == OSAL_TRUE)) {
        hmac_eapol_key_log((const mac_eapol_header_stru *)(oal_netbuf_rx_data(netbuf) + sizeof(mac_llc_snap_stru)),
            OAL_TRUE);
    }
    ...
    /* 漫游时接收不加密eapol帧 */
    if ((hmac_vap->vap_state == MAC_VAP_STATE_ROAMING) && (data_type == MAC_DATA_EAPOL)) {
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_rx_data_check_cipher::rx eapol, is roaming.}");
        return OSAL_TRUE;
    }
    ...
    oal_netbuf_stru *netbuf, dmac_rx_ctl_stru *cb_ctrl, mac_rx_ctl_stru *rx_info)
{
    ...
    if (frame_subtype == WLAN_NULL_FRAME || frame_subtype == WLAN_QOS_NULL_FRAME) {
        return;
    }
...
#endif
    /* EAPOL帧处理 */
    if (((mac_llc_snap_stru *)(uintptr_t)frame_hdr)->ether_type == oal_host2net_short(ETHER_TYPE_PAE)) {
...
WIFI_HMAC_TCM_TEXT osal_s32 hmac_rx_process_data_msg_process(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hmac_is_thruput_enable(THRUPUT_RX_PK) != 0) {
