...
OAL_STATIC OAL_INLINE osal_void hmac_tx_eapol_key_proc(const hmac_vap_stru *hmac_vap,
    mac_ether_header_stru *ether_header, mac_tx_ctl_stru *tx_ctl, osal_u8 tid)
{
...
/* TCP ACK FLAG */
#define TCP_FIN_FLAG_BIT    (1 << 0)
#define TCP_SYN_FLAG_BIT    (1 << 1)
#define TCP_RESET_FLAG_BIT  (1 << 2)
#define TCP_URGENT_FLAG_BIT (1 << 5)
#define TCP_VIP_FLAG_MASK (TCP_FIN_FLAG_BIT | TCP_SYN_FLAG_BIT | TCP_RESET_FLAG_BIT | TCP_URGENT_FLAG_BIT)
    ...
    mac_tx_ctl_stru *tx_ctl, mac_ether_header_stru *ether_header, osal_u8 *tid)
{
    ...
    *tid = ip_hdr->tos >> WLAN_IP_PRI_SHIFT;
    ...
    if (hmac_is_dhcp_port_etc(ip_hdr) == OAL_TRUE) {
        ...
        if (oal_netbuf_is_tcp_ack_etc((oal_ip_header_stru *)(uintptr_t)ip_hdr) == OSAL_TRUE) {
            /* 标记TCP ack帧 */
            mac_get_cb_tcp_ack(tx_ctl) = OAL_TRUE;
            /* 标记特殊TCP ack帧 SYN FIN RST URG */
            if (((tcp->flags) & TCP_VIP_FLAG_MASK) != 0) {
                mac_get_cb_vip_tcp_ack(tx_ctl) = OAL_TRUE;
                mac_get_cb_frame_subtype(tx_ctl) = MAC_DATA_VIP_FRAME;
            }
        }
    } else if (ip_hdr->protocol == MAC_ICMP_PROTOCAL) {
        /* 标记ICMP帧 */
        mac_get_cb_icmp(tx_ctl) = OAL_TRUE;
    }
}
    ...
    oal_netbuf_stru *netbuf, osal_u8 *tid)
{
    ...
    switch (ether_header->ether_type) {
        ...
        case oal_host2net_short(ETHER_TYPE_PAE):
            ...
            hmac_eapol_key_log((mac_eapol_header_stru *)(ether_header + 1), OAL_FALSE);
            break;
...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_u32 hmac_bridge_vap_discard_pkt(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (hmac_vap->vap_state == MAC_VAP_STATE_ROAMING) {
        if (hmac_get_data_type_from_8023_etc((osal_u8 *)oal_netbuf_payload(netbuf), MAC_NETBUFF_PAYLOAD_ETH,
            OAL_NETBUF_LEN(netbuf)) != MAC_DATA_EAPOL) {
            return OAL_SUCC;
        }
    } else {
...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_void hmac_bridge_vap_xmit_proc(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    if (fhook != OSAL_NULL) {
        if (((hmac_bridge_tx_process_cb)fhook)(netbuf, hmac_vap) == OAL_ERR_CODE_PROXY_STA_BUF_DROP) {
            oal_netbuf_free(netbuf);
            return;
        }
    }
...
WIFI_HMAC_TCM_TEXT osal_s32 hmac_bridge_vap_xmit_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hmac_vap == NULL || hmac_vap->init_flag == MAC_VAP_INVAILD) {
        oal_netbuf_free(netbuf);
