    ...
    dmac_rx_ctl_stru *cb_ctrl, oal_netbuf_stru *netbuf)
{
    ...
    hmac_vap->data_frame_rssi = cb_ctrl->rx_statistic.rssi_dbm;
    ...
    /* 过滤五：除了NULL帧以外，如果报文长度小于MAC头+SNAP头长度，说明是异常报文，不进行处理 */
    if (OAL_NETBUF_LEN(netbuf) < oal_netbuf_rx_header_len(netbuf) + sizeof(mac_llc_snap_stru)) {
        return OAL_FAIL;
    }
...
OSAL_STATIC osal_u32 hmac_rx_filter_data_frame_ap(hmac_vap_stru *hmac_vap, const hmac_user_stru *hmac_user,
