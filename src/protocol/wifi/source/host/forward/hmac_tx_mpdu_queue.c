...
#elif defined(_PRE_LWIP_ZERO_COPY_MEM_ALLOC_PKT_BUF)
    ...
    hmac_tid_stru *tid_queue, osal_u8 mpdu_num, oal_netbuf_stru *netbuf)
{
    ...
    if (osal_unlikely((tid_queue->mpdu_num + 1) > WLAN_SINGLE_TID_MPDU_NUM_LIMIT)) {
        oam_warning_log2(0, OAM_SF_TX, "vap_id[%d] {hmac_tid_tx_queue_enqueue::single_tid_mpdu_num exceed[%d].}",
            hmac_vap->vap_id, tid_queue->mpdu_num);
        return OAL_FAIL;
    }
