...
#ifdef _PRE_WLAN_FEATURE_ALWAYS_TX
static const osal_u16 g_phyrate_11b_rate_table[] = { 10, 20, 55, 110 };  /* mcs 0~3 单位100k */

static const osal_u16 g_phyrate_11g_rate_table[] = { 60, 90, 120, 180, 240, 360, 480, 540 }; /* mcs 0~7 单位100k */

static const osal_u16 g_phyrate_11n_rate_table[] = {
    65, 130, 195, 260, 390, 520, 585, 650, 780, 870 /* mcs 0~9 单位100k */
};

static const osal_u16 g_phyrate_11ax_rate_table[] = {
...
static osal_u32 wal_check_cmd_set_rate_power_param(osal_s32 protocol, osal_s32 protocol_num, osal_u8 *rate_max,
    osal_s32 rate, osal_s32 power_offset)
{
    osal_s32 min_power = -100;  // 最小可配置的偏移是-100 -> -10dB
    osal_s32 max_power = 80;    // 最大可配置的偏移是 80 -> 8dB
    osal_s32 rate_num;
...
osal_s32 wal_set_rate_power(oal_net_device_stru *net_dev, osal_s32 protocol, osal_s32 rate, osal_s32 power_offset)
{
    ...
    osal_u8 protocol_num = (osal_u8)(OAL_SIZEOF(rate_max_num) / OAL_SIZEOF(osal_u8));
    ...
    if (wal_check_cmd_set_rate_power_param(protocol, protocol_num,
    ...
    offset = 0x8;
    config_msg = ((osal_u32)((rate_max << offset) + rate_min) << offset) + (osal_u8)power_offset;
 
    return wal_sync_post2hmac_no_rsp(wal_util_get_vap_id(net_dev),
...
osal_u32 uapi_get_back_off_time(osal_u32 rate, osal_u32 len, osal_u32 ratio)
{
    ...
    back_off_time = (back_off_time > WAL_AL_TX_DELAY_OFF) ? back_off_time - WAL_AL_TX_DELAY_OFF : 0;
    ...
    if (ret == OAL_SUCC) {
        ...
        *param = *param + off_set;
        ...
        if (ret == OAL_SUCC) {
            ...
            *param = *param + off_set;
            set_bcast_param->tpc_code_carried = OSAL_TRUE;
        }
