...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
OSAL_STATIC osal_void mac_vap_init_clear_ip_flag(hmac_vap_stru *hmac_vap)
{
    osal_u32 nv_ret;
    osal_u8 nv_data;
    osal_u16 nv_data_len = 0;
 
    hmac_vap->disable_clear_ip = OSAL_FALSE;
    // read nv
    nv_ret = uapi_nv_read(NV_ID_CLR_IP_DISABLE, sizeof(nv_data), &nv_data_len, &nv_data);
    if (nv_ret == OAL_SUCC) {
        hmac_vap->disable_clear_ip = nv_data;
    }
}
#endif
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
    mac_vap_init_clear_ip_flag(hmac_vap);
#endif
    osal_adapt_atomic_set(&(hmac_vap->use_cnt), 1);
    return OAL_SUCC;
}
...
osal_u32 hmac_mib_get_bss_type_etc(hmac_vap_stru *hmac_vap, osal_u8 *len, osal_u8 *param)
{
    ...
    *len = (osal_u8)OAL_SIZEOF(osal_s32);
...
osal_u32 hmac_mib_get_ssid_etc(hmac_vap_stru *hmac_vap, osal_u8 *len, osal_u8 *param)
{
    ...
    *len = (osal_u8)OAL_SIZEOF(mac_cfg_ssid_param_stru);
...
osal_u32 hmac_mib_get_dtim_period_etc(hmac_vap_stru *hmac_vap, osal_u8 *len, osal_u8 *param)
{
    ...
    *len = (osal_u8)OAL_SIZEOF(osal_u32);
...
osal_u32 hmac_vap_save_app_ie_etc(hmac_vap_stru *hmac_vap, oal_app_ie_stru *app_ie, app_ie_type_uint8 type)
{
    ...
    if (memcpy_s((void *)hmac_vap->app_ie[type].ie, ie_len, (void *)app_ie->ie, ie_len) != EOK) {
        oal_mem_free(hmac_vap->app_ie[type].ie, OAL_TRUE);
        hmac_vap->app_ie[type].ie = OAL_PTR_NULL;
        hmac_vap->app_ie[type].ie_len = 0;
        oam_error_log2(0, OAM_SF_CFG, "hmac_vap_save_app_ie_etc::memcpy fail, len[%d], type[%d]", app_ie->ie_len, type);
        return OAL_FAIL;
    }
    hmac_vap->app_ie[type].ie_len = (osal_u16)ie_len;
...
osal_u32 hmac_vap_add_wep_key(hmac_vap_stru *hmac_vap, osal_u8 key_idx, osal_u8 key_len, osal_u8 *key)
{
    ...
    (osal_void)memcpy_s(wep_key->key, WLAN_WPA_KEY_LEN, key, key_len);
