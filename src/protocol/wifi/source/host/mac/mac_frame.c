...
#define WPA_KEY_INFO_INSTALL (1U << 6)
#define WPA_KEY_INFO_ACK (1U << 7)
#define WPA_KEY_INFO_MIC (1U << 8)
#define WPA_KEY_INFO_SECURE (1U << 9)
#define WPA_KEY_INFO_ENCR_KEY_DATA (1U << 12)
...
static osal_u8 mac_set_he_tx_rx_mcs_nss_field(const hmac_vap_stru *hmac_vap, const osal_u8 *buffer)
{
    ...
    len = (osal_u8)sizeof(mac_fram_he_mac_nsss_set_stru);
...
osal_void mac_set_dsss_params_etc(const osal_void *vap, osal_u8 *buffer, osal_u8 *ie_len, osal_u8 chan_num)
{
    ...
    ***************************************************************************/
    /* 5G下beacon或者probe rsp帧中不携带dsss ie信息，因此,导致有些商用网卡
       ...
       携带dsss ie信息，并且发现多款商用ap，5G only也携带了dsss ie信息 */
    unref_param(vap);
...
osal_void hmac_set_ht_opern_ie_etc(osal_void *vap, osal_u8 *buffer, osal_u8 *ie_len)
{
    ...
    *(buffer + 1) = MAC_HT_OPERN_LEN;
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
    if (is_legacy_ap(hmac_vap) &&
        uapi_nv_read(NV_ID_SOFTAP_FIX_DSPS, sizeof(nv_data), &nv_data_len, &nv_data) == OAL_SUCC) {
    ...
    /* 主信道编号 */
    ht_opern->primary_channel = chn_num != 0 ? chn_num : hmac_vap->channel.chan_number;
...
{
    mac_eapol_key_stru *key = NULL;
    osal_u16 keyinfo, key_data_length;
    mac_eapol_num idx;
    osal_char *eapol_str[MAC_EAPOL_BUTT] = {"1/4", "2/4", "3/4", "4/4", "1/2", "2/2"};
    ...
    keyinfo = (key->key_info[0] << 8) | key->key_info[1]; /* 8: 高位移位 */
    key_data_length = (key->key_data_length[0] << 8) | key->key_data_length[1]; /* 8: 高位移位 */

    if ((key->key_info[1] & WPA_KEY_INFO_KEY_TYPE) != 0) {
        if ((keyinfo & WPA_KEY_INFO_ACK) != 0) {
            idx = ((keyinfo & WPA_KEY_INFO_INSTALL) != 0) ? MAC_EAPOL_PTK_3_4 : MAC_EAPOL_PTK_1_4;
        } else {
            idx = (key_data_length == 0) ? MAC_EAPOL_PTK_4_4 : MAC_EAPOL_PTK_2_4;
        }
    } else {
        idx = ((keyinfo & WPA_KEY_INFO_ACK) != 0) ? MAC_EAPOL_GTK_1_2 : MAC_EAPOL_GTK_2_2;
    }

    wifi_printf_always("wifi:%s eapol %s\r\n", dir_str, eapol_str[idx]);
}

osal_bool hmac_is_rx_ucast_frame_for_current_bss(const osal_void *vap, const osal_u8 *mac_hdr)
{
    osal_u8 dest_mac[WLAN_MAC_ADDR_LEN];
    const hmac_vap_stru *hmac_vap = vap;

    mac_get_address1((osal_u8 *)mac_hdr, dest_mac);
    if (ether_is_multicast(dest_mac) == OSAL_TRUE) {
        return OSAL_FALSE;
    }
    if (memcmp(dest_mac, mac_mib_get_station_id(hmac_vap), WLAN_MAC_ADDR_LEN) != 0) {
        return OSAL_FALSE;
    }
    return OSAL_TRUE;
}
