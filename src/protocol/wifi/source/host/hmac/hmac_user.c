...
static mac_ap_type_enum_uint8 hmac_compability_ap_tpye_identify_etc(hmac_vap_stru *hmac_vap, osal_u8 *mac_addr)
{
    ...
    if (fhook != OSAL_NULL &&
        ((hmac_btcoex_check_ap_type_blacklist_cb)fhook)(hmac_vap, mac_addr, &support_max_nss) != OSAL_TRUE) {
...
#endif
    frw_util_notifier_notify(WLAN_UTIL_NOTIFIER_EVENT_DEL_USER_COMPLETE, hmac_vap);
    if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
    ...
    oal_netbuf_stru *net_buf)
{
    ...
    /* 填写tx rx公共部分 */
    tx_ctrl->frame_header_length = (osal_u16)sizeof(mac_ieee80211_qos_frame_stru);
    tx_ctrl->mpdu_num = 1;
    tx_ctrl->msdu_num = 1;
...
osal_u32  hmac_config_user_rate_info_syn_etc(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user)
{
    ...
    msg_info.data_len = (osal_u16)sizeof(mac_h2d_usr_rate_info_stru);
