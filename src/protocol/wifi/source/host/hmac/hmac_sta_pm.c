...
OSAL_STATIC osal_s32 hmac_config_set_sta_ps_mode(hmac_vap_stru *hmac_vap, osal_u8 cfg_pm_mode)
{
    ...
    if (hmac_vap == OSAL_NULL) {
        oam_warning_log0(0, OAM_SF_PWR, "{hmac_config_set_sta_ps_mode::mac_res_get_hmac_vap fail}");
        return OAL_FAIL;
    }
...
OSAL_STATIC osal_s32 hmac_config_set_sta_pm_on(hmac_vap_stru *hmac_vap, mac_cfg_ps_open_stru *pm_cfg)
{
    ...
    msg.data_len = (osal_u16)sizeof(mac_cfg_ps_open_stru);
...
osal_void hmac_config_mvap_set_sta_pm_on(hmac_vap_stru *hmac_vap, oal_bool_enum mvap_up)
{
    ...
    if (mvap_up != OSAL_FALSE && !(hmac_vap->vap_state == MAC_VAP_STATE_UP || hmac_vap->vap_state == MAC_VAP_STATE_PAUSE
...
osal_s32 hmac_sta_pm_linkloss_ignor(osal_void)
{
    ...
    if (hmac_sta_pm_ctrl->ignor_linkloss == OSAL_TRUE) {
        hmac_sta_pm_ctrl->ignor_linkloss = OSAL_FALSE;
        return OAL_SUCC;
    }
    return OAL_CONTINUE;
}
...
osal_void hmac_sta_pm_sleep_stage_proc(hmac_sta_pm_ctrl_info *hmac_sta_pm_ctrl)
{
    if (hmac_sta_pm_ctrl->pm_stage == HMAC_SLEEP_ALLOW_RCV) {
        ...
        hmac_sta_pm_ctrl->ignor_linkloss = OSAL_TRUE;
        hmac_sta_pm_stop_wdg(hmac_sta_pm_ctrl);
...
osal_u32 hmac_sta_pm_ctrl_init(osal_void)
{
    ...
    hmac_feature_hook_register(HMAC_FHOOK_PM_LINKLOSS_IGNOR, hmac_sta_pm_linkloss_ignor);
...
osal_u32 hmac_sta_pm_ctrl_exit(osal_void)
{
    ...
    hmac_feature_hook_unregister(HMAC_FHOOK_PM_LINKLOSS_IGNOR);
