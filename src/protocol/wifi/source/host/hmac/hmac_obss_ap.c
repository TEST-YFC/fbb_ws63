...
osal_u32 hmac_send_notify_chan_width(hmac_vap_stru *hmac_vap, const osal_u8 *data)
{
    ...
    netbuf = (oal_netbuf_stru *)oal_netbuf_alloc_ext(MAC_80211_FRAME_LEN + MAC_HT_NOTIFY_CHANNEL_WIDTH_LEN);
    if (netbuf == OSAL_NULL) {
    ...
    oal_netbuf_stru *netbuf)
{
    ...
    osal_u16 len = (osal_u16)(oal_netbuf_rx_frame_len(netbuf) - oal_netbuf_rx_header_len(netbuf));
    osal_u8 *ie = OSAL_NULL;
#ifdef _PRE_WLAN_FEATURE_20_40_80_COEXIST
...
{
    if (hmac_vap->channel.band == WLAN_BAND_2G) {
        return hmac_obss_check_40mhz_2g(hmac_vap, scan_record, dst_channel);
    } else {
#ifdef _PRE_WLAN_SUPPORT_5G
        return hmac_obss_check_40mhz_5g(hmac_vap, scan_record, dst_channel);
#endif
    }
    return OAL_TRUE;
}
