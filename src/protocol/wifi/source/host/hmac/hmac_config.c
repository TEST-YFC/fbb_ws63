...
#endif
OAL_STATIC osal_u8 g_custom_pkt_send_extra_cfg_enable = OSAL_FALSE;
OAL_STATIC mac_channel_stru g_negotiation_channel = {0};

OAL_STATIC osal_bool hmac_custom_pkt_send_recovery_cfg(osal_void *notify_data);
...
#endif
#if defined(_PRE_WLAN_FEATURE_TEMP_PROTECT) && defined(_PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION)
osal_void hmac_send_temp_protect_action(osal_s16 ave_temp, osal_u8 state);
#endif
...
osal_s32  hmac_config_get_mode_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    (void)memcpy_s(msg->rsp, sizeof(prot_param), &prot_param, sizeof(prot_param));
...
{
    osal_u8 i;
    mac_cfg_sta_conn_paras *data = OAL_PTR_NULL;

    if (OAL_UNLIKELY((hmac_vap == OAL_PTR_NULL) || (msg == OAL_PTR_NULL) || (msg->data == OAL_PTR_NULL) ||
        (msg->data_len != OAL_SIZEOF(mac_cfg_sta_conn_paras)))) {
        return OAL_FAIL;
    }

    data = (mac_cfg_sta_conn_paras *)msg->data;
    for (i = 0; i < MAC_CONFIG_CONNECT_PARA_MAX; i++) {
        if ((data->bitmap & BIT(i)) == BIT(i)) {
            hmac_set_sta_conn_paras(hmac_vap, i, data->conn_paras[i]);
        }
    }
...
{
    if (OAL_UNLIKELY((hmac_vap == OAL_PTR_NULL) || (msg == OAL_PTR_NULL) || (msg->rsp == OAL_PTR_NULL) ||
        (msg->rsp_buf_len != OAL_SIZEOF(osal_s8)))) {
        return OAL_FAIL;
    }

    *((osal_s8 *)msg->rsp) = hmac_vap->data_frame_rssi;
    msg->rsp_len = (osal_u16)OAL_SIZEOF(osal_s8);
...
OAL_STATIC osal_u32 hmac_config_set_freq_prepro(hmac_vap_stru *hmac_vap, osal_u8 channel)
{
    ...
    if ((hmac_vap->vap_state == MAC_VAP_STATE_STA_FAKE_UP) && (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA)) {
        hmac_vap->sta_last_conn_channel = hmac_vap->channel.chan_number;
    }
...
{
    if (hmac_vap == OSAL_NULL || hmac_vap->vap_mode != WLAN_VAP_MODE_BSS_STA) {
        return;
    }

    (osal_void)memcpy_s(&g_negotiation_channel, sizeof(mac_channel_stru),
        &hmac_vap->channel, sizeof(mac_channel_stru));
}
...
osal_s32 hmac_config_do_channel_off(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 单STA UP情况下做保护切离 */
    if (hmac_device->vap_num == 1 && hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA &&
        hmac_device_calc_up_vap_num_etc(hmac_device) == 1 && g_negotiation_channel.chan_number != 0) {
        if (g_negotiation_channel.chan_number == hmac_vap->channel.chan_number &&
            g_negotiation_channel.chan_number != channel) {
            /* 从协商信道切离, 先pause vap, 再进行切离 */
            hmac_vap_pause_tx(hmac_vap);
            hmac_config_set_freq_etc(hmac_vap, msg);
        } else if (g_negotiation_channel.chan_number != hmac_vap->channel.chan_number &&
            g_negotiation_channel.chan_number == channel) {
            /* 从其他信道切回协商信道后，需要恢复home信道上被暂停的发送，包括虚假队列包的搬移 */
            hmac_config_set_freq_etc(hmac_vap, msg);
            hmac_vap_resume_tx_by_chl(hmac_device, hmac_vap->hal_device, &g_negotiation_channel);
        } else {
            /* 其他情况，直接切离 */
            hmac_config_set_freq_etc(hmac_vap, msg);
        }
        oam_warning_log3(0, OAM_SF_CFG, "hmac_config_do_channel_off::home_chn[%d], cur_chn[%d], new_chn[%d]",
            g_negotiation_channel.chan_number, hmac_vap->channel.chan_number, channel);
        return OAL_SUCC;
    }

    return hmac_config_set_freq_etc(hmac_vap, msg);
}
...
osal_s32  hmac_config_get_freq_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    *((osal_u32 *)msg->rsp) = hmac_vap->channel.chan_number;
    if ((hmac_vap->vap_state == MAC_VAP_STATE_STA_FAKE_UP) && (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA)) {
        *((osal_u32 *)msg->rsp) = hmac_vap->sta_last_conn_channel;
    }
    ...
    mac_cfg_kick_user_param_stru *kick_user_param)
{
    ...
    oam_warning_log1(0, OAM_SF_UM,
        "hmac_config_kick_user_proc:send DISASSOC,err code[%u]", kick_user_param->reason_code);
...
OAL_STATIC osal_u32 hmac_config_connect_check_bss_dscr(hmac_scanned_bss_info *scanned_bss,
    hmac_vap_stru *hmac_vap, mac_conn_param_stru *connect_param)
{
    osal_u32 ret, buf_size;
    osal_void *fhook = OSAL_NULL;
    mac_bss_dscr_stru *bss_dscr = OAL_PTR_NULL;

    buf_size = (osal_u32)OAL_SIZEOF(hmac_scanned_bss_info) + WLAN_MGMT_NETBUF_SIZE -
        (osal_u8)OAL_SIZEOF(scanned_bss->bss_dscr_info.mgmt_buff);
    ret = hmac_scan_get_scanned_bss_item_by_bssid(hmac_vap, connect_param->bssid, scanned_bss, (osal_u16)buf_size);
    if (ret != OAL_SUCC) {
        oam_warning_log4(0, 0, "{hmac_config_connect_check_bss_dscr::find bss fail, bssid:%02X:%02X:%02X:%02X:XX:XX}",
...
osal_s32 hmac_config_connect_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 获取扫描的bss信息, 按管理帧最大长度申请内存 */
    scanned_bss = hmac_scan_alloc_scanned_bss(WLAN_MGMT_NETBUF_SIZE);
    if (scanned_bss == OAL_PTR_NULL) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_connect_etc::malloc bss fail}");
        return OAL_FAIL;
    }

    ret = (osal_s32)hmac_config_connect_check_bss_dscr(scanned_bss, hmac_vap, connect_param);
    if (ret != OAL_CONTINUE) {
    ...
    bss_dscr = &(scanned_bss->bss_dscr_info);
    wifi_printf("[%s] [%d] en_bandwidth[%d] chan_number[%d]\r\n", __func__, __LINE__,
        bss_dscr->channel_bandwidth, bss_dscr->st_channel.chan_number);

    ret = (osal_s32)hmac_config_connect_set_param(bss_dscr, hmac_vap, connect_param, &conn_sec);
    if (ret != OAL_CONTINUE) {
        oal_free(scanned_bss);
        return ret;
    }
    ...
    ***************************************************************************/
    ...
    ret = hmac_config_connect(hmac_vap, &msg_info);
    if (OAL_UNLIKELY(ret != OAL_SUCC)) {
        oal_free(scanned_bss);
        oam_error_log2(0, OAM_SF_CFG, "vap_id[%d] {hmac_config_connect_etc::connect fail[%d]}", hmac_vap->vap_id, ret);
        return ret;
    }

    ret = (osal_s32)hmac_sta_initiate_join_etc(hmac_vap, bss_dscr);
    oal_free(scanned_bss);
...
osal_u32 hmac_config_ch_status_sync(hmac_device_stru *hmac_device)
{
    ...
    (void)memcpy_s((osal_u8 *)ap_channel_list, sizeof(ap_channel_list),
        (osal_u8 *)(hmac_device->ap_channel_list), OAL_SIZEOF(ap_channel_list));
...
OAL_STATIC osal_s32 hmac_config_offline_cali_finish(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hwifi_get_data_sample() == OSAL_FALSE) {
        ...
        wifi_printf("wifi_host_init_finish![wifi_cali1 cost %d ms].\n", run_time);
        return OAL_SUCC;
    }
    ...
    current_timestamp = (osal_u32)osal_get_time_stamp_ms();
    run_time = osal_get_runtime(start_timestamp, current_timestamp);
    wifi_printf("wifi_host_init_finish![wifi_cali2 cost %d ms].\n", run_time);
    return ret;
}
#endif
...
OAL_STATIC osal_s32 hmac_config_set_cus_dts_cali(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    current_timestamp = (osal_u32)osal_get_time_stamp_ms();
    run_time = osal_get_runtime(start_timestamp, current_timestamp);
    oam_warning_log3(0, OAM_SF_CFG, "vap_id[%d] {hmac_config_set_cus_dts_cali::cali time[%u]ms ret[%u]}",
        hmac_vap->vap_id, run_time, ret);
    return OAL_SUCC;
}
...
OAL_STATIC osal_s32 hmac_config_set_power_cali(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    fe_efuse_get_all_power_cali((osal_u8 *)msg->data, (osal_u8)msg->data_len); /* 读efuse覆盖消息数据 */
...
OAL_STATIC osal_s32 hmac_config_set_rssi(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    fe_efuse_get_all_rssi_comp((osal_u8 *)msg->data, (osal_u8)msg->data_len); /* 读efuse覆盖消息数据 */
...
OAL_STATIC osal_bool hmac_config_record_conn_info(osal_void *notify_data)
{
    ...
    if (state_change_notify->new_state == MAC_VAP_STATE_UP) {
        ...
        (osal_void)memcpy_s(&g_negotiation_channel, sizeof(mac_channel_stru),
            &hmac_vap->channel, sizeof(mac_channel_stru));
    } else if (state_change_notify->new_state == MAC_VAP_STATE_STA_FAKE_UP) {
        /* 清除STA协商时的信道信息 */
        (osal_void)memset_s(&g_negotiation_channel, sizeof(mac_channel_stru), 0, sizeof(mac_channel_stru));
    }
    return OSAL_TRUE;
}
...
osal_void hmac_cfg_init(osal_void)
{
    ...
    frw_msg_hook_register(WLAN_MSG_W2H_C_CFG_GET_VENDOR_IE, hmac_config_get_vendor_ie);
...
#endif
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_DOWN_VAP, hmac_custom_pkt_send_recovery_cfg);
    frw_util_notifier_register(WLAN_UTIL_NOTIFIER_EVENT_VAP_STATE_CHANGE, hmac_config_record_conn_info);
#if defined(_PRE_WLAN_FEATURE_TEMP_PROTECT) && defined(_PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION)
    hmac_alg_register_temp_protect_notify_func(hmac_send_temp_protect_action);
#endif
}

osal_void hmac_cfg_exit(osal_void)
{
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_DOWN_VAP, hmac_custom_pkt_send_recovery_cfg);
    frw_util_notifier_unregister(WLAN_UTIL_NOTIFIER_EVENT_VAP_STATE_CHANGE, hmac_config_record_conn_info);
#if defined(_PRE_WLAN_FEATURE_TEMP_PROTECT) && defined(_PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION)
    hmac_alg_unregister_temp_protect_notify_func();
#endif
}
...
osal_s32 hmac_config_set_scan_params_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    hmac_vap->single_probe_send_times = scan_param->single_probe_send_times; /* 目前配置值[1,3], 3bit 够用 */

    return OAL_SUCC;
}

#ifdef _PRE_WLAN_FEATURE_WPA3
#define HMAC_EXT_AUTH_FAIL 5210
OAL_STATIC osal_u32 hmac_config_external_auth_add_pmksa(hmac_vap_stru *hmac_vap, hmac_external_auth_stru ext_auth)
{
    ...
    if (entry != OAL_PTR_NULL) {
        ...
        (void)memcpy_s(entry->pmkid, WLAN_PMKID_LEN, ext_auth.pmkid, WLAN_PMKID_LEN);
    } else {
        ...
        (void)memcpy_s(new_pmksa->pmkid, WLAN_PMKID_LEN, ext_auth.pmkid, WLAN_PMKID_LEN);
        (void)memcpy_s(new_pmksa->mac_addr, WLAN_MAC_ADDR_LEN, ext_auth.bssid, WLAN_MAC_ADDR_LEN);
...
osal_s32 hmac_config_external_auth(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if ((ext_auth.status != MAC_SUCCESSFUL_STATUSCODE) && (hmac_user->user_asoc_state != MAC_USER_STATE_ASSOC)) {
        ...
        if (ext_auth.status != MAC_ANTI_CLOGGING) {
            hmac_report_connect_failed_result_etc(hmac_vap, ext_auth.status + HMAC_EXT_AUTH_FAIL);
        }
        /* 删除用户 */
...
#define HH503_DIAG_CTL_CFG_PKT_RAM_EN (HH503_DIAG_CTL_BASE + 0x200)
#define HH503_DIAG_CTL_CFG_PKT_RAM_CLK_EN (HH503_DIAG_CTL_BASE + 0x208)
#define HH503_DIAG_CTL_CFG_MONITOR_SEL (HH503_DIAG_CTL_BASE + 0x210)
#define HH503_DIAG_CTL_CFG_SAMPLE_SEL (HH503_DIAG_CTL_BASE + 0x218)
#define HH503_DIAG_CTL_CFG_SAMPLE_MODE (HH503_DIAG_CTL_BASE + 0x220)
#define HH503_DIAG_CTL_CFG_SAMPLE_LENGTH (HH503_DIAG_CTL_BASE + 0x228)
#define HH503_DIAG_CTL_CFG_SAMPLE_START_ADDR (HH503_DIAG_CTL_BASE + 0x230)
#define HH503_DIAG_CTL_CFG_SAMPLE_END_ADDR (HH503_DIAG_CTL_BASE + 0x238)
#define HH503_DIAG_CTL_SAMPLE_DONE (HH503_DIAG_CTL_BASE + 0x248)
#define HH503_DIAG_CTL_CFG_MONITOR_CLOCK (HH503_DIAG_CTL_BASE + 0x288)
#define HH503_DIAG_CTL_CFG_MUX_TEST       (HH503_DIAG_CTL_BASE + 0x2A0)
#define HH503_DIAG_CTL_CFG_ADC_DAC_TEST       (HH503_DIAG_CTL_BASE + 0x2F8)
#define HH503_DIAG_CTL_PKE_MEM_OBS_SAMPLE_ADDR (HH503_DIAG_CTL_BASE + 0x390)
...
osal_s32 hmac_config_set_freq(hmac_vap_stru *hmac_vap, osal_u8 value)
{
    ...
    if ((hal_device->hal_dev_fsm.oal_fsm.cur_state != HAL_DEVICE_BUTT_STATE) ||
...
#endif
        if (sta_channel->band == WLAN_BAND_2G) {
            oam_warning_log0(0, OAM_SF_ANY,
...
OAL_STATIC osal_bool hmac_custom_pkt_send_recovery_cfg(osal_void *notify_data)
{
    ...
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    osal_u8 recovery_record = 0;

    if (g_custom_pkt_send_extra_cfg_enable != OSAL_TRUE) {
        wifi_printf("{hmac_custom_pkt_send_recovery_cfg::no need recovery cfg}\r\n");
        return OAL_TRUE;
    }

    /* STA down事件 恢复未关联时对任意帧的配置 */
    if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
        recovery_record |= BIT0;
        ...
        g_custom_pkt_send_extra_cfg_enable = OSAL_FALSE;
    }

    /* 未发现UP的vap，且当前非扫描状态，恢复PA */
    if (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING &&
        hmac_device_calc_up_vap_num_etc(hmac_device) == 0) {
        recovery_record |= BIT1;
    }

    wifi_printf("{hmac_custom_pkt_send_recovery_cfg::recovery record[%d]}\r\n", recovery_record);
    return OAL_TRUE;
}
...
OAL_STATIC osal_void hmac_config_prepare_custom_pkt_send(hmac_vap_stru *hmac_vap, osal_u16 user_idx)
{
    ...
    if (hmac_vap->hal_device->hal_dev_fsm.oal_fsm.cur_state == HAL_DEVICE_IDLE_STATE &&
        ...
        wifi_printf("{hmac_config_prepare_custom_pkt_send::hal_vap change to work state, open PA.}");
        hal_device_handle_event(hmac_vap->hal_device, HAL_DEVICE_EVENT_JOIN_COMP, sizeof(hal_to_dmac_vap_stru),
                                (osal_u8 *)(hmac_vap->hal_vap));
        g_custom_pkt_send_extra_cfg_enable = OSAL_TRUE;
    }
}
...
osal_s32 hmac_config_send_custom_pkt(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    len = msg->data_len;
    /* 申请管理帧内存 */
    netbuf = oal_netbuf_alloc_ext(len);
    if (netbuf == OSAL_NULL) {
    ...
    if (memset_s(oal_netbuf_cb(netbuf), sizeof(netbuf->cb), 0, sizeof(netbuf->cb)) != EOK) {
    ...
    tx_ctl->frame_type = msg->data[0] & 0x0F;
...
osal_s32 hmac_config_query_status(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if (hmac_vap == OSAL_NULL) {
        oam_warning_log0(0, 0, "{hmac_config_query_status::hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    if (memcpy_s(msg->rsp, msg->rsp_buf_len, &rsp, sizeof(hmac_stat_info_rsp_stru)) != EOK) {
        oam_warning_log1(0, 0, "vap_id[%d] {hmac_config_query_status::memcpy_s fail.}", hmac_vap->vap_id);
        return OAL_FAIL;
    }
    msg->rsp_len = sizeof(hmac_stat_info_rsp_stru);
...
osal_s32  hmac_config_query_rssi(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    mac_cfg_query_rssi_stru user_info = {0};

    if ((hmac_vap == OSAL_NULL) || (msg->data == OSAL_NULL) || (msg->data_len != WLAN_MAC_ADDR_LEN)) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_get_user_bset_rate_and_rssi:: hmac_vap or msg is Null .}");
        return OAL_FAIL;
    }
    ...
    (osal_void)memset_s(&mac_addr, WLAN_MAC_ADDR_LEN, 0, WLAN_MAC_ADDR_LEN);
    ...
    /* 查找用户 */
    (osal_void)memcpy_s(mac_addr, WLAN_MAC_ADDR_LEN, (osal_u8 *)msg->data, msg->data_len);
    hmac_user = mac_vap_get_hmac_user_by_addr_etc(hmac_vap, (const osal_u8 *)mac_addr);
    if (hmac_user == OSAL_NULL) {
        oam_warning_log1(0, 0, "vap_id[%d] {hmac_config_query_rssi::hmac_user null.}", hmac_vap->vap_id);
        return OAL_FAIL;
    }
    ...
    ret = send_cfg_to_device(hmac_vap->vap_id, WLAN_MSG_W2D_C_CFG_GET_TX_PARAMS, &cfg_info, OSAL_TRUE);
    if ((ret != OAL_SUCC) || (cfg_info.rsp == OSAL_NULL)) {
        oam_warning_log1(0, OAM_SF_ANY, "{hmac_config_query_rssi::get rate fail, ret[%d]}", ret);
        return OAL_FAIL;
    }

    user_info.rssi = oal_get_real_rssi(hmac_user->rx_rssi);
    user_info.free_power = hmac_config_get_free_power(hmac_vap);
    user_info.best_rate = rsp_info.tx_best_rate / 100; /* 除以100：把单位转换为kbps */

    msg->rsp_len = (osal_u16)sizeof(mac_cfg_query_rssi_stru);
    (osal_void)memcpy_s(msg->rsp, sizeof(mac_cfg_query_rssi_stru), (osal_u8 *)&user_info,
        sizeof(mac_cfg_query_rssi_stru));
    return OAL_SUCC;
}

static oal_bool_enum_uint8 hmac_config_is_vendor_ie(const osal_u8 *ie)
{
    osal_u8 i;
    const osal_u8 ie_table[][6] = {
        {MAC_EID_P2P, 0, WFA_OUI_BYTE1, WFA_OUI_BYTE2, WFA_OUI_BYTE3, WFA_P2P_V1_0},
        {MAC_EID_WPA, 0, MAC_WLAN_OUI_MICRO0, MAC_WLAN_OUI_MICRO1, MAC_WLAN_OUI_MICRO2, MAC_OUITYPE_WPA},
        {MAC_EID_WFA_TPC_RPT, 0, MAC_WLAN_OUI_MICRO0, MAC_WLAN_OUI_MICRO1, MAC_WLAN_OUI_MICRO2, MAC_OUITYPE_WPS},
        {MAC_EID_WFA_TPC_RPT, 0, WFA_OUI_BYTE1, WFA_OUI_BYTE2, WFA_OUI_BYTE3, MAC_OUITYPE_WFA},
    };

    if (hmac_is_wmm_ie_etc(ie) == OSAL_TRUE) {
        return OSAL_FALSE;
    }

    for (i = 0; i < sizeof(ie_table) / sizeof(ie_table[0]); i++) {
        /* 0:EID, 2:OUI第一位, 3:OUI第二位 */
        if ((ie[0] == ie_table[i][0]) && (ie[2] == ie_table[i][2]) && (ie[3] == ie_table[i][3]) &&
            /* 4:OUI第三位, 5:OUI type */
            (ie[4] == ie_table[i][4]) && (ie[5] == ie_table[i][5])) {
            return OSAL_FALSE;
        }
    }
...
/*****************************************************************************
 功能描述 : 查找厂家自定义 IE
*****************************************************************************/
static osal_u16 hmac_get_vendor_ie_etc(hmac_vap_stru *hmac_vap, frw_msg *msg, hmac_scanned_bss_info *scanned_bss,
    osal_u8 *mac_addr)
{
    struct mac_ieee80211_vendor_ie *ie = OSAL_NULL;
    osal_u8 *pos = OSAL_NULL;
    osal_u8 *end = OSAL_NULL;
    osal_u16 current_copy_len = 0;
    osal_u16 bss_info_size;
    osal_u8 *ies = OSAL_NULL;
    osal_u32 ies_len;
    osal_u32 offset;

    bss_info_size = (osal_u16)OAL_SIZEOF(hmac_scanned_bss_info) + WLAN_MGMT_NETBUF_SIZE -
        (osal_u16)OAL_SIZEOF(scanned_bss->bss_dscr_info.mgmt_buff);
    if (hmac_scan_get_scanned_bss_item_by_bssid(hmac_vap, mac_addr, scanned_bss, bss_info_size) != OAL_SUCC) {
        oam_warning_log0(0, 0, "{hmac_get_vendor_ie_etc::find bss failed}");
        return 0;
    }
    offset = MAC_80211_FRAME_LEN + MAC_TIME_STAMP_LEN + MAC_BEACON_INTERVAL_LEN + MAC_CAP_INFO_LEN;
    if (scanned_bss->bss_dscr_info.mgmt_len < offset) {
        return 0;
    }

    ies = scanned_bss->bss_dscr_info.mgmt_buff + offset;
    ies_len = (osal_u32)(scanned_bss->bss_dscr_info.mgmt_len - offset);
    pos = ies;
    end = ies + ies_len;
    while (pos < end) {
        pos = mac_find_ie_etc(MAC_EID_VENDOR, pos, (osal_s32)(end - pos));
        if (pos == OSAL_NULL) {
            return current_copy_len;
        }
        ie = (struct mac_ieee80211_vendor_ie *)pos;
        if (ie->len >= (sizeof(*ie) - MAC_IE_HDR_LEN)) {
            if (hmac_config_is_vendor_ie((const osal_u8 *)ie) != OSAL_TRUE) {
                pos += MAC_IE_HDR_LEN + ie->len;
                continue;
            }
            if (msg->rsp_buf_len - current_copy_len < MAC_IE_HDR_LEN + ie->len) {
                return current_copy_len;
            }
            if (memcpy_s(msg->rsp + current_copy_len, msg->rsp_buf_len - current_copy_len, pos,
                MAC_IE_HDR_LEN + ie->len) != EOK) {
                return current_copy_len;
            }
            current_copy_len += MAC_IE_HDR_LEN + ie->len;
        }
        pos += MAC_IE_HDR_LEN + ie->len;
    }
    return current_copy_len;
...
osal_s32 hmac_config_get_vendor_ie(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    if ((hmac_vap == OSAL_NULL) || (msg->data == OSAL_NULL) || (msg->data_len != WLAN_MAC_ADDR_LEN) ||
        (msg->rsp == OSAL_NULL) || (msg->rsp_buf_len == 0)) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_get_vendor_ie:: invalid param .}");
        return OAL_FAIL;
    }

    if (memcpy_s(mac_addr, WLAN_MAC_ADDR_LEN, (osal_u8 *)msg->data, msg->data_len) != EOK) {
        oam_warning_log0(0, OAM_SF_CFG, "{hmac_config_get_vendor_ie:: memcpy_s mac_addr error.}");
        return OAL_FAIL;
    }

    size = (osal_u32)OAL_SIZEOF(hmac_scanned_bss_info) + WLAN_MGMT_NETBUF_SIZE -
    ...
    if (OAL_UNLIKELY(scanned_bss == OAL_PTR_NULL)) {
        oam_warning_log0(0, OAM_SF_SCAN,
            "{hmac_config_get_vendor_ie::alloc failed.}");
        return OAL_PTR_NULL;
    }

    (osal_void)memset_s(scanned_bss, size, 0, size);

    msg->rsp_len = hmac_get_vendor_ie_etc(hmac_vap, msg, scanned_bss, mac_addr);
    if (msg->rsp_len == 0) {
        ret = OAL_FAIL;
        goto exit;
    }
...
#if (defined(_PRE_WLAN_REG_RW_WHITELIST) || defined(CONFIG_REG_WHITELIST))
...
WIFI_TCM_TEXT WIFI_HMAC_TCM_TEXT osal_s32 hmac_config_tx_comp_schedule(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    osal_s32 ret;
...
osal_s32 hmac_hal_device_sync(hal_to_dmac_device_stru *hal_device)
{
    ...
    hal_device_info_sync_stru.fix_power_level = hal_device->fix_power_level;
...
osal_s32  hmac_config_beacon_chain_switch(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    osal_u8 value;
    if (hmac_vap == OSAL_NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_config_beacon_chain_switch::hmac_vap null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }
...
OAL_STATIC osal_s32 hmac_config_mgmt_report_switch(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    wifi_printf("hmac_set_mgmt_report_switch:switch[%d] vap_mode[%d]\r\n", g_mgmt_report_enable, hmac_vap->vap_mode);

    /* mode = 1: 允许接收其他BSS的广播/组播管理帧 */
    if (g_mgmt_report_enable == 1) {
        /* 使能混杂开关，防止状态机切换导致帧过滤配置失效 */
        if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
            hmac_vap->hal_device->promis_switch = 1;
            hmac_hal_device_sync(hmac_vap->hal_device);
        }
        filter = 0;
        frw_msg_init((osal_u8 *)&filter, sizeof(osal_u8), OSAL_NULL, 0, &cfg);
        ret = frw_send_msg_to_device(hmac_vap->vap_id, WLAN_MSG_H2D_C_CFG_FILTER_OTHER_BSS_MGMT, &cfg, OSAL_TRUE);
        if (ret != OAL_SUCC) {
            oam_warning_log1(0, OAM_SF_CFG, "{hmac_config_mgmt_report_switch: send msg failed ret[%d]}", ret);
        }
    } else if (g_mgmt_report_enable == 0) {
        /* STA断连时，关闭混杂开关 */
        if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
            hmac_vap->hal_device->promis_switch = 0;
            hmac_hal_device_sync(hmac_vap->hal_device);
        }
    }
    return OAL_SUCC;
...
#if defined(_PRE_WLAN_FEATURE_TEMP_PROTECT) && defined(_PRE_WLAN_FEATURE_TEMP_PROTECT_ACTION)
osal_void hmac_encap_temp_protect_action(oal_netbuf_stru *buf, osal_u16 *len, hmac_vap_stru *hmac_vap, osal_s16 temp,
    osal_u8 state)
{
    mac_ieee80211_frame_stru *mac_header = (mac_ieee80211_frame_stru *)oal_netbuf_header(buf);
    osal_u8 *payload_addr = (osal_u8 *)oal_netbuf_data_offset(buf, MAC_80211_FRAME_LEN);
    ...
    (osal_void)memcpy_s((osal_u8 *)mac_header + WLAN_HDR_ADDR1_OFFSET, WLAN_MAC_ADDR_LEN,
        BROADCAST_MACADDR, WLAN_MAC_ADDR_LEN);
    (osal_void)memcpy_s((osal_u8 *)mac_header + WLAN_HDR_ADDR2_OFFSET, WLAN_MAC_ADDR_LEN,
        mac_mib_get_station_id(hmac_vap), WLAN_MAC_ADDR_LEN);
    (osal_void)memcpy_s((osal_u8 *)mac_header + WLAN_HDR_ADDR3_OFFSET, WLAN_MAC_ADDR_LEN,
        hmac_vap->bssid, WLAN_MAC_ADDR_LEN);
    *len += MAC_80211_FRAME_LEN;

    payload_addr[0] = (temp >> 8) & 0xFF; /* 8:取高位 */
    payload_addr[1] = temp & 0xFF;
    *len += (sizeof(osal_s16));

    payload_addr[2] = state; /* 2:state */
    *len += (sizeof(osal_u8));
}
...
osal_void hmac_send_temp_protect_action(osal_s16 ave_temp, osal_u8 state)
{
    ...
    hmac_encap_temp_protect_action(mgmt_buf, &len, hmac_vap, ave_temp, state);
    /* 调用发送管理帧接口 */
    ...
    tx_ctl->frame_subtype  = 0x0D; /* 0x0D:自定义报文类型 */
    if (hmac_tx_mgmt(hmac_vap, mgmt_buf, len, OSAL_FALSE) != OAL_SUCC) {
