...
osal_s32 hmac_get_vap_stat_new(dft_vap_statis_command_stru *vap_statis_command)
{
    ...
    (void)memcpy_s((osal_u8*)(g_hmac_vap_pkts_stat[vap_id] + HOST_PKTS_CNT_TYPE_END),
...
#ifdef _PRE_WLAN_DFT_STAT_ICMP
osal_void hmac_icmp_parse(mac_ip_header_stru *iph, osal_u16 ether_type, icmp_trx_type_enum_u8 pos)
{
    mac_icmp_header_stru *icmph = OSAL_NULL;
    osal_char *pos_str[ICMP_TRX_TYPE_BUTT] = {"RX_D2H", "RX_H2O", "TX_O2H", "TX_H2D"};

    if (ether_type != oal_host2net_short(ETHER_TYPE_IP)) {
        return;
    }

    if (iph->protocol != MAC_ICMP_PROTOCAL) {
        return;
    }

    icmph = (mac_icmp_header_stru *)(iph + 1);
    wifi_printf("[%u][%s][%s] icmp_seq[%d]\r\n", (osal_u32)oal_time_get_stamp_ms(), pos_str[pos],
        ((icmph->icmp_type == MAC_ICMP_RESPONSE) ? "ping reply" : ((icmph->icmp_type == MAC_ICMP_REQUEST) ?
        "ping request" : "other icmp pkt")),
        oal_host2net_short(icmph->icmp_seq));
}

OSAL_STATIC osal_u32 hmac_rx_data_h2o_stat(oal_netbuf_stru **netbuf, hmac_vap_stru *hmac_vap)
{
    // 送给协议栈前已经将ETH head剥离，需要偏移回去获取以太头
    mac_ether_header_stru *ether_hdr =
        (mac_ether_header_stru *)(oal_netbuf_header(*netbuf) - sizeof(mac_ether_header_stru));
    mac_ip_header_stru *iph = (mac_ip_header_stru *)(ether_hdr + 1);

    hmac_icmp_parse(iph, ether_hdr->ether_type, RX_ICMP_HMAC_END);

    return OAL_CONTINUE;
}

OSAL_STATIC osal_u32 hmac_tx_data_o2h_stat(oal_netbuf_stru **netbuf,
    hmac_vap_stru *hmac_vap)
{
    mac_ether_header_stru *ether_hdr = (mac_ether_header_stru *)oal_netbuf_header(*netbuf);
    mac_ip_header_stru *iph = (mac_ip_header_stru *)(ether_hdr + 1);

    hmac_icmp_parse(iph, ether_hdr->ether_type, TX_ICMP_HMAC_START);
    return OAL_CONTINUE;
}

OSAL_STATIC osal_u32 hmac_tx_data_h2d_stat(oal_netbuf_stru **netbuf, hmac_vap_stru *hmac_vap)
{
    mac_llc_snap_stru *snap = OSAL_NULL;
    mac_ip_header_stru *iph = OSAL_NULL;
    ...
    iph = (mac_ip_header_stru *)(snap + 1);
    hmac_icmp_parse(iph, snap->ether_type, TX_ICMP_HMAC_END);
...
#ifdef _PRE_WLAN_DFT_STAT_ICMP
    snap = (mac_llc_snap_stru *)(oal_netbuf_header(*netbuf) + rx_cb->rx_info.mac_header_len);
    iph = (mac_ip_header_stru *)(snap + 1);
    hmac_icmp_parse(iph, snap->ether_type, RX_ICMP_HMAC_START);
#endif
...
#ifdef _PRE_WLAN_DFT_STAT_ICMP
hmac_netbuf_hook_stru g_vapstat_data_h2o_netbuf_hook = {
    .hooknum = HMAC_FRAME_DATA_RX_EVENT_H2O,
    .priority = HMAC_HOOK_PRI_MIDDLE,
    .hook_func = hmac_rx_data_h2o_stat,
};

hmac_netbuf_hook_stru g_vapstat_data_o2h_netbuf_hook = {
    .hooknum = HMAC_FRAME_DATA_TX_BRIDGE_IN,
    .priority = HMAC_HOOK_PRI_MIDDLE,
    .hook_func = hmac_tx_data_o2h_stat,
};

hmac_netbuf_hook_stru g_vapstat_data_h2d_netbuf_hook = {
    .hooknum = HMAC_FRAME_DATA_TX_EVENT_H2D,
    .priority = HMAC_HOOK_PRI_MIDDLE,
    .hook_func = hmac_tx_data_h2d_stat,
};
#endif
...
#ifdef _PRE_WLAN_DFT_STAT_ICMP
    (osal_void)hmac_register_netbuf_hook(&g_vapstat_data_h2d_netbuf_hook);
    (osal_void)hmac_register_netbuf_hook(&g_vapstat_data_o2h_netbuf_hook);
    (osal_void)hmac_register_netbuf_hook(&g_vapstat_data_h2o_netbuf_hook);
#endif
...
osal_s32 hmac_config_get_sniffer_info(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    (void)memcpy_s(msg->rsp, sizeof(chan_stats), (osal_u8 *)&chan_stats, sizeof(chan_stats));
