...
OAL_STATIC osal_u8 g_auth_max_retry_times = MAX_AUTH_CNT;
OAL_STATIC osal_u8 g_assoc_max_retry_times = MAX_ASOC_CNT;
...
{
    osal_u8 param_index = index;
    switch (param_index) {
        case MAC_CONFIG_AUTH_MAX_RETRY:
            g_auth_max_retry_times = (osal_u8)value;
            break;
        case MAC_CONFIG_AUTH_RECV_TIMEOUT:
            mac_mib_set_AuthenticationResponseTimeOut(hmac_vap, (osal_u32)value * 2); /* 2倍放大,使用时会除以2 */
            break;
        case MAC_CONFIG_ASSOC_MAX_RETRY:
            g_assoc_max_retry_times = (osal_u8)value;
            break;
        case MAC_CONFIG_ASSOC_RECV_TIMEOUT:
            mac_mib_set_dot11AssociationResponseTimeOut(hmac_vap, (osal_u32)value * 2); /* 2倍放大,使用时会除以2 */
            break;
        default:
            break;
    }
...
{
    return g_auth_max_retry_times;
}

osal_u8 hmac_get_assoc_max_retry_times(osal_void)
{
    return g_assoc_max_retry_times;
}
    ...
    mac_scan_req_stru *scan_params, osal_s8 *param)
{
    ...
    if (cfg80211_scan_param->acs_scan_flag == OAL_TRUE) {
        ...
        scan_params->scan_func = MAC_SCAN_FUNC_BSS | MAC_SCAN_FUNC_MEAS | MAC_SCAN_FUNC_STATS; /* 默认扫描bss */
    }
#else
    ...
    scan_params->max_scan_count_per_channel = 4; /* 4:每个信道的扫描次数 */
    ...
    if (scan_params->channel_nums == 1 && hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA &&
#ifdef _PRE_WLAN_FEATURE_P2P
        hmac_vap->p2p_mode == WLAN_LEGACY_VAP_MODE &&
#endif  /* _PRE_WLAN_FEATURE_P2P */
        hmac_vap->vap_state != MAC_VAP_STATE_UP && hmac_vap->vap_state != MAC_VAP_STATE_PAUSE) {
        scan_params->max_scan_count_per_channel += 3;   // 指定信道扫描时，发4+3=7个probe req
    }
...
osal_void hmac_prepare_join_req_etc(hmac_join_req_stru *join_req, mac_bss_dscr_stru *bss_dscr)
{
    ...
    (void)memcpy_s(&(join_req->bss_dscr), sizeof(mac_bss_dscr_stru), bss_dscr, sizeof(mac_bss_dscr_stru));
...
osal_s32 hmac_cfg80211_start_sched_scan_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 拷贝上层下发的pno扫描参数 */
    (void)memcpy_s(&pno_scan_params, sizeof(mac_pno_scan_stru), cfg80211_pno_scan, sizeof(mac_pno_scan_stru));
...
osal_void hmac_handle_auth_rsp_sta_etc(hmac_vap_stru *hmac_vap, osal_u8 *msg)
{
    ...
    hmac_user_stru *hmac_user;
    osal_void *fhook = hmac_get_feature_fhook(HMAC_FHOOK_DFX_USER_CONN_ABNORMAL_RECORD);
    osal_u16 report_code = auth_rsp->status_code;
    ...
    oam_warning_log3(0, OAM_SF_AUTH, "vap_id[%d] hmac_handle_auth_rsp_sta_etc::auth fail[%u],cnt[%u]",
                     hmac_vap->vap_id, auth_rsp->status_code, mac_mib_get_StaAuthCount(hmac_vap));
    wifi_printf_always("wifi:auth fail[%u],cnt[%u]\r\n", auth_rsp->status_code, mac_mib_get_StaAuthCount(hmac_vap));

    if ((auth_rsp->status_code == MAC_UNSUPT_ALG ||
        mac_mib_get_StaAuthCount(hmac_vap) >= hmac_get_auth_max_retry_times()) &&
    ...
    if (mac_mib_get_StaAuthCount(hmac_vap) < hmac_get_auth_max_retry_times()) {
        /* 需要将状态机设置为 */
        hmac_fsm_change_state_etc(hmac_vap, MAC_VAP_STATE_STA_JOIN_COMP);
    ...
    hmac_report_connect_failed_result_etc(hmac_vap,
        report_code > MAC_JOIN_RSP_TIMEOUT ? report_code : report_code + MAC_STATUS_AUTH_MAX);
    if (fhook != OSAL_NULL) {
...
OAL_STATIC osal_void hmac_handle_asoc_fail_proc(hmac_vap_stru *hmac_vap, const hmac_asoc_rsp_stru *asoc_rsp)
{
    ...
    osal_void *fhook = hmac_get_feature_fhook(HMAC_FHOOK_DFX_USER_CONN_ABNORMAL_RECORD);
    osal_u16 report_code = asoc_rsp->status_code;

    max_reassoc_count = hmac_get_assoc_max_retry_times();
...
#endif
    oam_warning_log3(0, OAM_SF_DFT, "hmac_handle_asoc_fail_proc: status_code[%u], max_count[%u], current_count[%u]",
        asoc_rsp->status_code, max_reassoc_count, mac_mib_get_StaAssocCount(hmac_vap));
    wifi_printf_always("wifi:assoc fail, code[%u], max_cnt[%u], cur_cnt[%u]\r\n",
        asoc_rsp->status_code, max_reassoc_count, mac_mib_get_StaAssocCount(hmac_vap));
    if (mac_mib_get_StaAssocCount(hmac_vap) >= max_reassoc_count) {
        /* 上报关联失败到wpa_supplicant */
        hmac_report_connect_failed_result_etc(hmac_vap,
            report_code >= MAC_JOIN_RSP_TIMEOUT ? report_code : report_code + MAC_STATUS_ASSOC_MAX);
        if (fhook != OSAL_NULL) {
...
void hmac_handle_assoc_rsp_succ_sta(const hmac_vap_stru *hmac_vap, hmac_asoc_rsp_stru *asoc_rsp)
{
    ...
    (void)memcpy_s(mgmt_data, asoc_rsp->asoc_rsp_ie_len,
        (osal_u8 *)asoc_rsp->asoc_rsp_ie_buff, asoc_rsp->asoc_rsp_ie_len);
...
#endif
        (void)memcpy_s(sta_asoc.auc_mac_addr, WLAN_MAC_ADDR_LEN, mac_addr, WLAN_MAC_ADDR_LEN);
    } else {
...
osal_void hmac_set_scan_param(hmac_vap_stru *hmac_vap, osal_u8 channel, mac_scan_req_stru *scan_param)
{
    ...
    scan_param->scan_func = (MAC_SCAN_FUNC_BSS | MAC_SCAN_FUNC_MEAS | MAC_SCAN_FUNC_STATS);
