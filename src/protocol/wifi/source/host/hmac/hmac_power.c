...
#ifdef _PRE_WLAN_FEATURE_ALWAYS_TX
// 用于判断是否抬压 低速率下进行抬压优化mask; 满占空比做射频滤波收窄
osal_void hmac_config_al_tx_cta_cfg(hal_to_dmac_device_stru *hal_device)
{
    osal_u8 flag = 0;   // 低4位为速率标志 高4位为占空比
    osal_u8 proto = hmac_get_al_tx_protocol();
    osal_u32 rate = hmac_get_al_tx_mcs();
    osal_u8 duty_rate = hmac_get_al_tx_duty_rate();
    if (hal_device->al_tx_flag == HAL_ALWAYS_TX_RF) {
        flag = (((proto == WLAN_LEGACY_OFDM_PHY_PROTOCOL_MODE) && (rate != 8 && rate != 12)) || // 8和12表示11g的48M和54M
            (rate < 6)) ? 1 : 0; // 非11g小于mcs6 低速置位
        flag |= ((duty_rate & 0xF) << BIT_OFFSET_4);
    }
    // 刷新是否低速 低速做抬压 使用保留位
    hal_device->dyn_cali_val.cali_chain_status_info.auc_resv[0] = flag;
}
#endif
#ifdef __cplusplus
#if __cplusplus
