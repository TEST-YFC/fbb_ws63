...
osal_u8 g_al_tx_duty_rate = 7;  // 常发占空比 默认70%
...
{
    g_al_tx_duty_rate = val;
}

osal_u8 hmac_get_al_tx_duty_rate(osal_void)
{
    return g_al_tx_duty_rate;
}
/*****************************************************************************
 功能描述  : 常发设置地址
*****************************************************************************/
...
static osal_void hmac_config_stop_always_tx(hmac_vap_stru *hmac_vap)
{
    ...
    hal_device->fix_power_level = 0; /* 恢复为默认固定功率的档位 */
...
static osal_void hmac_config_set_al_tx_param(hmac_vap_stru *hmac_vap, mac_cfg_tx_comp_stru *event_set_al_tx)
{
    ...
    /* 开常发时，不带参数，按回退功率处理 */
    if (hal_device->al_tx_flag == EXT_SWITCH_ON && event_set_al_tx->tpc_code_carried == OSAL_FALSE) {
        hal_device->fix_power_level = 1;
    }
}
...
static osal_void hmac_config_al_tx_cfg_param(mac_cfg_tx_comp_stru *event_set_al_tx)
{
    ...
    g_al_tx_cfg_param = *event_set_al_tx;
...
oal_netbuf_stru *hmac_config_create_al_tx_packet(osal_u32 size, mac_rf_payload_enum_uint8 payload_flag)
{
    ...
    buf = (tx_size <= WLAN_LARGE_NETBUF_SIZE) ?
        oal_netbuf_alloc_ext((osal_u16)tx_size) :
...
OSAL_STATIC osal_u32 hmac_config_set_pm_switch(const hmac_vap_stru *hmac_vap, osal_u8 len, const osal_u8 *param)
{
    unref_param(len);
    switch (*param) {
        case ALL_DISABLE:
...
osal_s32 hmac_config_set_always_tx(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    hmac_hal_device_sync(hal_device);
    hmac_config_al_tx_cta_cfg(hal_device);
    if (hal_device->al_tx_flag == HAL_ALWAYS_TX_RF) {
        ...
        if (hmac_config_al_tx_bcast_pkt(hmac_vap, event_set_al_tx->payload_len) != OAL_SUCC) {
...
osal_s32 hmac_config_get_rx_ppdu_info(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    ret = hal_rf_get_rx_ppdu_info(clean_flag, (osal_char *)msg->rsp, (osal_u16)msg->rsp_buf_len, &copy_len);
    if (ret != OAL_SUCC) {
        return ret;
    }

    msg->rsp_len = copy_len;
    return OAL_SUCC;
}
#ifdef __cplusplus
#if __cplusplus
