...
OAL_STATIC osal_u32 hmac_scan_obss_timeout_fn(osal_void *arg)
{
    ...
    if (scan_now == OSAL_TRUE) {
        if (g_obss_sta_scan_disable == OSAL_TRUE) {
            oam_warning_log0(0, OAM_SF_SCAN, "{hmac_scan_obss_timeout_fn::OBSS SCAN ABORT}");
            return OAL_SUCC;
        }
        /* 准备OBSS扫描参数，准备发起扫描 */
...
{
    if (hmac_vap == OSAL_NULL) {
        oam_warning_log0(0, OAM_SF_PWR, "{hmac_ie_proc_obss_scan_ie::mac_res_get_hmac_vap fail}");
        return;
    }
...
OSAL_STATIC osal_s32 hmac_ccpriv_obss_scan_disable(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    g_obss_sta_scan_disable = (osal_u8)value;
