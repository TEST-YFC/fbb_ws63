    ...
    hmac_user_stru *hmac_user, const oal_bool_enum_uint8 ps, oal_bool_enum_uint8 is_from_ps_queue)
{
    ...
    tx_ctrl->frame_header_length = (osal_u8)sizeof(mac_ieee80211_frame_stru);
    tx_ctrl->mpdu_num = 1;
...
static osal_u32 hmac_psm_sch_tid_queue(hmac_user_stru *hmac_user, osal_u32 ps_mpdu_num)
{
    ...
    for (tid_idx = 0; tid_idx < WLAN_TID_MAX_NUM; tid_idx++) {
        ...
        if (osal_list_empty(&txtid->retry_q) == OSAL_TRUE) {
            continue;
        }

        // hmac_tid_resume中会调度并发送，可能删除当前tid的一个或者多个帧，即便使用SAFE版本也不能解决
        // 必须在删除之后立刻退出该循环
        osal_list_for_each(dlist_pos, &txtid->retry_q)
        {
            /* 节能队列非空或该用户TID队列还有其他mpdu，需要设置moredata */
            /* 先设置再resume，虽然调度出的帧可能并不是首帧 */
            if (ps_mpdu_num > 1) {
                tx_dscr = (hal_tx_dscr_stru *)osal_list_entry(dlist_pos, hal_tx_dscr_stru, entry);
                mgmt_buf = (oal_netbuf_stru *)tx_dscr->skb_start_addr;
...
osal_u32 hmac_psm_ps_enqueue(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user, oal_netbuf_stru *net_buffer)
{
    ...
    if ((hmac_device->psm_mpdu_num + tx_ctrl->mpdu_num) > MAX_MPDU_NUM_IN_PS_QUEUE_TOTAL) {
        osal_spin_unlock(&hmac_user->ps_structure.lock_ps);
        oam_warning_log3(0, OAM_SF_PWR, "{hmac_psm_enqueue:: drop pkt user[%d] all total %d %d.}",
            hmac_user->assoc_id, hmac_device->psm_mpdu_num, tx_ctrl->mpdu_num);
        return OAL_FAIL;
    }
