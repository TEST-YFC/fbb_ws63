...
osal_void hmac_mgmt_send_disasoc_msg(hmac_vap_stru *hmac_vap, osal_u8 *da, osal_u16 err_code)
{
    ...
    msg_info.data_len = (osal_u16)sizeof(disasoc_deauth_event);
    ...
    hmac_disasoc_misc_stru *disasoc_misc_stru)
{
    ...
    /* 删除对应用户 */
    hmac_user_del_etc(hmac_vap, hmac_user);
    /* 此流程的断连都是驱动主动断连,不是AP触发,上报时添加私有偏移标识是STA主动触发,
       这里disasoc_reason第一个状态码DMAC_DISASOC_MISC_LINKLOSS对应MAC_DISASOC_MISC_LINKLOSS,在偏移量上补1 */
    hmac_sta_handle_disassoc_rsp_etc(hmac_vap,
        disasoc_misc_stru->disasoc_reason + MAC_REPORT_ACTIVE_DISCONN_OFFSET + 1);
    return OAL_SUCC;
}
...
osal_void hmac_send_disasoc_misc_msg(hmac_vap_stru *hmac_vap, osal_u16 user_idx, osal_u16 disasoc_reason)
{
    ...
    msg_info.data_len = (osal_u16)sizeof(disasoc_misc);
...
static osal_s32 hmac_rx_mgmt_ctrl_classify(oal_netbuf_stru *netbuf)
{
    ...
    if (frame_hdr->frame_control.type == WLAN_MANAGEMENT) {
        hmac_mgmt_rx_print(hmac_vap, (mac_rx_ctl_stru *)(&(rx_ctl->rx_info)), (osal_u8 *)frame_hdr);
    }
...
static osal_u32 hmac_rx_multi_mgmt_ctrl_frame(oal_netbuf_stru *netbuf, dmac_rx_ctl_stru *cb_ctrl)
{
    ...
    for (vap_idx = 0; vap_idx < hmac_device->vap_num; vap_idx++) {
        ...
        if (vap_idx == hmac_device->vap_num - 1) {
            ...
            netbuf_copy = oal_netbuf_alloc_ext(WLAN_MGMT_NETBUF_SIZE);
    ...
    oal_netbuf_stru *action_frame)
{
    ...
    /* 管理帧后部保存相关信息 */
    action_info_offset = (osal_u32)(tx_ctl->mpdu_payload_len - (osal_u16)sizeof(hmac_ctx_action_event_stru));
    ...
    hmac_vap_stru *up_vap, mac_channel_stru *channel)
{
    ...
    /* 当前没有处在关联状态的another vap，可以直接切信道 */
    if (up_vap == OSAL_NULL) {
