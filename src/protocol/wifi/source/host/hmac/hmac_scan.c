    ...
    oal_bool_enum_uint8 *report_bss, osal_u8 *go_on)
{
    ...
    *report_bss = OSAL_FALSE;
    ...
    if (hmac_device->scan_params.scan_mode == WLAN_SCAN_MODE_BACKGROUND_OBSS) {
        ...
        hmac_detect_2040_te_a_b(hmac_vap, frame_body, frame_len - MAC_80211_FRAME_LEN, offset,
    ...
    if ((hmac_device->scan_params.scan_mode == WLAN_SCAN_MODE_BACKGROUND_SDP) ||
        (hmac_device->scan_params.scan_mode == WLAN_SCAN_MODE_BACKGROUND_CSA)) {
        /* 不需要继续上报报文内容或扫描结果到host */
        *go_on = OSAL_FALSE;
        return;
    }
    /* 其它模式扫描，上报扫描到的扫描结果 */
    ...
    osal_u32 ssid_len, osal_u8 is_send_cap_ie)
{
    ...
    /* 申请管理帧内存 */
    mgmt_buf = oal_netbuf_alloc_ext(WLAN_SMGMT_NETBUF_SIZE);
...
OAL_STATIC osal_void hmac_scan_back_home_channel(const hal_scan_params_stru *hal_scan_params,
    osal_u8 scan_complete_event, hal_to_dmac_device_stru *hal_device, hmac_vap_stru * const hmac_vap,
    hmac_device_stru *hmac_device)
{
    osal_void *fhook = OSAL_NULL;
    mac_channel_stru work_channel = {0};
    if (hal_scan_params->need_switch_back_home_channel == OSAL_TRUE) {
        ...
        fhook = hmac_get_feature_fhook(HMAC_FHOOK_RADAR_SENSOR_GET_WORK_CHAN);
        if (fhook != OSAL_NULL) {
            work_channel.chan_number = ((hmac_radar_sensor_get_work_ch_num_cb)fhook)();
        }
        if ((work_channel.chan_number == 0) && (hmac_vap->channel.chan_number != 0)) {
            work_channel.chan_number = hmac_vap->channel.chan_number;
        }
        if (work_channel.chan_number != 0) {
            work_channel.band = WLAN_BAND_2G;
            work_channel.en_bandwidth = WLAN_BAND_WIDTH_20M;
            work_channel.chan_idx = work_channel.chan_number - 1;
            hmac_mgmt_switch_channel(hal_device, &work_channel, OSAL_TRUE);
            hmac_scan_switch_channel_notify_alg(hal_device, hmac_vap, &work_channel);
        }
    }
...
OAL_STATIC osal_u32 hmac_scan_send_bcast_probe(const hmac_device_stru *hmac_device, osal_u8 band, osal_u8 index)
{
    ...
    */
    if (hmac_vap->mib_info == OSAL_NULL) {
        oam_error_log3(0, 0, "{hmac_scan_send_bcast_probe:vap mib info is null,vap_id[%d], p_fn_cb[%p],scan_func[%d].}",
            hmac_device->scan_params.vap_id, (uintptr_t)hmac_device->scan_params.fn_cb,
    ...
    /* 单个probereq发送次数,相当于host侧重发次数,目前配置范围[1,3], 0表示未进行配置 */
    send_times = (hmac_vap->single_probe_send_times == 0) ? 1 : hmac_vap->single_probe_send_times;
    ...
    if (ret != OAL_SUCC) {
        oam_warning_log2(0, 0, "vap[%d]hmac_scan_send_bcast_probe:send probe request fail[%d]", hmac_vap->vap_id, ret);
    }
    ...
    hal_scan_params_stru *hal_scan_params, hmac_device_stru *hmac_device)
{
    ...
    if ((hmac_device->scan_params.scan_func & (MAC_SCAN_FUNC_MEAS | MAC_SCAN_FUNC_STATS)) != OSAL_FALSE) {
        ...
        (osal_void)memset_s(&(hal_device->mac_ch_stats), sizeof(hal_ch_mac_statics_stru),
            0, sizeof(hal_ch_mac_statics_stru));
    }
    ...
    hal_to_dmac_device_stru *hal_device, const mac_scan_req_stru *scan_params, hmac_vap_stru *hmac_vap)
{
    ...
    if (do_meas != 0) {
        ...
        /* 手动清空MAC统计信息, (PHY统计信息自动清空) */
        hal_set_mac_clken(OSAL_TRUE);
        hal_set_counter1_clear();
        hal_set_mac_clken(OSAL_FALSE);
        /* 使能信道测量中断 */
...
osal_void hmac_scan_end(hmac_device_stru *hmac_device)
{
    ...
    hmac_device->curr_scan_state = MAC_SCAN_STATE_IDLE;
    hmac_device->scan_params.scan_end_mode = scan_mode;
...
/* 更新指针的值 */
OSAL_STATIC osal_void hmac_scan_update_scanned_bss_item_ie(mac_bss_dscr_stru *bss_buf, mac_bss_dscr_stru *bss_dscr_item)
{
    if (bss_dscr_item->rsn_ie > (osal_u8 *)bss_dscr_item) {
        bss_buf->rsn_ie = (osal_u8 *)bss_buf + (bss_dscr_item->rsn_ie - (osal_u8 *)bss_dscr_item);
    }

    if (bss_dscr_item->wpa_ie > (osal_u8 *)bss_dscr_item) {
        bss_buf->wpa_ie = (osal_u8 *)bss_buf + (bss_dscr_item->wpa_ie - (osal_u8 *)bss_dscr_item);
    }

#ifdef _PRE_WLAN_WIRELESS_EXT
    if (bss_dscr_item->wps_ie > (osal_u8 *)bss_dscr_item) {
        bss_buf->wps_ie = (osal_u8 *)bss_buf + (bss_dscr_item->wps_ie - (osal_u8 *)bss_dscr_item);
    }

    if (bss_dscr_item->wapi_ie > (osal_u8 *)bss_dscr_item) {
        bss_buf->wapi_ie = (osal_u8 *)bss_buf + (bss_dscr_item->wapi_ie - (osal_u8 *)bss_dscr_item);
    }
#endif
}

/*****************************************************************************
 功能描述  : 根据mac地址获取扫描结果中的bss信息, 需要调用者传入内存, 拷贝出去
*****************************************************************************/
...
{
    hmac_bss_mgmt_stru *bss_mgmt = OAL_PTR_NULL; /* 管理扫描的bss结果的结构体 */
    hmac_scanned_bss_info *scanned_bss_info = OAL_PTR_NULL;
    hmac_device_stru *hmac_device = OAL_PTR_NULL;
    osal_u32 size;

    if ((hmac_vap == OAL_PTR_NULL) || (addr == OAL_PTR_NULL) || (bss_buf == OAL_PTR_NULL)) {
        return OAL_FAIL;
    }
    ...
    hmac_device = hmac_res_get_mac_dev_etc(hmac_vap->device_id);
    if (hmac_device == OAL_PTR_NULL) {
        oam_warning_log1(0, 0, "hmac_scan_get_scanned_bss_item_by_bssid:hmac_device null,id[%d]", hmac_vap->device_id);
        return OAL_FAIL;
    }
    ...
    scanned_bss_info = hmac_scan_find_scanned_bss_by_bssid_etc(bss_mgmt, addr);
    if (scanned_bss_info == OAL_PTR_NULL) {
        oam_warning_log4(0, 0, "{hmac_scan_get_scanned_bss_item_by_bssid:find bss failed[%02X:%02X:%02X:%02X:XX:XX]}",
                         /* 0:1:2:3:数组下标 */
    ...
    size = (osal_u32)OAL_SIZEOF(hmac_scanned_bss_info) + (scanned_bss_info->bss_dscr_info.mgmt_len) -
        (osal_u8)OAL_SIZEOF(scanned_bss_info->bss_dscr_info.mgmt_buff);
    if (memcpy_s((osal_u8 *)bss_buf, buf_size, (osal_u8 *)scanned_bss_info, size) != EOK) {
        oam_warning_log0(0, OAM_SF_SCAN, "{hmac_scan_get_scanned_bss_item_by_bssid::memcpy_s fail.}");

        osal_spin_unlock(&(bss_mgmt->lock));
        return OAL_FAIL;
    }

    hmac_scan_update_scanned_bss_item_ie(&(bss_buf->bss_dscr_info), &(scanned_bss_info->bss_dscr_info));
    ...
    osal_u16         frame_len)
{
    ...
    osal_u8    offset;
    osal_u8    rates[WLAN_USER_MAX_SUPP_RATES] = {0};
...
{
    mac_ext_cap_get_multi_bssid_ie_stru *ext_cap = OSAL_NULL;
    ...
    if ((ie != OSAL_NULL) && (ie[1] > EXT_CAPA_GET_MULTI_BSSID_LEN)) { /* 帧体大于2byte，才包含Multiple BSSID能力位 */
        ext_cap = (mac_ext_cap_get_multi_bssid_ie_stru *)(&(ie[MAC_IE_HDR_LEN]));
    ...
    osal_u8 *mgmt_frame, hmac_vap_stru *hmac_vap, osal_u16 frame_body_len)
{
    ...
    *****************************************************************************/
    ...
    bss_dscr->ssid_ie = ssid;
    bss_dscr->ssid_ie_len = ssid_len;
...
{
    if ((new_bss->bss_dscr_info.ac_ssid[0] == '\0') && (old_bss->bss_dscr_info.ac_ssid[0] != '\0')) {
        /*  隐藏SSID,若保存过此AP信息,且ssid不为空,此次通过BEACON帧扫描到此AP信息,且SSID为空,则不进行更新 */
        oam_warning_log4(0, 0, "vap[%d]{hmac_scan_is_hidden_ssid:find hide ssid:%.2x:%.2x:%.2x,ignore update}", vap_id,
            /* 1 2 3 数组下标 */
            new_bss->bss_dscr_info.bssid[1], new_bss->bss_dscr_info.bssid[2], new_bss->bss_dscr_info.bssid[3]);
        return OAL_TRUE;
    }
    ...
    hmac_scanned_bss_info *new_scanned_bss, oal_netbuf_stru *bss_mgmt_netbuf, osal_u8 multi_flag)
{
    ...
    mac_ieee80211_frame_stru *frame_header = OSAL_NULL;
#endif
    osal_u32 curr_time_stamp = (osal_u32)oal_time_get_stamp_ms();
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    ...
    if (hmac_scan_is_hidden_ssid(vap_id, new_scanned_bss, old_scanned_bss) == OAL_TRUE) {
        goto free_new_bss_info;
    }

    if (hmac_scan_check_chan(bss_mgmt_netbuf, new_scanned_bss) == OAL_FALSE) {
        old_scanned_bss->bss_dscr_info.timestamp = curr_time_stamp;
        goto free_new_bss_info;
    }
    ...
    hmac_bss_mgmt_stru *bss_mgmt, osal_u8 *maxbssid_indicator)
{
    ...
    if (bss_mgmt->bss_num > 0 && nontrans_bssid != OSAL_NULL) {
        hmac_scan_add_nontrans_bss_to_list(bss_mgmt, nontrans_bssid);
    }
    ...
    hmac_scanned_nontrans_bssid_info *bss_profile, osal_u8 *skip_addr, osal_u16 *beacon_offset, osal_u16 profile_offset)
{
    ...
    if (subelement_len > 0) {
        if (((*beacon_offset) >= WLAN_MGMT_NETBUF_SIZE) ||
...
osal_s32 hmac_scan_clean_result(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    osal_u32 expire_time = msg->data[0];

    oam_info_log1(0, OAM_SF_SCAN, "{hmac_scan_clean_result::clean driver scan results. expire_time[%d]}", expire_time);
    hmac_scan_clean_expire_scanned_bss(hmac_vap, &(hmac_device->scan_mgmt.scan_record_mgmt), expire_time);

    return OAL_SUCC;
}

/* 回填驱动扫描链表参数到上层wpa supplicant / service api应用 */
static osal_void hmac_scan_copy_bss_para_back(mac_bss_dscr_stru *bss_dscr, wifi_app_common_param *param, osal_u8 index)
{
    oal_ieee80211_mgmt_stru *mgmt = (oal_ieee80211_mgmt_stru *)(bss_dscr->mgmt_buff);

    param->output_para.ssid_ie_len = bss_dscr->ssid_ie_len;
    param->output_para.ssid = bss_dscr->ssid_ie;
    param->output_para.bssid = bss_dscr->bssid;
    param->output_para.caps = mgmt->u.probe_resp.capab_info;
    param->output_para.beacon_int = mgmt->u.probe_resp.beacon_int;
    param->output_para.freq = oal_ieee80211_channel_to_frequency(bss_dscr->st_channel.chan_number,
        bss_dscr->st_channel.band);
    param->output_para.level = ((bss_dscr->c_rssi > 0) ? 0 : (bss_dscr->c_rssi)) * 100; /* 扩大100倍 */
    param->output_para.frame_ie = mgmt->u.probe_resp.variable;
    param->output_para.frame_ie_len = bss_dscr->mgmt_len -
        (osal_u32)OAL_OFFSET_OF(oal_ieee80211_mgmt_stru, u.probe_resp.variable);
    param->output_para.channel = bss_dscr->st_channel.chan_number;
    param->output_para.index = index;
}
...
osal_s32 hmac_scan_handle_app_service(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    hmac_device_stru *hmac_device = hmac_res_get_mac_dev_etc(0);
    osal_s32 ret = -1;
    wifi_app_common_param *param = (wifi_app_common_param *)msg->data;
    osal_u8 index = 0;

    unref_param(hmac_vap);

    if ((param == OSAL_NULL) || (param->input_para.wifi_app_service_cb == OSAL_NULL) || (hmac_device == OSAL_NULL)) {
        oam_warning_log0(0, OAM_SF_SCAN, "{hmac_scan_handle_app_service::para null.}");
        return -1;
    }
    ...
    osal_list_for_each_safe(entry, entry_tmp, &(bss_mgmt->bss_list_head)) {
        ...
        hmac_scan_copy_bss_para_back(bss_dscr, param, index);
        index++;

        /* ret可能取值 0(继续循环), -1(失败), 1(成功) */
        ret = param->input_para.wifi_app_service_cb(param);
        if (ret != 0) {
            goto OUT;
        }
    }
...
OUT:
    ...
    /* 整个链表都未匹配上的时候 ret 的值会是 0; 有的接口可以不去关注返回值 */
    if (ret == 0) {
        return -1;
    }
    return (ret > 0) ? 0 : ret;
