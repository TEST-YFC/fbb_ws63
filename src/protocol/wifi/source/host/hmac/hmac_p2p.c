    ...
    hmac_fsm_input_type_enum_uint8 input_req)
{
    ...
    osal_u8 vap_num, vap_idx, vap_state_priority, input_priority, i;
    ...
    const osal_u8 *ra, const osal_u8 *data)
{
    ...
    (void)memcpy_s(&payload_addr[index], MAC_OUI_LEN, mac_frame_get_p2p_oui(), MAC_OUI_LEN);
    index += MAC_OUI_LEN;
...
osal_u8 hmac_p2p_listen_rx_mgmt(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    frame_body = oal_netbuf_rx_data(netbuf);
    frame_len = (osal_u16)(oal_netbuf_rx_frame_len(netbuf) - oal_netbuf_rx_header_len(netbuf));
...
osal_void  mac_set_p2p0_ssid_ie(const hmac_vap_stru *hmac_vap, osal_u8 *buffer, osal_u8 *ie_len, osal_u16 frm_type)
{
    ...
    *(buffer + 1) = ssid_len;
    ...
    (void)memcpy_s(buffer + MAC_IE_HDR_LEN, ssid_len, HMAC_P2P_WILDCARD_SSID, ssid_len);
...
osal_void mac_set_p2p_noa(hmac_vap_stru *hmac_vap, osal_u8 *buffer, osal_u8 *ie_len)
{
    ...
    /* OUI */
    (void)memcpy_s(&buffer[index], MAC_OUI_LEN, mac_frame_get_p2p_oui(), MAC_OUI_LEN);
    index += MAC_OUI_LEN;
