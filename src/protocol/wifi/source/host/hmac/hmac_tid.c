...
*****************************************************************************/
/*****************************************************************************
 函 数 名  : hmac_free_tx_dscr
 功能描述  :释放发送描述符
*****************************************************************************/
static osal_void hmac_free_tx_dscr(hal_tx_dscr_stru *tx_dscr)
{
    oal_netbuf_stru *netbuf = OSAL_NULL;

    netbuf = (oal_netbuf_stru *)tx_dscr->skb_start_addr;
    tx_dscr->skb_start_addr = OSAL_NULL;
...
osal_u32 hmac_tid_clear(hmac_user_stru *hmac_user, hmac_device_stru *hmac_device)
{
    ...
    hal_tx_dscr_stru *tx_dscr = OSAL_NULL;
    oal_netbuf_stru *netbuf = OSAL_NULL;
    ...
    for (tid_idx = 0; tid_idx < WLAN_TID_MAX_NUM; tid_idx++) {
        ...
        /* 释放重传包 */
        entry = tid_queue->retry_q.next;
        if (entry == OSAL_NULL) {
            continue;
        }
        while (entry != &tid_queue->retry_q) {
            tx_dscr = osal_list_entry(entry, hal_tx_dscr_stru, entry);
            osal_dlist_delete_entry(&tx_dscr->entry);
            /* 重传队列中可能存在AMSDU帧,当做单帧处理 */
            hmac_tid_tx_dequeue_update(hmac_device, tid_queue, 1);
            hmac_free_tx_dscr(tx_dscr);
...
#endif
        OSAL_INIT_LIST_HEAD(&tid_queue->retry_q);
        irq_status = frw_osal_irq_lock();
...
osal_u32 hmac_tid_get_mpdu_by_index(hmac_tid_stru *tid_queue, osal_u16 mpdu_num, oal_netbuf_stru **netbuf_stru)
{
    osal_u32 irq_status;
    osal_u8 mpdu_idx = 0;
    struct osal_list_head *dscr_entry = OSAL_NULL;
    oal_netbuf_stru *netbuf_tmp = OSAL_NULL;
    ...
    /* 先遍历重传队列 */
    if (osal_list_empty(&tid_queue->retry_q) != OSAL_TRUE) {
        dscr_entry = tid_queue->retry_q.next;
        for (mpdu_idx = 0; mpdu_idx < mpdu_index; mpdu_idx++) {
            /* 入参us_mpdu_index=0表示第一个缓存包 */
            dscr_entry = dscr_entry->next;
            if (dscr_entry == &tid_queue->retry_q) {
                dscr_entry = OSAL_NULL;
                break;
            }
        }
    }
    ...
    /* 判断在重传队列是否找到 */
    if ((mpdu_idx == mpdu_index) && (dscr_entry != OSAL_NULL)) {
        hal_tx_dscr_stru *dscr = osal_list_entry(dscr_entry, hal_tx_dscr_stru, entry);
        *netbuf_stru = (oal_netbuf_stru *)dscr->skb_start_addr;
        return OAL_SUCC;
    }
