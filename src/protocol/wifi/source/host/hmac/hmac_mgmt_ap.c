...
/* 保证直接对payload进行偏移运算不会越界访问 */
#define MIN_ADDBA_REQ_PAYLOAD_LEN (9)
#define MIN_ADDBA_RSP_PAYLOAD_LEN (9)
#define MIN_DELBA_PAYLOAD_LEN (6)
    ...
    osal_u8 *payload, osal_s32 payload_len, mac_status_code_enum_uint16 *status_code)
{
    ...
    osal_u32 akm_suites[WLAN_AUTHENTICATION_SUITES] = {0};
...
OAL_STATIC osal_void hmac_ap_rx_send_auth_rsp(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req, osal_u8 *chtxt)
{
    ...
    if (auth_rsp_len == 0) {
        ...
        oam_warning_log1(0, OAM_SF_AUTH, "vap_id[%d] {hmac_ap_rx_send_auth_rsp::auth_rsp_len is 0.}",
            hmac_vap->vap_id);
...
OAL_STATIC oal_bool_enum_uint8 hmac_ap_re_rx_auth_protect_check(hmac_vap_stru *hmac_vap,
    oal_netbuf_stru *auth_req, mac_rx_ctl_stru *rx_ctrl, osal_u16 auth_seq)
{
    hmac_user_stru *hmac_user = OAL_PTR_NULL;
...
#ifdef CONFIG_PLAT_TRNG_TRIG_RPT
OAL_STATIC osal_void hmac_trng_random_bytes(osal_u8 vap_id)
{
...
OAL_STATIC osal_void hmac_ap_rx_auth_req(hmac_vap_stru *hmac_vap, oal_netbuf_stru *auth_req)
{
    ...
    if (osal_unlikely(rx_ctrl == NULL)) {
        oam_error_log0(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_req: rx_ctrl is null, drop auth!}");
        return;
    }
    mac_hdr = oal_netbuf_header(auth_req);
    auth_seq = mac_get_auth_seq_num(mac_hdr, mac_get_rx_cb_payload_len(rx_ctrl));
    if (hmac_is_rx_ucast_frame_for_current_bss(hmac_vap, mac_hdr) != OSAL_TRUE) {
        oam_warning_log0(0, OAM_SF_AUTH, "{hmac_ap_rx_auth_req: da not match}");
        return;
    }
    /* 过滤重复auth帧 */
    if (hmac_ap_re_rx_auth_protect_check(hmac_vap, auth_req, rx_ctrl, auth_seq) == OAL_TRUE) {
        return;
    }

#ifdef CONFIG_PLAT_TRNG_TRIG_RPT
    hmac_trng_random_bytes(hmac_vap->vap_id);
#endif
    ...
    oal_bool_enum_uint8 is_protected)
{
    ...
    if ((ETHER_IS_MULTICAST(da) != OAL_TRUE) &&
        ...
        oam_error_log3(0, OAM_SF_AUTH, "vap_id[%d] {hmac_ap_rx_deauth_req::PMF check failed %d %d.}",
            hmac_vap->vap_id, is_protected, hmac_user->cap_info.pmf_active);
        return OAL_FAIL;
    }
    /* 抛事件上报内核，已经去关联某个STA */
...
OAL_STATIC osal_u16 hmac_check_wpa_cipher_ap(hmac_vap_stru *hmac_vap, osal_u8 *ie)
{
    ...
    if (mac_mib_wpa_akm_match_suites(hmac_vap, crypto.akm_suite, osal_array_size(crypto.akm_suite)) == 0) {
    ...
    osal_u8 *ie)
{
    ...
    if (mac_mib_rsn_akm_match_suites(hmac_vap, crypto.akm_suite, osal_array_size(crypto.akm_suite)) == 0) {
        return MAC_INVALID_AKMP_CIPHER;
    }
    ...
    osal_u8 *payload, osal_u32 msg_len, hmac_user_stru *hmac_user, mac_status_code_enum_uint16 *pen_status_code)
{
    ...
    ***************************************************************************/
    ...
    if (hmac_ap_up_update_sta_cap_info(hmac_vap, us_cap_info, hmac_user, pen_status_code) != OAL_TRUE) {
        oam_warning_log2(0, OAM_SF_ASSOC,
            "vap_id[%d] {hmac_ap_up_update_sta_user::hmac_ap_up_update_sta_cap_info failed, status_code=%d.}",
            hmac_vap->vap_id, *pen_status_code);
        return OAL_FAIL;
    }
...
OAL_STATIC osal_u32  hmac_ap_up_rx_asoc_req(
    hmac_vap_stru *hmac_vap, osal_u8 mgmt_frm_type, osal_u8 *mac_hdr,
    osal_u32 mac_hdr_len, osal_u8 *payload, osal_u32 payload_len)
{
    osal_u32                      rslt;
    hmac_user_stru                 *hmac_user = OAL_PTR_NULL;
    osal_u16                      user_idx = 0;
    ...
    if (hmac_is_rx_ucast_frame_for_current_bss(hmac_vap, mac_hdr) != OSAL_TRUE) {
        oam_warning_log0(0, OAM_SF_AUTH, "{hmac_ap_up_rx_asoc_req: da not match}");
        return OAL_FAIL;
    }
    ...
    if ((status_code != MAC_REJECT_TEMP) && (status_code != MAC_FINITE_CYCLIC_GROUP_NOT_SUPPORTED)) {
        ...
        /* 同步ap带宽，能力等信息到dmac */
        hmac_chan_sync_etc(hmac_vap, hmac_vap->channel.chan_number, mac_vap_get_cap_bw(hmac_vap), OAL_FALSE);
    ...
    oal_netbuf_stru *netbuf, osal_u8 *data, oal_bool_enum_uint8 is_protected)
{
    ...
    switch (data[MAC_ACTION_OFFSET_CATEGORY]) {
        case MAC_ACTION_CATEGORY_BA:
            hmac_ap_up_rx_action_ba(hmac_vap, hmac_user, data, netbuf);
            break;
...
osal_void hmac_sae_add_pmksa_to_list(hmac_vap_stru *hmac_vap, hmac_sae_pmksa_entry_info *pmksa_entry)
{
    ...
    oam_warning_log4(0, OAM_SF_WPA, "{add new pmksa[%x:%x:%x:%x:XX:XX] to list", pmksa_entry->mac_addr[0],
        pmksa_entry->mac_addr[1], pmksa_entry->mac_addr[2], pmksa_entry->mac_addr[3]); // mac addr 1 2 3
}
    ...
    const mac_ieee80211_frame_stru *frame_hdr, hmac_ap_up_stru *ap_up_param, const dmac_rx_ctl_stru *rx_ctl)
{
    ...
    if (ap_up_param->netbuf_len < ap_up_param->payload_len) {
        oam_error_log3(0, OAM_SF_SCAN,
            "vap_id[%d] {hmac_ap_up_proc::payload_len=[%d] over net_buf_size=[%d].}", hmac_vap->vap_id,
            ap_up_param->payload_len, ap_up_param->netbuf_len);
    }

    /* 调用发送管理帧接口 */
    tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(mgmt_buf);
    tx_ctl->tx_user_idx = MAC_INVALID_USER_ID; /* 若此时对端用户不存在，则填一个非法值，发送完成会直接释放 */
    ...
    const osal_u8 *addr, osal_char rssi, osal_u8 *go_on)
{
    ...
    ap_up_param.is_p2p_req =
        (hmac_find_vendor_ie_etc(MAC_WLAN_OUI_WFA, MAC_WLAN_OUI_TYPE_WFA_P2P, probe_req, payload_len) != OSAL_NULL) ?
    ...
    if ((ap_up_param.is_p2p_req == OSAL_TRUE) &&
        ...
        mgmt_buf = oal_netbuf_alloc_ext(WLAN_LARGE_NETBUF_SIZE);
    ...
    ap_up_param.payload_len = payload_len;
