...
osal_s32  hmac_vap_destroy_etc(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    msg_info.data_len = (osal_u16)OAL_SIZEOF(mac_cfg_down_vap_param_stru);
    ...
    msg_info.data_len = (osal_u16)OAL_SIZEOF(hmac_cfg_del_vap_param_stru);
...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT hmac_vap_stru *hmac_get_valid_vap(osal_u32 idx)
{
    ...
    hmac_vap_stru *down_vap, hmac_vap_stru *up_vap)
{
    ...
    oal_netbuf_stru *netbuf_temp = OSAL_NULL;
    ...
    for (q_num = HAL_TX_QUEUE_BUTT - 1; q_num >= 0; q_num--) {
        ...
        for (netbuf = oal_netbuf_peek(head); netbuf != OSAL_NULL; netbuf = netbuf_temp) {
            netbuf_temp = oal_netbuf_peek_next(netbuf, head);
    ...
    const dmac_rx_ctl_stru * const rx_ctrl)
{
    ...
    /* AMPDU最后一个mpdu且不为11b才更新snr信息，否则上报值为0 */
    if (rx_ctrl->rx_status.last_mpdu_flag == OSAL_TRUE &&
        rx_ctrl->rx_statistic.nss_rate.legacy_rate.protocol_mode != WLAN_11B_PHY_PROTOCOL_MODE) {
        /* 转换后snr单位为0.5db */
        hal_phy_rx_get_snr_info(hal_chip_get_hal_device(),
            (uint8_t)rx_ctrl->rx_statistic.nss_rate.legacy_rate.protocol_mode,
            (uint16_t)rx_ctrl->rx_statistic.snr_ant0, (osal_s8 *)&(hmac_vap->query_stats.snr_ant0));
        hal_phy_rx_get_snr_info(hal_chip_get_hal_device(),
            (uint8_t)rx_ctrl->rx_statistic.nss_rate.legacy_rate.protocol_mode,
            (uint16_t)rx_ctrl->rx_statistic.snr_ant1, (osal_s8 *)&(hmac_vap->query_stats.snr_ant1));
    }
}
...
static osal_u32 hmac_config_add_vap_preproc_para_check(hmac_cfg_add_vap_param_stru *param,
    const hmac_device_stru *hmac_device, hmac_vap_stru *hmac_vap)
{
    if (osal_unlikely(hmac_device == OAL_PTR_NULL)) {
...
OSAL_STATIC osal_u32 hmac_config_del_vap_pre(hmac_vap_stru *hmac_vap, osal_bool is_cfg)
{
    ...
    if (hmac_vap->vap_mode == WLAN_VAP_MODE_CONFIG) {
        ...
        /* linux在unregister流程由内核释放netdev,los在此主动释放 */
#if defined(_PRE_OS_VERSION_LITEOS) && defined(_PRE_OS_VERSION) && (_PRE_OS_VERSION_LITEOS == _PRE_OS_VERSION)
        oal_net_free_netdev(net_device);
#endif
...
osal_s32 hmac_cfg_vap_init_event(hmac_device_stru *hmac_device)
{
    ...
    frame_len = (osal_u16)sizeof(mac_data_rate_stru) * MAC_DATARATES_PHY_80211G_NUM;
    ...
    const osal_u8 *param, osal_u8 vap_id)
{
    ...
    fhook = hmac_get_feature_fhook(HMAC_FHOOK_RADAR_SENSOR_DEL_VAP_NOTIFY);
    if (fhook != OSAL_NULL) {
        ((hmac_radar_sensor_del_vap_cb)fhook)();
    }
...
osal_s32 hmac_config_down_vap(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 删除组播用户tid队列中的所有信息 */
    hmac_tid_clear(hmac_user, hmac_device);
