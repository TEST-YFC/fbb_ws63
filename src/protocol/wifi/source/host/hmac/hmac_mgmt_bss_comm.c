    ...
    hmac_user_stru *hmac_user, const mac_action_mgmt_args_stru *action_args)
{
    ...
    mac_get_cb_mpdu_len(tx_ctl) = frame_len + (osal_u16)OAL_SIZEOF(hmac_ctx_action_event_stru);
    ...
    const mac_action_mgmt_args_stru *action_args)
{
    ...
    if (addba_req == OAL_PTR_NULL) {
        oam_warning_log1(0, OAM_SF_BA, "vap_id[%d] {hmac_mgmt_tx_addba_req::addba_req null.}",
            hmac_vap->vap_id);
    ...
    hmac_ba_rx_stru *ba_rx_stru, hmac_vap_stru *hmac_vap)
{
    ...
    ba_rx_stru->ba_lock.lock = OSAL_NULL;
    osal_spin_lock_init(&ba_rx_stru->ba_lock);
    ...
    osal_u8 *payload)
{
    ...
    (osal_void)memset_s(ba_rx_stru, sizeof(hmac_ba_rx_stru), 0, sizeof(hmac_ba_rx_stru));
    /* 初始化接收侧BA会话句柄 */
...
osal_u32 hmac_tx_mgmt_send_event_etc(hmac_vap_stru *hmac_vap, oal_netbuf_stru *mgmt_frame,
    osal_u16 frame_len)
{
    osal_u32 ret, info_ret;
    hmac_mgmt_print_info info = {0};
    ...
    info_ret = hmac_get_mgmt_print_info(mac_header, frame_len, &info, OAL_FALSE);
...
#endif
    if (info_ret == OAL_SUCC) {
        hmac_mgmt_print(&info, OAL_FALSE);
    }
    return OAL_SUCC;
}
    ...
    osal_u8 *da, osal_u16 err_code)
{
    ...
    msg_info.data_len = (osal_u16)OAL_SIZEOF(osal_u32);
    ...
    osal_slong l_freq)
{
    ...
    msg_info.data_len = (osal_u16)OAL_SIZEOF(hmac_rx_mgmt_event_stru);
    ...
    osal_u8 reason)
{
    ...
    msg.data_len = (osal_u16)sizeof(wlan_ctx_action);
...
osal_u8 hmac_get_dsss_ie_channel_num(const hmac_vap_stru *hmac_vap, osal_u8 scan_flag)
{
    ...
    if (is_legacy_ap(hmac_vap)) {
        if (uapi_nv_read(NV_ID_SOFTAP_FIX_DSPS, sizeof(nv_data), &nv_data_len, &nv_data) == OAL_SUCC) {
            if ((nv_data != 0) && (nv_data < CHANNEL14)) {
                chan_num = nv_data;
            }
        }
    }
...
{
    osal_u8 *sa = OSAL_NULL;

    if (frame_len <= MAC_80211_FRAME_LEN || mac_get_frame_type((osal_u8 *)mac_hdr) != WLAN_FC0_TYPE_MGT) {
        return OAL_FAIL;
    }

    info->subtype = mac_get_frame_sub_type(mac_hdr);
    switch (info->subtype) {
        case WLAN_FC0_SUBTYPE_AUTH:
            info->auth_alg = mac_get_auth_alg((osal_u8 *)mac_hdr);
            info->auth_seq = mac_get_auth_seq_num((osal_u8 *)mac_hdr, frame_len - MAC_80211_FRAME_LEN);
            info->status = mac_get_auth_status((osal_u8 *)mac_hdr);
            break;
        case WLAN_FC0_SUBTYPE_ASSOC_RSP:
        case WLAN_FC0_SUBTYPE_REASSOC_RSP:
            info->status = mac_get_asoc_status(mac_hdr + MAC_80211_FRAME_LEN);
            break;
        case WLAN_FC0_SUBTYPE_DEAUTH:
        case WLAN_FC0_SUBTYPE_DISASSOC:
            info->status = *((osal_u16 *)(mac_hdr + MAC_80211_FRAME_LEN));
            break;
    ...
    if (is_rx != OAL_TRUE) {
        mac_get_address1((osal_u8 *)mac_hdr, info->peer);
    } else {
        mac_rx_get_sa((mac_ieee80211_frame_stru *)mac_hdr, &sa);
        (osal_void)memcpy_s(info->peer, WLAN_MAC_ADDR_LEN, sa, WLAN_MAC_ADDR_LEN);
    }
...
{
    const osal_char *dir_str = (is_rx == OAL_TRUE) ? "rx" : "tx";

    switch (info->subtype) {
        case WLAN_FC0_SUBTYPE_AUTH:
            wifi_printf_always("wifi:%s auth seq %u alg %u code %u\r\n", dir_str,
                info->auth_seq, info->auth_alg, info->status);
            break;

        case WLAN_FC0_SUBTYPE_DEAUTH:
        case WLAN_FC0_SUBTYPE_DISASSOC:
            wifi_printf_always("wifi:%s %s, code[%d], peer[%02X:%02X:%02X:%02X:XX:XX].\r\n",
                dir_str, (info->subtype == WLAN_FC0_SUBTYPE_DEAUTH) ? "deauth" : "disassoc", info->status,
                info->peer[0], info->peer[1], info->peer[2], info->peer[3]); /* sa的第0/1/2/3个字节 */
            break;

        case WLAN_FC0_SUBTYPE_ASSOC_REQ:
        case WLAN_FC0_SUBTYPE_REASSOC_REQ:
            wifi_printf_always("wifi:%s %s req\r\n", dir_str,
                (info->subtype == WLAN_FC0_SUBTYPE_ASSOC_REQ) ? "assoc" : "reassoc");
            break;

        case WLAN_FC0_SUBTYPE_ASSOC_RSP:
        case WLAN_FC0_SUBTYPE_REASSOC_RSP:
            wifi_printf_always("wifi:%s %s rsp code %d\r\n", dir_str,
                (info->subtype == WLAN_FC0_SUBTYPE_ASSOC_RSP) ? "assoc" : "reassoc", info->status);
            break;
...
static osal_bool hmac_is_rx_print_frame_for_current_bss(const hmac_vap_stru *hmac_vap, const osal_u8 *mac_hdr)
{
    osal_u8 *sa = OSAL_NULL;
    osal_u8 *da = OSAL_NULL;
    osal_u16 user_idx = 0xffff;

    /* sta模式 需要先检查是否user是否存在，防止打印其他bss的广播管理帧/已断连的AP管理帧 */
    if (hmac_vap->vap_mode == WLAN_VAP_MODE_BSS_STA) {
        mac_rx_get_sa((mac_ieee80211_frame_stru *)mac_hdr, &sa);
        if (hmac_vap_find_user_by_macaddr_etc((hmac_vap_stru *)hmac_vap, (const osal_u8 *)sa,
            &user_idx) != OAL_SUCC) {
            return OSAL_FALSE;
        }
        if (mac_res_get_hmac_user_etc(user_idx) == OAL_PTR_NULL) {
            return OSAL_FALSE;
        }
    ...
    /* 判断是否发给本bss的单播管理帧（AP模式不打印广播管理帧） */
    return hmac_is_rx_ucast_frame_for_current_bss(hmac_vap, mac_hdr);
}

osal_void hmac_mgmt_rx_print(const hmac_vap_stru *hmac_vap, const mac_rx_ctl_stru *rx_info, const osal_u8 *mac_hdr)
{
    hmac_mgmt_print_info info = {0};
    if (hmac_get_mgmt_print_info(mac_hdr, rx_info->frame_len, &info, OAL_TRUE) != OAL_SUCC) {
        return;
    }
    if (hmac_is_rx_print_frame_for_current_bss(hmac_vap, mac_hdr) != OSAL_TRUE) {
        return;
    }
    hmac_mgmt_print(&info, OAL_TRUE);
}
