...
OSAL_STATIC osal_s32 hmac_save_cali_data_to_nv(osal_u8 *data, osal_u16 data_len)
{
    osal_u32 nv_ret = OAL_FAIL;
...
osal_u32 hmac_send_cali_data_etc(hmac_vap_stru *hmac_vap)
{
    ...
    if (hwifi_get_init_priv_value(WLAN_CFG_PRIV_CALI_DATA_MASK, &cali_data_mask) != OAL_SUCC ||
        ((osal_u32)cali_data_mask & WLAN_RF_FE_CALI_DATA_MASK_DOWNLOAD) != WLAN_RF_FE_CALI_DATA_MASK_DOWNLOAD) {
        return OAL_SUCC;
    }
#endif
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
...
osal_s32 hmac_save_cali_event_etc(osal_u8 *data, osal_u16 data_len)
{
    ...
    osal_size_t mem_offset = sizeof(hmac_cali_data_header_stru);
    hmac_cali_data_header_stru *data_msg = (hmac_cali_data_header_stru *)data;
    // 保存校准数据长度以及校准数据接收标志
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
    data_msg->cali_data_received = OAL_TRUE;
    ret = hmac_save_cali_data_to_nv(data, data_msg->cali_data_len + (osal_u16)mem_offset);
#else
    osal_u8 *buffer = OAL_PTR_NULL;
    buffer = hmac_get_cali_data_buffer_addr();
    // 尚未初始化说明还没有接收到上报数据 这是首次接收校准数据 需要申请空间
    if (buffer == OAL_PTR_NULL) {
        ret = (osal_s32)hmac_init_cali_data_buffer_addr(data_msg->cali_data_len + (osal_u16)mem_offset);
        buffer = hmac_get_cali_data_buffer_addr();
    }
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_CFG, "hmac_save_cali_event_etc::init fail, len:%u.", data_msg->cali_data_len);
        return ret;
    }
    data_msg->cali_data_received = OAL_TRUE;
    ret = memcpy_s(buffer, data_msg->cali_data_len + (osal_u16)mem_offset, data,
        data_msg->cali_data_len + (osal_u16)mem_offset);
#endif
    wifi_printf_always("save cali data len:%u ret:%d.\r\n", data_len, ret);
    return ret;
}
#ifdef __cplusplus
#if __cplusplus
