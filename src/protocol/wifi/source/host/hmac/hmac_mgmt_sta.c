...
osal_u32 hmac_sta_wait_auth_seq2_rx_etc(hmac_vap_stru *hmac_vap, osal_void *msg)
{
    ...
    if ((mac_get_frame_type_and_subtype(mac_hdr) == (WLAN_FC0_SUBTYPE_AUTH | WLAN_FC0_TYPE_MGT)) &&
        (auth_seq < WLAN_AUTH_TRASACTION_NUM_ONE || auth_seq > WLAN_AUTH_TRASACTION_NUM_BUTT)) {
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_WPA3
...
osal_u32 hmac_sta_wait_auth_seq4_rx_etc(hmac_vap_stru *hmac_vap, osal_void *p_msg)
{
    ...
    if (mac_get_frame_type_and_subtype(mac_hdr) == (WLAN_FC0_SUBTYPE_AUTH | WLAN_FC0_TYPE_MGT)) {
        ...
        if ((auth_seq == WLAN_AUTH_TRASACTION_NUM_FOUR) && (status == MAC_SUCCESSFUL_STATUSCODE)) {
    ...
    const hmac_user_stru *hmac_user)
{
    ...
    hmac_vap->mgmt_timetout_param.vap_id = hmac_vap->vap_id;
    hmac_vap->mgmt_timetout_param.status_code = MAC_ASOC_RSP_TIMEOUT;
}
...
osal_u32 hmac_sta_auth_timeout_etc(hmac_vap_stru *hmac_vap, osal_void *p_param)
{
    ...
    if (hmac_vap->vap_state == MAC_VAP_STATE_STA_WAIT_AUTH_SEQ2) {
        ...
        auth_rsp.status_code = MAC_AUTH_RSP_TIMEOUT;
    }
...
osal_void hmac_sta_handle_disassoc_rsp_etc(hmac_vap_stru *hmac_vap, osal_u16 disasoc_reason_code)
{
    ...
    if (hmac_vap->disable_clear_ip == OSAL_TRUE) {
        return;
    }
    /* 上报给Lwip */
...
OAL_STATIC osal_void hmac_sta_set_reason_code(osal_u8 mgmt_type, osal_u8 vap_state, osal_u16 *reason_code)
{
    if (reason_code == OAL_PTR_NULL) {
        return;
    }

    if (mgmt_type == WLAN_FC0_SUBTYPE_DISASSOC) {
        *reason_code |= (osal_u16)(1 << 15); /* 15bit表示管理帧类型,0:deauth,1:disassoc */
    }

    if (vap_state == MAC_VAP_STATE_STA_WAIT_ASOC) {
        *reason_code |= (osal_u16)(1 << 14); /* 14bit 认证状态，只在认证状态有效,0:auth,1:assoc */
    }
}
    ...
    oal_bool_enum_uint8 is_protected, hmac_user_stru *hmac_user_vap)
{
    ...
    reason_code = *((osal_u16 *)(mac_hdr + MAC_80211_FRAME_LEN));
    hmac_sta_set_reason_code(mac_get_frame_sub_type(mac_hdr), hmac_vap->vap_state, &reason_code);
    ...
    if (hmac_user_del_etc(hmac_vap, hmac_user_vap) != OAL_SUCC) {
        ...
        /* 上报内核sta已经和某个ap去关联 */
        hmac_sta_handle_disassoc_rsp_etc(hmac_vap, reason_code | MAC_REASON_MAX);
        return OAL_FAIL;
    }
...
OAL_STATIC osal_u32 hmac_sta_rx_deauth_req_etc(const hmac_vap_stru *hmac_vap, osal_u8 *mac_hdr)
{
    ...
    oal_bool_enum_uint8 is_protected)
{
    ...
    if (hmac_user == OAL_PTR_NULL) {
        ...
        /* 上报内核sta已经和某个ap去关联 */
        hmac_sta_handle_disassoc_rsp_etc(hmac_vap, reason_code);
        return OAL_FAIL;
    }
