    ...
    osal_u8 vap_id, osal_u8 *user_mac, osal_u16 user_rd_len)
{
    ...
    (void)memcpy_s(node_rd->user_mac, WLAN_MAC_ADDR_LEN, user_mac, WLAN_MAC_ADDR_LEN);
...
OAL_STATIC osal_u8 dft_is_vip_frame(oal_netbuf_stru *netbuf, osal_u8 direction)
{
    ...
    if (frame_type != WLAN_DATA_BASICTYPE || frame_subtype == WLAN_NULL_FRAME || frame_subtype == WLAN_QOS_NULL_FRAME) {
        return OSAL_FALSE;
    }
...
OAL_STATIC osal_s32 dfx_get_rx_user_rate_info(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    user_id = *(msg->data);
    (void)memcpy_s(msg->rsp, sizeof(mac_cfg_rx_user_rate_stru),
        (osal_u8 *)&g_user_rx_rate_info[user_id], sizeof(mac_cfg_rx_user_rate_stru));
...
WIFI_TCM_TEXT WIFI_HMAC_TCM_TEXT OAL_STATIC osal_u32 dfx_frame_report_hso_rx(oal_netbuf_stru **netbuf,
