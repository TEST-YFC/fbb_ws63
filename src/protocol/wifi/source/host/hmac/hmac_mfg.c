...
#define HMAC_MFG_PRINT_MAX_LEN 64
#define HMAC_MFG_ALLOC_PRINT_LEN     384
...
{
    osal_s32 ret;
    osal_char rsp_print_buff[HMAC_MFG_PRINT_MAX_LEN] = {0};

    unref_param(hmac_vap);
    unref_param(param);
    ret = hal_get_efuse_rssi_offset(rsp_print_buff, sizeof(rsp_print_buff));
    if (ret != OAL_SUCC) {
        return ret;
    }

    wifi_printf_always(rsp_print_buff);
    wifi_printf_always("OK\r\n");
    return OAL_SUCC;
}
...
OAL_STATIC osal_s32 hmac_ccpriv_efuse_read_power_info(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    unref_param(hmac_vap);
    unref_param(param);
    ret = hal_read_efuse_read_power_info(rsp_print_buff, sizeof(rsp_print_buff));
    if (ret != OAL_SUCC) {
        return ret;
    }
...
OAL_STATIC osal_s32 hmac_ccpriv_read_efuse_temp(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    unref_param(hmac_vap);
    unref_param(param);
    ret = hal_efuse_read_temp(rsp_print_buff, sizeof(rsp_print_buff));
    if (ret != OAL_SUCC) {
        return ret;
    }
...
{
    osal_s32 ret;
    osal_char *rsp_print_buff = OSAL_NULL;

    unref_param(hmac_vap);
    unref_param(param);

    rsp_print_buff = (osal_char *)oal_memalloc(HMAC_MFG_ALLOC_PRINT_LEN);
    if (rsp_print_buff == OSAL_NULL) {
        return OAL_FAIL;
    }
    (osal_void)memset_s(rsp_print_buff, HMAC_MFG_ALLOC_PRINT_LEN, 0, HMAC_MFG_ALLOC_PRINT_LEN);

    ret = hal_efuse_status(rsp_print_buff, HMAC_MFG_ALLOC_PRINT_LEN);
    if (ret != OAL_SUCC) {
        goto exit;
    }
...
exit:
    if (rsp_print_buff != OSAL_NULL) {
        oal_free(rsp_print_buff);
        rsp_print_buff = OSAL_NULL;
    }
...
osal_u32 hmac_mfg_init(osal_void)
{
    ...
    hmac_ccpriv_register((const osal_s8 *)"get_temp", hmac_ccpriv_get_temp);
