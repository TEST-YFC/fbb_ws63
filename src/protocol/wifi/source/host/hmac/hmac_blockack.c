...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT OSAL_STATIC osal_u32 hmac_ba_send_frames_with_gap(hmac_ba_rx_stru *ba_rx_hdl,
    oal_netbuf_head_stru *netbuf_header, osal_u16 last_seqnum, hmac_vap_stru *hmac_vap)
{
    osal_u8            num_frms  = 0;
    osal_u16           seq_num;
    hmac_rx_buf_stru    *rx_buf  = OAL_PTR_NULL;
    ...
    while (seq_num != last_seqnum) {
        rx_buf = hmac_remove_frame_from_reorder_q(ba_rx_hdl, seq_num);
...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT OSAL_STATIC osal_u16 hmac_ba_send_frames_in_order(hmac_ba_rx_stru *ba_rx_hdl,
    oal_netbuf_head_stru *netbuf_header, hmac_vap_stru *hmac_vap)
{
    ...
    seq_num = ba_rx_hdl->baw_start;
    while ((rx_buf = hmac_remove_frame_from_reorder_q(ba_rx_hdl, seq_num)) != OSAL_NULL) {
        ba_rx_hdl->mpdu_cnt--;
        seq_num = (osal_u16)hmac_ba_seqno_add(seq_num, 1);
        pst_netbuf = oal_netbuf_peek(&rx_buf->netbuf_head);
...
OAL_STATIC OAL_INLINE osal_void  hmac_ba_reorder_rx_data(hmac_ba_rx_stru *ba_rx_hdl,
    oal_netbuf_head_stru *netbuf_header, hmac_vap_stru *hmac_vap, osal_u16 seq_num)
{
    osal_u8 seqnum_pos;
    ...
    if (seqnum_pos == HMAC_BA_BETWEEN_SEQLO_SEQHI) {
        ba_rx_hdl->baw_start = hmac_ba_send_frames_in_order(ba_rx_hdl, netbuf_header, hmac_vap);
        ...
        hmac_ba_send_frames_with_gap(ba_rx_hdl, netbuf_header, temp_winstart, hmac_vap);
    ...
    mac_rx_ctl_stru *cb_ctrl, oal_netbuf_head_stru *netbuf_header, oal_bool_enum_uint8 *is_ba_buf)
{
    ...
    ret = hmac_ba_filter_serv(hmac_vap, hmac_user, (dmac_rx_ctl_stru*)cb_ctrl, frame_hdr, &ba_rx_hdl);
    if (ret != OAL_CONTINUE) {
        return ret;
    }
    ...
    if ((seq_num == ba_rx_hdl->baw_start) && (ba_rx_hdl->mpdu_cnt == 0) &&
        ...
        *is_ba_buf = OAL_TRUE;
        ...
        hmac_ba_reorder_rx_data(ba_rx_hdl, netbuf_header, hmac_vap, seq_num);
...
/*****************************************************************************
 ...
 输出参数  : 无
*****************************************************************************/
osal_void hmac_reorder_ba_rx_buffer_bar_etc(hmac_ba_rx_stru *rx_ba, osal_u16 start_seq_num,
    hmac_vap_stru *hmac_vap)
{
    ...
    /* 针对BAR的的SSN在窗口内才移窗 */
    if (seqnum_pos == HMAC_BA_BETWEEN_SEQLO_SEQHI) {
        hmac_ba_send_frames_with_gap(rx_ba, &netbuf_head, start_seq_num, hmac_vap);
        rx_ba->baw_start = start_seq_num;
        rx_ba->baw_start = hmac_ba_send_frames_in_order(rx_ba, &netbuf_head, hmac_vap);
...
OAL_STATIC osal_u32  hmac_ba_send_reorder_timeout(hmac_ba_rx_stru *rx_ba, hmac_vap_stru *hmac_vap,
    hmac_ba_alarm_stru *alarm_data, osal_u16 *pus_timeout)
{
    osal_u32                  time_diff;
    osal_u32                  rx_timeout;
    oal_netbuf_head_stru        netbuf_head;
    ...
    osal_spin_unlock(&rx_ba->ba_lock);
...
osal_u32  hmac_ba_timeout_fn_etc(osal_void *p_arg)
{
    ...
    if (direction == MAC_RECIPIENT_DELBA) {
        ...
        if (rx_ba->mpdu_cnt > 0) {
            hmac_ba_send_reorder_timeout(rx_ba, hmac_vap, alarm_data, &timeout);
        }
    ...
    oal_bool_enum_uint8 is_aging)
{
    ...
    osal_spin_lock_destroy(&((*ppst_rx_ba)->ba_lock));
...
osal_void  hmac_up_rx_bar_etc(hmac_vap_stru *hmac_vap, dmac_rx_ctl_stru *rx_ctl, oal_netbuf_stru *netbuf)
{
    ...
    ba_rx_info  = ta_user->tx_tid_queue[tidno].ba_rx_info;

    hmac_reorder_ba_rx_buffer_bar_etc(ba_rx_info, start_seqnum, hmac_vap);
}
...
WIFI_HMAC_TCM_TEXT WIFI_TCM_TEXT osal_u32 hmac_ba_filter_serv(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    const dmac_rx_ctl_stru *cb_ctrl, const mac_ieee80211_frame_stru *frame_hdr, hmac_ba_rx_stru **ba_rx_hdl)
{
    hmac_tid_stru          *tid_queue = OSAL_NULL;
    hmac_ba_rx_stru        *ba_rx_tmp = OSAL_NULL;
    ...
    ba_rx_tmp = tid_queue->ba_rx_info;
    if (ba_rx_tmp == OAL_PTR_NULL) {
        return OAL_SUCC;
    }
    ...
    *ba_rx_hdl = ba_rx_tmp;
    ...
    /* BA会话没有建立 */
    if (ba_rx_tmp->ba_status != HMAC_BA_COMPLETE) {
        if (cb_ctrl->rx_status.ampdu == OSAL_TRUE) {
            oam_warning_log2(0, 0, "vap_id[%d] {hmac_ba_filter_serv::ba not created but recv ampdu,ba status: %d.}",
                hmac_vap->vap_id, ba_rx_tmp->ba_status);

            tid_queue->rx_wrong_ampdu_num++;

            if (tid_queue->rx_wrong_ampdu_num == WLAN_RX_RESET_BA_THREHOLD) {
                tid_queue->rx_wrong_ampdu_num = 0;
