...
#define WLAN_LINKLOSS_MAX_TH    530 /* 默认最大Linkloss门限，超出则认为时间戳获取存在异常 */
#define LINKLOSS_MAX_TH_CONF    500 /* 最大可配置的Linkloss门限 */

/*****************************************************************************
  2 全局变量定义
*****************************************************************************/
/* linkloss各模式老化门限初始值，BT 200，DBAC 200 正常模式 100 */
osal_u16 g_linkloss_threshold[WLAN_LINKLOSS_MODE_BUTT] = {200, 200, 100};
...
OSAL_STATIC osal_void hmac_vap_send_beacon_pkt_msg(osal_u8 vap_id, hal_beacon_tx_params_stru *beacon_tx_params)
{
    ...
    msg_ret = dmac_frw_msg_exec_callback(WLAN_MSG_H2D_C_CFG_SEND_BEACON, vap_id, &msg_info);
#else
...
osal_u32 hmac_beacon_alloc(hmac_vap_stru *hmac_vap)
{
    ...
    if (hmac_vap->beacon_buffer != OSAL_NULL) {
        ...
        /* 申请第一个beacon帧内存 */
        hmac_vap->beacon_buffer = (oal_netbuf_stru *)oal_netbuf_alloc_ext(WLAN_MGMT_NETBUF_SIZE);
        if (hmac_vap->beacon_buffer == OSAL_NULL) {
...
osal_s32 hmac_config_set_linkloss_params(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    /* 参数配置限制，linkloss老化时间必须在50~500之间，对应基础老化时间5s到50s */
    if (linkloss_para->linkloss_threshold < 50 || linkloss_para->linkloss_threshold > LINKLOSS_MAX_TH_CONF) {
    ...
    g_linkloss_threshold[WLAN_LINKLOSS_MODE_BT] = linkloss_para->linkloss_threshold;
    g_linkloss_threshold[WLAN_LINKLOSS_MODE_DBAC] = linkloss_para->linkloss_threshold;
...
/*****************************************************************************
 功能描述  : 更新linkloss计数器值
*****************************************************************************/
OSAL_STATIC osal_void hmac_calc_linkloss_cnt(hmac_vap_stru *hmac_vap, osal_u16 old_th_base, osal_u16 old_linkloss_cnt)
{
...
{
    osal_u8 mode;
    osal_u16 old_th_base = get_current_linkloss_threshold(hmac_vap);
    ...
    for (mode = WLAN_LINKLOSS_MODE_BT; mode < WLAN_LINKLOSS_MODE_BUTT; mode++) {
        ...
        if ((mac_mib_get_powermanagementmode(hmac_vap)) == WLAN_MIB_PWR_MGMT_MODE_PWRSAVE) {
            ...
            hmac_vap->linkloss_info.linkloss_info[mode].linkloss_threshold = ((PERIOD_100_MS *
                g_linkloss_threshold[mode] / hmac_vap->linkloss_info.dot11_beacon_period) < LINKLOSS_MAX_TH_CONF) ?
                (osal_u16)(PERIOD_100_MS * g_linkloss_threshold[mode] / hmac_vap->linkloss_info.dot11_beacon_period) :
                LINKLOSS_MAX_TH_CONF;
        }
        ...
        oam_warning_log2(0, OAM_SF_RX,
            "{hmac_linkloss_update_threshold::linkloss_mode[%d], int_threshold[%d]!}",
            mode, hmac_vap->linkloss_info.linkloss_info[mode].linkloss_threshold);
    }
    /* 阈值改变，更新linkloss计数器值 */
...
OSAL_STATIC osal_void hmac_vap_linkloss_cnt_update(hmac_vap_stru *hmac_vap,
    const hmac_device_stru *hmac_device, osal_u64 current_time)
{
...
OSAL_STATIC osal_void hmac_vap_linkloss_incr(hmac_vap_stru *hmac_vap, const hmac_device_stru *hmac_device)
{
    ...
    osal_void *csa_cb = hmac_get_feature_fhook(HMAC_FHOOK_CSA_STA_FSM_IS_WAITING);
    osal_void *pm_stat_cb = hmac_get_feature_fhook(HMAC_FHOOK_PM_STA_STAT);
    osal_void *linkloss_igr_cb = hmac_get_feature_fhook(HMAC_FHOOK_PM_LINKLOSS_IGNOR);

    /* 当前host定时器触发时间精度有限，需要用系统时间要校准linkloss cnt */
    current_time = osal_get_time_stamp_ts();
    /* 扫描、雷达、2040切信道时候，linkloss计数器不计数 */
    if ((csa_cb == OSAL_NULL || ((hmac_sta_csa_fsm_is_waiting_cb)csa_cb)(hmac_vap) == OSAL_FALSE) &&
        (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING) &&
        (pm_stat_cb == OSAL_NULL || ((hmac_sta_pm_get_wlan_dev_state_cb)pm_stat_cb)() == HMAC_DISALLOW_SLEEP)) {
        if (linkloss_igr_cb != OSAL_NULL && ((hmac_sta_pm_ignor_linkloss_cb)linkloss_igr_cb)() != OAL_CONTINUE) {
            oam_warning_log0(0, OAM_SF_ANY, "{hmac_vap_linkloss_incr:: skip linkloss for pm!}");
            hmac_vap_linkloss_cnt_update(hmac_vap, hmac_device, current_time);
            return;
        }
...
/*****************************************************************************
 函 数 名  : hmac_report_linkloss_info
 功能描述  : 上报linkloss统计信息
*****************************************************************************/
OSAL_STATIC osal_void  hmac_report_linkloss_info(const hmac_vap_stru *hmac_sta, const hmac_device_stru *hmac_device)
{
    osal_void *fhook = OSAL_NULL;
    osal_u64 last_active_time;
...
OSAL_STATIC osal_void hmac_sta_tbtt_prior_linkloss_handler(hmac_vap_stru *hmac_vap)
{
    ...
    if ((get_current_linkloss_cnt(hmac_vap) % WLAN_LINKLOSS_REPORT == 0) &&
        ...
        (hmac_device->curr_scan_state != MAC_SCAN_STATE_RUNNING)) {
        /* 打印link_loss计数 */
        hmac_report_linkloss_info(hmac_vap, hmac_device);
    }
}
...
OSAL_STATIC osal_void hmac_sta_up_misc(hmac_vap_stru *hmac_vap)
{
    ...
    if (hmac_is_linkloss(hmac_vap) == OSAL_TRUE) {
        ...
        wifi_printf("Oh no,LinkLoss!! BSSID[%02X:%02X:%02X:%02X:XX:XX]\r\n", hmac_vap->bssid[0],
            ...
            hmac_vap->bssid[1], hmac_vap->bssid[2], hmac_vap->bssid[3]);
        /* 打印link_loss计数 */
        hmac_report_linkloss_info(hmac_vap, hmac_res_get_mac_dev_etc(0));
        hmac_vap_linkloss_clean(hmac_vap);
#ifdef _PRE_WIFI_DEBUG
        hal_show_linkloss_reg_info();
#endif
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
OSAL_STATIC osal_u8 hmac_get_nv_disable_linkloss(void)
{
    osal_u8 nv_data;
    osal_u16 nv_data_len = 0;

    // read nv
    if (uapi_nv_read(NV_ID_LINKLOSS_DISABLE, sizeof(nv_data), &nv_data_len, &nv_data) == OAL_SUCC) {
        return nv_data;
    }
...
#if defined(CONFIG_MIDDLEWARE_SUPPORT_NV)
        && (hmac_get_nv_disable_linkloss() == 0)
#endif
    ) {
...
OSAL_STATIC osal_void hmac_tbtt_event_ap_bottom_half(hmac_vap_stru *hmac_vap)
{
    ...
    if (led_tbtt_timer < offset) {
        ...
        // dtim = 0, dtim force clear
        ps_mpdu_num = (osal_u32)osal_adapt_atomic_read(&hmac_multi_user->ps_structure.mpdu_num);
        if (ps_mpdu_num != 0) {
    ...
    oal_netbuf_stru *netbuf, osal_u16 msg_len, osal_u8 stream_change)
{
    ...
    hmac_user->user_num_spatial_stream = hmac_m2s_get_bss_max_nss(hmac_vap, netbuf, msg_len + MAC_BEACON_OFFSET,
        OSAL_TRUE);
    ...
    wlan_prot_mode_enum_uint8 prot_mode)
{
    ...
    if (hmac_vap == OSAL_NULL) {
        oam_error_log0(0, OAM_SF_CFG, "{hmac_set_protection_mode::hmac_vap null.}");
...
osal_void hmac_ap_up_rx_obss_beacon(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf)
{
    ...
    payload = oal_netbuf_rx_data(netbuf);
    /* 获取帧体长度 */
    payload_len = (osal_u16)(oal_netbuf_rx_frame_len(netbuf) - oal_netbuf_rx_header_len(netbuf));
