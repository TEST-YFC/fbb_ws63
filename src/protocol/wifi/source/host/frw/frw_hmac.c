...
OAL_STATIC WIFI_HMAC_TCM_TEXT osal_u16 host_frw_msg_convert(osal_u16 msg_id)
{
...
OAL_STATIC osal_s32 frw_check_rcv_data_valid(oal_netbuf_stru *netbuf, osal_u16 frame_len)
{
    td_u8 *end = skb_end_pointer(netbuf);
    td_u8 *tail = skb_tail_pointer(netbuf);
    if ((end < tail) || ((end - tail) < frame_len)) {
        osal_printk("err netbuf %p:len:%d ptr[head:%lx,data:%lx,tail:%p,end:%p]\n",
            netbuf, frame_len, (unsigned long)(uintptr_t)netbuf->head, (unsigned long)(uintptr_t)netbuf->data,
            tail, end);
        return OAL_FAIL;
    }
...
OAL_STATIC WIFI_HMAC_TCM_TEXT osal_u32 frw_hmac_rcv_mgnt_data(oal_netbuf_stru *netbuf)
{
    ...
    if (frw_check_rcv_data_valid(netbuf, rx_ctrl->frame_len) != OAL_SUCC) {
        goto err;
    }
...
osal_s32 frw_send_cfg_to_device(osal_u16 msg_id, osal_u8 vap_id, osal_bool sync, osal_u16 time_out, frw_msg *msg)
{
    ...
    if (sync && osal_get_current_tid() == frw_get_wifi_frw_task_id()) {
        return dmac_frw_msg_exec_callback(msg_id, vap_id, msg);
    }
#endif
