...
WIFI_HMAC_TCM_TEXT OAL_STATIC osal_u32 frw_netbuf_len_align(oal_netbuf_stru *netbuf, osal_u32 align_len)
{
    ...
    /* align the netbuf */
    len_algin = oal_round_up((osal_u16)len, (osal_u16)align_len);
...
OAL_STATIC osal_void hcc_service_init_hdr(hcc_transfer_param *param, osal_u8 main_type, osal_u8 sub_type,
...
OAL_STATIC osal_u32 frw_tx_wifi_cfg_by_hcc(osal_u8 *buf, osal_u16 len, osal_u8 data_type)
{
    hcc_transfer_param hcc_tx_param;
    osal_u16 fc_flag;
    osal_u8 queue_id;
    osal_u16 len_align = (osal_u16)oal_round_up(len, HCC_DMA_DATA_ALIGN);

#ifdef _PRE_WLAN_FEATURE_POWERSAVE
    osal_void *fhook_wkup_dev = hmac_get_feature_fhook(HMAC_FHOOK_PM_STA_WAKEUP_DEV);
    if ((fhook_wkup_dev != OSAL_NULL) &&
        (((hmac_sta_pm_wakeup_dev_cb)fhook_wkup_dev)(OSAL_NULL, len, data_type) != OAL_SUCC)) {
...
OSAL_STATIC osal_s32 frw_send_msg_adapt_by_hcc(osal_u16 msg_id, frw_msg *cfg, osal_u8 vap_id, osal_u32 msg_seq,
    osal_bool sync)
{
    osal_s32 ret;
    osal_u8 *cfg_buf = OSAL_NULL;
    frw_msg_adapt_comm msg_adapt = {0};
    osal_u16 payload_len = (osal_u16)(sizeof(frw_msg_adapt_comm) + cfg->data_len);
    osal_u16 extend_len = (osal_u16)(frw_get_head_len() + hcc_get_head_len());
...
WIFI_HMAC_TCM_TEXT osal_u32 frw_tx_wifi_netbuf_by_hcc(oal_netbuf_stru *netbuf, osal_u16 payload_len, osal_u8 data_type)
{
    ...
    osal_void *fhook_wkup_dev = hmac_get_feature_fhook(HMAC_FHOOK_PM_STA_WAKEUP_DEV);
    if ((fhook_wkup_dev != OSAL_NULL) &&
        (((hmac_sta_pm_wakeup_dev_cb)fhook_wkup_dev)(OSAL_NULL, payload_len, data_type) != OAL_SUCC)) {
        oam_warning_log0(0, OAM_SF_FRW, "frw_tx_wifi_netbuf_by_hcc: sta_pm");
        return OAL_FAIL;
    }
#endif

    if (hmac_is_thruput_enable(THRUPUT_TX_BEFORE_HCC) != 0) {
    ...
    return (osal_u32)hcc_tx_data(HCC_CHANNEL_AP, oal_netbuf_payload(netbuf), (osal_u16)OAL_NETBUF_LEN(netbuf),
        &hcc_tx_param);
}
    ...
    oal_netbuf_stru **netbuf)
{
    ...
    if (frw_alloc_pbuf(payload_len, &hmacbuf) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_FRW, "frw_dmac_netbuf_conver_hmac_netbuf fail\r\n");
        return OAL_FAIL;
    }
...
#endif
    if (ret != OAL_SUCC) {
        oal_mem_netbuf_free((oal_dmac_netbuf_stru *)netbuf);
        oam_warning_log1(0, OAM_SF_FRW, "[E]frw_dmac_netbuf_conver_hmac_netbuf drop!, ret = 0x%x\n", ret);
        return OAL_SUCC;
    }

    /* 由被调用处释放内存 */
    (osal_void)frw_hmac_rcv_netbuf(hmac_netbuf);
    return OAL_SUCC;
}
#endif
