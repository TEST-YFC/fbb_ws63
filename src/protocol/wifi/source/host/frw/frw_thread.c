...
osal_s32 frw_msg_check_frw_deadlock(osal_s32 msg_id)
{
    ...
    if (th->pid == osal_adapt_get_current_tid()) {
        wifi_printf_always("\n frw call msg err! msg[%d] frw_pid[%d]\n", msg_id, th->pid);
...
osal_s32 frw_msg_check_hcc_deadlock(osal_s32 msg_id)
{
    ...
    if (tc->hcc_pid == osal_adapt_get_current_tid()) {
        wifi_printf("\n frw call msg err! msg[%d] hcc_pid[%d]\n", msg_id, tc->hcc_pid);
        return OAL_ERR_CODE_THREAD_DEADLOCK;
    }
#else
    unref_param(msg_id);
    ...
    frw_thread_que_type type)
{
    ...
    osal_spin_lock_irqsave(&que->lock, &flags);
    if (th->status == FRW_THREAD_EXIT) {
        osal_spin_unlock_irqrestore(&que->lock, &flags);
...
OSAL_STATIC osal_void frw_free_fail_sync_node(const frw_thread *th, frw_thread_que *que, osal_u16 msg_id,
    osal_u32 msg_seq)
{
    osal_bool is_empty;
    frw_msg_node *node = OSAL_NULL;
...
OSAL_STATIC osal_s32 frw_queue_clear(frw_thread *th)
{
    ...
    for (j = 0; j < sizeof(th->thread_que) / sizeof(th->thread_que[0]); j++) {
        ...
        if (j == FRW_THREAD_QUE_SEND_SYNC_MSG || j == FRW_THREAD_QUE_POST_SYNC_MSG) {
            osal_bool is_empty;
            /* 同步消息队列为空直接释放锁 */
            osal_spin_lock_irqsave(&cur_que->lock, &flags);
            is_empty = osal_list_empty(que_list);
            osal_spin_unlock_irqrestore(&cur_que->lock, &flags);
...
OSAL_STATIC osal_void frw_thread_resource_exit(frw_thread *th)
{
    frw_queue_clear(th);
...
WIFI_HMAC_TCM_TEXT static inline osal_bool frw_thread_should_wake(const frw_thread *th)
{
