...
/*****************************************************************************
    全局变量定义
*****************************************************************************/
OAL_STATIC struct osal_list_head g_timer_list_head[WLAN_FRW_MAX_NUM_CORES];
OAL_STATIC osal_spinlock g_timer_list_lock[WLAN_FRW_MAX_NUM_CORES];
OAL_STATIC osal_timer g_os_timer[WLAN_FRW_MAX_NUM_CORES];
OAL_STATIC osal_u32 g_next_start_time_stamp[WLAN_FRW_MAX_NUM_CORES] = {0}; /* 记录下一次软中断定时器启动时间 */
OAL_STATIC osal_u32 g_os_timer_tick_cnt[WLAN_FRW_MAX_NUM_CORES] = {0};     /* 记录硬timer到期次数，统计timer消息频率 */
OAL_STATIC osal_u32 g_timer_stoped = OSAL_FALSE;
#ifdef _PRE_WLAN_FEATURE_POWERSAVE
static osal_u32 g_timer_restart = OSAL_FALSE;
#endif
/*****************************************************************************
    函数实现
*****************************************************************************/
...
osal_void frw_timer_restart_all_timer(osal_void)
{
    ...
    for (core_id = 0; core_id < WLAN_FRW_MAX_NUM_CORES; core_id++) {
        if (osal_list_empty(&g_timer_list_head[core_id]) == OAL_FALSE) {
            osal_adapt_timer_mod(&g_os_timer[core_id], FRW_TIMER_DEFAULT_TIME);
            g_next_start_time_stamp[core_id] = present_time + FRW_TIMER_DEFAULT_TIME;
        }
    }
    g_timer_restart = OSAL_FALSE;
}
...
osal_void frw_timer_stop_all_timer(osal_void)
{
    ...
    for (core_id = 0; core_id < WLAN_FRW_MAX_NUM_CORES; core_id++) {
        osal_timer_stop(&g_os_timer[core_id]);
        g_next_start_time_stamp[core_id] = 0;
    }
    g_timer_restart = OSAL_TRUE;
}
#endif
/*****************************************************************************
 功能描述 : 初始化定时器
...
osal_void frw_timer_exit_clean(osal_u32 core_id)
{
    ...
    while (timeout_entry != &g_timer_list_head[core_id]) {
        ...
        /* 清除timer状态 */
        timeout_element->is_enabled = OSAL_FALSE;
        timeout_element->is_registerd = OSAL_FALSE;
...
OAL_STATIC osal_void frw_timer_timeout_proc_run(osal_u32 *present_time, osal_u32 core_id)
{
    ...
    uintptr_t timeout_element_func;
    ...
    while (timeout_entry != &g_timer_list_head[core_id]) {
        ...
        if (frw_time_after(*present_time, timeout_element->time_stamp) != 0) {
            ...
            timeout_element_func = (uintptr_t)timeout_element->func;
            ...
            if ((osal_u32)oal_jiffies_to_msecs(2) <= runtime_func_end) {  /* 2转换时间 */
                oam_warning_log2(0, OAM_SF_FRW, "{frw_timer_timeout_proc_run::timer 0x%x, runtime=%u}",
                    timeout_element_func, runtime_func_end);
            }
        } else {
            break;
        }
