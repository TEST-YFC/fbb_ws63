...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
OSAL_STATIC osal_void alg_autorate_customize_proc(alg_autorate_dev_stru *autorate_dev)
{
    osal_s32 priv_value = 0;
#ifdef CONFIG_MIDDLEWARE_SUPPORT_NV
    osal_u8 nv_priv_val = 0;
    osal_u16 nv_priv_val_len = 0;
#endif
    if (hwifi_get_init_priv_value(WLAN_CFG_PRIV_CROSS_PROT_ENABLE, &priv_value) == OAL_SUCC) {
        if (priv_value == OSAL_TRUE || priv_value == OSAL_FALSE) {
            autorate_dev->enable_cross_protocol = (osal_u8)priv_value;
        }
    }
#ifdef CONFIG_MIDDLEWARE_SUPPORT_NV
    if (uapi_nv_read(NV_ID_ALG_CROSS_PROT_ENABLE, sizeof(nv_priv_val), &nv_priv_val_len, &nv_priv_val) == OAL_SUCC) {
        if (nv_priv_val == OSAL_TRUE || nv_priv_val == OSAL_FALSE) {
            autorate_dev->enable_cross_protocol = nv_priv_val;
        }
    }
#endif
}
...
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    alg_autorate_customize_proc(autorate_dev);
#endif
    return OAL_SUCC;
}
