...
{
    osal_u8 cur_index;
    alg_intf_det_intf_duty_cyc_info_stru *cyc_info;

    if (osal_unlikely(cur_state >= ALG_INTF_DET_STATE_BUTT)) {
        return;
    }

    cyc_info = (alg_intf_det_intf_duty_cyc_info_stru *)&(intf_det->last_10_times_intf_duty_cyc[cur_state]);
    cur_index = cyc_info->cur_index;

    if (cyc_info->total_cnt < ALG_INTF_DET_MAX_INTF_DUTY_CNT) {
        cyc_info->sum_of_intf_duty_cyc_20 += intf_det->adjch_intf_stat.duty_cyc_ratio_20;
        cyc_info->sum_of_intf_duty_cyc_40 += intf_det->adjch_intf_stat.duty_cyc_ratio_40;
        (cyc_info->total_cnt)++;
    } else {
        cyc_info->sum_of_intf_duty_cyc_20 = cyc_info->sum_of_intf_duty_cyc_20 -
            cyc_info->intf_duty_cyc_20_recoard[cur_index] +
            intf_det->adjch_intf_stat.duty_cyc_ratio_20;
        cyc_info->sum_of_intf_duty_cyc_40 = cyc_info->sum_of_intf_duty_cyc_40 -
            cyc_info->intf_duty_cyc_40_recoard[cur_index] +
    ...
    cyc_info->intf_duty_cyc_40_recoard[cur_index] =
        intf_det->adjch_intf_stat.duty_cyc_ratio_40;
    (cyc_info->cur_index)++;
    cyc_info->cur_index %= ALG_INTF_DET_STATISTIC_CNT_TH;
    handler->duty_cyc_info[cur_state].duty_cyc_ratio_20 =
        (osal_u16)cyc_info->sum_of_intf_duty_cyc_20 /
            cyc_info->total_cnt;
    handler->duty_cyc_info[cur_state].duty_cyc_ratio_40 =
        (osal_u16)cyc_info->sum_of_intf_duty_cyc_40 /
            cyc_info->total_cnt;
#if ALG_INTF_DET_DEBUG_MODE
    if (intf_det->intf_det_debug == OSAL_TRUE) {
...
/******************************************************************************
 功能描述  : 获取同频干扰类型
******************************************************************************/
WIFI_TCM_TEXT WIFI_HMAC_TCM_TEXT oal_bool_enum_uint8 alg_intf_det_get_curr_coch_intf_type(
    ...
    const wlan_scan_chan_stats_stru *chan_result)
{
    ...
    /* 1000000:倍数 */
    abort_time_us_end = btcoex_statistics->abort_duration_ * 1000000 + btcoex_statistics->abort_duration_us;
    ...
    if (intf_det->intf_det_debug == ALG_INTF_DET_SERIAL_DEBUG) {
        ...
        wifi_printf("alg_intf_det_stat_get_duty_cyc_ratio: abort_duration_us = %d, abort_duration = %d, \
            abort_time = %d\n", btcoex_statistics->abort_duration_us, btcoex_statistics->abort_duration_,
            intf_det->adjch_intf_stat.abort_time_us);
#endif
#endif
    }
...
osal_void alg_intf_det_adjch_stat_handler(hal_to_dmac_device_stru *hal_device, alg_intf_det_stru *intf_det)
{
    ...
    intf_det->adjch_intf_stat.abort_time_us =
        /* 1000000:倍数 */
        btcoex_statistics->abort_duration_ * 1000000 + btcoex_statistics->abort_duration_us;
#endif
}
...
/******************************************************************************
 功能描述  : 获取邻叠频干扰类型
******************************************************************************/
WIFI_TCM_TEXT WIFI_HMAC_TCM_TEXT hal_alg_intf_det_mode_enum_uint8 alg_intf_det_get_curr_adjch_intf_type(
    hal_to_dmac_device_stru *hal_device)
{
    osal_u32 ret;
    alg_intf_det_stru *intf_det = OSAL_NULL;
