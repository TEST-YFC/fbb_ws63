...
osal_s32 alg_cca_opt_init(osal_void)
{
    ...
    hmac_alg_register_cca_opt_radar_notify_func(alg_cca_opt_radar_sw);
#endif
...
#endif
#ifdef _PRE_RADAR_CCA_SW_OPT
    hmac_alg_unregister_cca_opt_radar_notify_func();
#endif
...
osal_void alg_cca_opt_radar_sw(osal_bool radar_switch, osal_bool is_mwo_mode)
{
    alg_intf_det_stru *intf_det = OSAL_NULL;
    alg_cca_opt_stru *cca_opt = alg_cca_get_cca_opt();
    hal_to_dmac_device_stru *dev = hal_chip_get_hal_device();
    osal_s8 radar_cca_prot_val = 0;
    osal_s8 radar_cca_ed_val = 0;

    if (hmac_alg_get_device_priv_stru(dev, HAL_ALG_DEVICE_STRU_ID_INTF_DET, (osal_void **)&intf_det) != OAL_SUCC) {
        return;
    }

    radar_cca_prot_val = (intf_det->adjch_intf_info.intf_type == HAL_ALG_INTF_DET_ADJINTF_CERTIFY) ?
        RADAR_ENABLE_CCA_ED_RSSI_CERTIFY : RADAR_ENABLE_CCA_ED_RSSI;
    radar_cca_ed_val = (is_mwo_mode == OSAL_TRUE) ? RADAR_ENABLE_CCA_ED_RSSI_MICROWAVE : radar_cca_prot_val;

    /* 雷达开启需要将CCA能量门限和协议门限都设置到不小于-60，避免影响雷达帧间隔 */
    if (radar_switch == OSAL_TRUE) {
        cca_opt->cca_opt_en = WLAN_CCA_OPT_DISABLE;
        if (cca_opt->ed_high_20th_reg < radar_cca_ed_val || is_mwo_mode == OSAL_TRUE) {
            hal_set_ed_high_th(radar_cca_ed_val, radar_cca_ed_val, OSAL_TRUE);
        }
        hal_set_cca_prot_th(radar_cca_prot_val, radar_cca_prot_val);
    } else {
        cca_opt->cca_opt_en = WLAN_CCA_OPT_ENABLE;
        hal_set_ed_high_th(cca_opt->ed_high_20th_reg, cca_opt->ed_high_40th_reg, OSAL_TRUE);
        hal_set_cca_prot_th(cca_opt->sd_cca_20th_dsss, cca_opt->sd_cca_20th_ofdm);
