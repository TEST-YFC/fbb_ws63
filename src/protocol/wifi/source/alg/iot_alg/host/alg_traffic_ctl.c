...
OSAL_STATIC osal_void alg_traffic_ctl_display_rsp_para(osal_u16 alg_cfg, frw_msg *msg);
...
OSAL_STATIC osal_void alg_traffic_ctl_info_dump(osal_u8 *msg_rsp)
{
...
osal_void alg_traffic_ctl_display_rsp_para(osal_u16 alg_cfg, frw_msg *msg)
{
#ifdef _PRE_WIFI_PRINTK
    if (alg_cfg == MAC_ALG_CFG_TRAFFIC_CTL_INFO_DUMP) {
        alg_traffic_ctl_info_dump(msg->rsp);
...
osal_u32 alg_traffic_ctl_hmac_config_param(hmac_vap_stru *hmac_vap, frw_msg *msg)
{
    ...
    alg_traffic_ctl_display_rsp_para(alg_param->alg_cfg, msg);
    return (osal_u32)ret;
}
