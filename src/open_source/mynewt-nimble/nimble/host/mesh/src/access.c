...
static int pub_delay_schedule(struct bt_mesh_model_pub *pub, int delay)
{
	...
	k_work_reschedule(&pub->timer, K_MSEC(random));
 
	BT_DBG("Publication delayed by %dms", random);
	return 0;
}

static void mod_publish(struct ble_npl_event *work)
{
	struct k_work_delayable *dwork = (struct k_work_delayable *)k_work_delayable_from_work(work);
	...
	BT_DBG("");
	if (pub->count) {
		if (pub->update && bt_mesh_model_pub_is_retransmission(pub->mod)) {
			err = pub->update(pub->mod);
			if (err) {
		...
		if (!pub_delay_schedule(pub, RANDOM_DELAY_SHORT)) {
	...
	if (err) {
		BT_DBG("Failed to publish (err %d)", err);
		if (pub->count == BT_MESH_PUB_TRANSMIT_COUNT(pub->retransmit)) {
			pub->period_start = k_uptime_get_32();
		}
            ...
            bool vnd, bool primary, void *user_data)
{
    ...
    int *err = user_data;
    if (*err) {
		BT_ERR("user_data value is not valid");
    }
    ...
    if (mod->pub) {
        ...
        k_work_cancel_delayable_delete(&mod->pub->timer);
    }
...
int bt_mesh_comp_unregister(void)
{
    int err = 0;
    /* There must be at least one element */
    if (!dev_comp || !dev_comp->elem_count) {
        return -EINVAL;
    }
...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
	...
	BT_INFO("src 0x%04x OpCode 0x%08x seq:0x%06x rx ttl:%d", rx->ctx.addr, opcode, rx->seq, rx->ctx.recv_ttl);
...
int bt_mesh_model_publish_duration(struct bt_mesh_model *model, uint32_t delay_time, uint8_t trans_time)
{
	if (!model) {
		return -ENOTSUP;
	}
	...
	pub->retransmit = trans_time;
	/* Account for initial transmission */
	pub->count = BT_MESH_PUB_TRANSMIT_COUNT(pub->retransmit) + 1;
	BT_INFO("Publish Retransmit Count %u Interval %ums, pub->retransmit: %d", pub->count,
	       BT_MESH_PUB_TRANSMIT_INT(pub->retransmit), pub->retransmit);
	k_work_reschedule(&pub->timer, delay_time);
	return 0;
}
...
struct mod_store_val {
    ...
    uint16_t keys[CONFIG_BT_MESH_MODEL_KEY_COUNT];
    uint16_t groups[CONFIG_BT_MESH_MODEL_GROUP_COUNT];
    struct mod_pub_val pub_val;
} __packed;

#define CONFIG_BT_MESH_MAX_MOD_STORE_NUM              MYNEWT_VAL(BLE_MESH_MAX_MOD_STORE_NUM)
static struct mod_store_val mod_nv[CONFIG_BT_MESH_MAX_MOD_STORE_NUM] = {0};

static int mod_set_bind(struct bt_mesh_model *mod, struct mod_store_val *mod_val)
{
    unsigned int i;
    ...
    memcpy(&mod->keys, &mod_val->keys, sizeof(mod_val->keys));

    BT_INFO("Decoded %u bound keys for model:0x%x", mod_val->key_conut, mod->id);
    return 0;
}

static int mod_set_sub(struct bt_mesh_model *mod, struct mod_store_val *mod_val)
{
    int len, err;

    /* Start with empty array regardless of cleared or set value */
    memcpy(&mod->groups, &mod_val->groups, sizeof(mod_val->groups));
    BT_INFO("Decoded %u subscribed group addresses for model", mod_val->group_conut);
    return 0;
}

static int mod_set_pub(struct bt_mesh_model *mod, struct mod_store_val *mod_val)
{
    mod->pub->addr = mod_val->pub_val.addr;
    mod->pub->key = mod_val->pub_val.key;
    mod->pub->cred = mod_val->pub_val.cred;
    mod->pub->ttl = mod_val->pub_val.ttl;
    mod->pub->period = mod_val->pub_val.period;
    mod->pub->retransmit = mod_val->pub_val.retransmit;
    mod->pub->period_div = mod_val->pub_val.period_div;
    mod->pub->count = 0;

    BT_INFO("Restored model publication, dst 0x%04x app_idx 0x%03x", mod_val->pub_val.addr, mod_val->pub_val.key);
    return 0;
}
...
static int mod_set(struct mod_store_val *mod_val)
{
    struct bt_mesh_model *mod;
    uint8_t elem_idx, mod_idx;
    uint16_t mod_key;
    ...
    elem_idx = mod_val->elem_idx;
    mod_idx = mod_val->mod_idx;

    BT_DBG("recover elem_idx %u mod_idx %u is_vnd:%d", elem_idx, mod_idx, mod_val->is_vnd);

    mod = bt_mesh_model_get(mod_val->is_vnd, elem_idx, mod_idx);
    if (!mod) {
        BT_ERR("Failed to get model for elem_idx %u mod_idx %u", elem_idx, mod_idx);
        return -ENOENT;
    }

    if (mod_val->key_conut > 0) {
        err = mod_set_bind(mod, mod_val);
        if (err) {
            BT_ERR("Failed to recover mod bind info elem_idx %u mod_idx %u err:%d", elem_idx, mod_idx, err);
        }
    }

    if (mod_val->group_conut > 0) {
        err = mod_set_sub(mod, mod_val);
        if (err) {
            BT_ERR("Failed to recover mod sub info elem_idx %u mod_idx %u err:%d", elem_idx, mod_idx, err);
        }
    }

    if (mod_val->pub_valid) {
        err = mod_set_pub(mod, mod_val);
        if (err) {
            BT_ERR("Failed to recover mod pub info elem_idx %u mod_idx %u err:%d", elem_idx, mod_idx, err);
        }
    }
...
static void store_pending_mod_bind(struct bt_mesh_model *mod, bool vnd, struct mod_store_val *mod_val)
{
    uint16_t keys[CONFIG_BT_MESH_MODEL_KEY_COUNT] = {0};
    int count;
    unsigned int i;
    ...
    mod_val->is_vnd = vnd;
    mod_val->key_conut = count;
    memcpy(mod_val->keys, keys, sizeof(keys));
}

static void store_pending_mod_sub(struct bt_mesh_model *mod, bool vnd, struct mod_store_val *mod_val)
{
    uint16_t groups[CONFIG_BT_MESH_MODEL_GROUP_COUNT] = {0};
    int i, count;
    ...
    mod_val->is_vnd = vnd;
    mod_val->group_conut = count;
    memcpy(mod_val->groups, groups, sizeof(groups));
}

static void store_pending_mod_pub(struct bt_mesh_model *mod, bool vnd, struct mod_store_val *mod_val)
{
    mod_val->is_vnd = vnd;
    if (!mod->pub || mod->pub->addr == BT_MESH_ADDR_UNASSIGNED) {
        mod_val->pub_valid = false;
        memset(&mod_val->pub_val, 0x00, sizeof(struct mod_pub_val));
    } else {
        mod_val->pub_valid = true;
        mod_val->pub_val.addr = mod->pub->addr;
        mod_val->pub_val.key = mod->pub->key;
        mod_val->pub_val.ttl = mod->pub->ttl;
        mod_val->pub_val.retransmit = mod->pub->retransmit;
        mod_val->pub_val.period = mod->pub->period;
        mod_val->pub_val.period_div = mod->pub->period_div;
        mod_val->pub_val.cred = mod->pub->cred;
    }
}

static void store_pending_mod(
    struct bt_mesh_model *mod, struct bt_mesh_elem *elem, bool vnd, bool primary, void *user_data)
{
    unused(elem);
    unused(primary);
    unused(user_data);
    ...
    struct mod_store_val *mod_val = NULL;
	bool need_store = false;
	// 每次更新mod相关的所有信息
    for (int i = 0; i < CONFIG_BT_MESH_MAX_MOD_STORE_NUM; i++) {
        if (mod_nv[i].is_valid == false && mod_val == NULL) {
            mod_val = &mod_nv[i];
            continue;
        }

        if (mod_nv[i].elem_idx == mod->elem_idx && mod_nv[i].mod_idx == mod->mod_idx && mod_nv[i].is_vnd == vnd) {
            mod_val = &mod_nv[i];
            break;
        }
    }
    if (mod_val == NULL) {
        BT_ERR("store full: elm:0x%0x mod_idx:0x%0x", mod->elem_idx, mod->mod_idx);
        return;
    }

    if (mod->flags & BT_MESH_MOD_BIND_PENDING) {
        mod->flags &= ~BT_MESH_MOD_BIND_PENDING;
		store_pending_mod_bind(mod, vnd, mod_val);
        need_store = true;
    }

    if (mod->flags & BT_MESH_MOD_SUB_PENDING) {
        mod->flags &= ~BT_MESH_MOD_SUB_PENDING;
		store_pending_mod_sub(mod, vnd, mod_val);
        need_store = true;
    }

    if (mod->flags & BT_MESH_MOD_PUB_PENDING) {
        mod->flags &= ~BT_MESH_MOD_PUB_PENDING;
		store_pending_mod_pub(mod, vnd, mod_val);
        need_store = true;
    }
    if (need_store) {
		if (mod_val->key_conut == 0 && mod_val->group_conut == 0 && mod_val->pub_valid == false) {
			mod_val->elem_idx = 0;
        	mod_val->mod_idx = 0;
        	mod_val->is_valid = false;
		} else {
			mod_val->elem_idx = mod->elem_idx;
        	mod_val->mod_idx = mod->mod_idx;
        	mod_val->is_valid = true;
		}
        
        BT_DBG("elem_idx: %d", mod_val->elem_idx);
        BT_DBG("mod_idx: %d", mod_val->mod_idx);
    }
}
...
void bt_mesh_model_pending_store(void)
{
    int err = 0;
    bt_mesh_model_foreach(store_pending_mod, NULL);
    err = ble_mesh_nv_push(BTH_BLE_MESH_MOD_NV_ID, (uint8_t *)mod_nv, sizeof(mod_nv));
    if (err) {
        BT_ERR("Failed to save mod, err: 0x%x", err);
    } else {
        BT_INFO("success save mod");
    }
}
...
void bt_mesh_model_recover(void)
{
    uint16_t real_len = 0;
    errcode_t nv_err = 0;

    BT_DBG("load model");
    nv_err = ble_mesh_data_read(BTH_BLE_MESH_MOD_NV_ID, sizeof(mod_nv), &real_len, (uint8_t *)mod_nv);
    if (nv_err || (real_len != sizeof(mod_nv))) {
        BT_ERR("Failed to load mod nv, err:0x%x, size:%d, real:%d", nv_err, sizeof(mod_nv), real_len);
        return;
    }
    for (uint8_t i = 0; i < CONFIG_BT_MESH_MAX_MOD_STORE_NUM; i++) {
        if (mod_nv[i].is_valid) {
			mod_set(&mod_nv[i]);
	    }
    }
}
...
void bt_mesh_access_init(void)
{
	memset(&mod_nv, 0x00, sizeof(mod_nv));
}
