...
static inline void adv_send(struct os_mbuf *buf)
{
	...
	param.max_events = BT_MESH_TRANSMIT_COUNT(BT_MESH_ADV(buf)->xmit) + 1;
	param.duration = 0;
	param.conn_mode = BLE_GAP_CONN_MODE_NON;

	int64_t time = k_uptime_get();

	err = bt_le_adv_start(&param, &ad, 1, NULL, 0, &adv_id);
    ...
    duration = adv_int * (param.max_events + 1);
		...
		if (MYNEWT_VAL(BLE_MESH_GATT_SERVER)) {
			...
			while (!ev) {
                ...
                if (osal_kthread_should_stop() != 0) {
                    BT_INFO("mesh_adv_thread exit while ev \r\n");
                    break;
                }
...
void bt_mesh_clear_adv_buf(void)
{
	ble_npl_eventq_deinit(&bt_mesh_adv_queue);
}

uint8_t bt_mesh_get_adv_buf_num(void)
{
	return bt_mesh_adv_queue.size;
}
...
void bt_mesh_adv_deinit(void)
{
	...
	os_mbuf_pool_deinit(&adv_os_mbuf_pool);

	os_mempool_deinit(&adv_buf_mempool);

    memset_s(&adv_buf_mempool, sizeof(adv_buf_mempool), 0, sizeof(adv_buf_mempool));
    memset_s(&adv_os_mbuf_pool, sizeof(adv_os_mbuf_pool), 0, sizeof(adv_os_mbuf_pool));
		      ...
		      const struct bt_data *sd, size_t sd_len)
{
	...
	return bt_le_adv_start(param, ad, ad_len, sd, sd_len, &adv_id);
}
#endif
