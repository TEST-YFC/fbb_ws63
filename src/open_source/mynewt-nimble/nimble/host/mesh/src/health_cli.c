...
static int health_cli_update(struct bt_mesh_model *mod)
{
    return 0;
}
...
static int health_cli_deinit(struct bt_mesh_model *model)
{
    ...
    bt_mesh_msg_ack_ctx_deinit(&cli->ack_ctx);
    return 0;
}
...
static int health_cli_init(struct bt_mesh_model *model)
{
    ...
    model->pub->update = health_cli_update;
...
void *bt_mesh_get_health_cli_model(void)
{
    if (health_cli) {
        return (void *)health_cli->model;
    }
    return NULL;
}