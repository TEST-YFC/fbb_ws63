...
int bt_mesh_hb_recv(struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
	...
	uint32_t rem_ms = k_ticks_to_ms_floor32(k_work_delayable_remaining_get(&sub_timer));
	if(!rem_ms){
		BT_DBG("over time");
		return 0;
	}
	...
	if (sub.count < 0xffff && sub.period > 0) {
		sub.count++;
	}
...
void bt_mesh_hb_deinit(void)
{
	k_work_cancel_delayable_delete(&pub_timer);
	k_work_cancel_delayable_delete(&sub_timer);
}
...
static int hb_pub_set(struct hb_pub_val *val)
{
	struct bt_mesh_hb_pub pub;

	pub.dst = val->dst;
	pub.period = bt_mesh_hb_pwr2(val->period);
	pub.ttl = val->ttl;
	pub.feat = val->feat;
...
void bt_mesh_hb_pub_pending_store(void)
{
	...
	err = ble_mesh_nv_push(BTH_BLE_MESH_HB_PUB_NV_ID, (uint8_t *)&val, sizeof(val));
	if (err) {
		BT_ERR("Failed to store Heartbeat Publication");
	} else {
		BT_INFO("Stored Heartbeat Publication");
	}
}

void bt_mesh_hb_pub_recover(void)
{
	struct hb_pub_val val;
	int err;
	uint16_t real_len = 0;
	...
	BT_DBG("load hb pub");
	nv_err = ble_mesh_data_read(BTH_BLE_MESH_HB_PUB_NV_ID, sizeof(val), &real_len, (uint8_t *)&val);
	if (nv_err) {
		BT_ERR("Failed to load Heartbeat Publication, err:0x%x", nv_err);
		return;
	}

	if (real_len != sizeof(val)) {
		BT_ERR("Failed to load Heartbeat Publication, unexpected value len real:%d: %d:", real_len, sizeof(val));
		return;
	}

	if (val.dst == BT_MESH_ADDR_UNASSIGNED) {
		BT_INFO("load hb pub no pub info");
		return;
	}

	err = hb_pub_set(&val);
	if (err) {
		BT_ERR("Failed to set hb pub, err:%d", err);
		return;
	}

}

int bt_mesh_hb_send(void)
{
	return heartbeat_send(NULL, NULL);
}
#endif