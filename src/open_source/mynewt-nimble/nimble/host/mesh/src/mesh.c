...
static void bt_mesh_comp_data_clear(void)
{
...
{
    BT_DBG("0x%04x devkey: %s", addr, bt_hex(bt_mesh.dev_key_cand, sizeof(bt_mesh.dev_key_cand)));
...
{
    BT_DBG("%s", bt_hex(key, len));
    if (memcpy_s(bt_mesh.dev_key_cand, sizeof(bt_mesh.dev_key_cand), key, len) != EOK) {
        return;
    }
...
void bt_mesh_dev_key_cand_remove(void)
{
    ...
    BT_DBG("");
    memset_s(bt_mesh.dev_key_cand, sizeof(bt_mesh.dev_key_cand), 0, sizeof(bt_mesh.dev_key_cand));
    return;
}
...
void bt_mesh_dev_key_cand_activate(void)
{
    ...
    memcpy_s(bt_mesh.dev_key, sizeof(bt_mesh.dev_key), bt_mesh.dev_key_cand, sizeof(bt_mesh.dev_key_cand));
    memset_s(bt_mesh.dev_key_cand, sizeof(bt_mesh.dev_key), 0, sizeof(bt_mesh.dev_key));
...
int bt_mesh_start(void)
{
	...
	if (bt_mesh_beacon_enabled() || (bt_mesh_priv_beacon_get() == BT_MESH_PRIV_BEACON_ENABLED)) {
		bt_mesh_beacon_enable();
	} else {
		bt_mesh_beacon_disable();
	}
	...
	(bt_mesh_prov_link.bearer != NULL && bt_mesh_prov_link.bearer->type == BT_MESH_PROV_ADV)) {
