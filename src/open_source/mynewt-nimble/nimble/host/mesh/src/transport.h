...
#define BT_MESH_SDU_UNSEG_MAX          MYNEWT_VAL(BT_MESH_SDU_UNSEG_MAX)
...
#define TRANS_CTL_OP_PING_REQ          0x0b
#define TRANS_CTL_OP_PING_RSP          0x0C
...
// mesh ping回调函数
typedef void (*bt_mesh_ping_callback)(struct bt_mesh_net_rx *rx, struct os_mbuf *buf);

typedef struct {
    bt_mesh_ping_callback mesh_ping_func;       /*!< @if Eng bt mesh ping callback
                                                    @else   bt mesh ping 回调函数 @endif */
} bt_mesh_ping_callback_t;
