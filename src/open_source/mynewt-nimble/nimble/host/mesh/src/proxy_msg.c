			   ...
			   struct os_mbuf *msg)
{
	...
	if ((role == NULL) || (msg == NULL)) {
        BT_ERR("role: %s, msg: %s",
            role == NULL?"NULL": "OK", msg == NULL?"NULL": "OK");
		return -EINVAL;
	}
    ...
    if (mtu > msg->om_len) {
		...
		return role->cb.send(conn_handle, msg->om_data, msg->om_len);// call proxy_send
    }
	...
	proxy_recv_cb_t recv)
{
    ...
    for (uint16_t i = 0; i < CONFIG_BT_MAX_CONN; i++) {
        if (roles[i].conn_handle == 0xFFFF && role == NULL) {
            role = &roles[i];
            continue;
        }

        if (roles[i].conn_handle == conn_handle) {
            role = &roles[i];
            break;
        }
    }
