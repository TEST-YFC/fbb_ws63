...
static void k_work_deinit(struct ble_npl_callout *work)
{
    ble_npl_callout_stop(work);
    work->c_ev.ev_cb = NULL;
    work->c_ev.ev_arg = NULL;
    work->c_ev.ev_queued = 0;
    work->c_evq = NULL;
    work->c_active = false;
}

void k_work_deinit_delete(struct ble_npl_callout *work)
{
    k_work_deinit(work);
    ble_npl_callout_delete(work->c_timer);
}
...
{
#ifndef MYNEWT
    ble_npl_callout_init(work, NULL, handler, NULL);
#else
...
{
#ifndef MYNEWT
    ble_npl_callout_init(&w->work, NULL, f, NULL);
#else
...
int
k_work_cancel_delayable_delete(struct k_work_delayable *w)
{
    k_work_deinit_delete(&w->work);
    return 0;
}
...
int
bt_dh_key_gen(const uint8_t *pub_key, const uint8_t *priv_key, uint8_t *dhkey)
{
