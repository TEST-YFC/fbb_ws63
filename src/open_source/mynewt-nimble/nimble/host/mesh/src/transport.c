...
static struct seg_tx {
	...
	uint8_t               seg_len;       /* length of segment packet */
    uint8_t               blocked : 1,      /* Blocked by ongoing tx */
                          ...
                          slab_buff : 1;    /* Buff is use slab buff */
...
static bt_mesh_ping_callback_t g_bt_mesh_ping_cbks;

/*
 * Add for feature support extend segment.
 * This function takes effect when the CONFIG_BT_MESH_EN_SEG_ADV_SUPPORT switch is turned on
 * and extended segment support is enabled.
 */
bool g_ext_adv_seg_enable = false;
static bool bt_mesh_is_en_adv(void)
{
    return IS_ENABLED(CONFIG_BT_MESH_EN_SEG_ADV_SUPPORT) && g_ext_adv_seg_enable;
}
...
{
    /*
     * Modify for feature support extend segment.
     * When using extended segments, memory is not allocated from the memory pool(segs).
	 * Instead, memory is allocated using osal_kmalloc, and it needs to be released using osal_kfree.
     */
    if (tx->slab_buff) {
        k_mem_slab_free(&segs, (void **)&tx->seg[seg_idx]);
    } else {
        osal_kfree(tx->seg[seg_idx]);
    }
			     ...
			     struct os_mbuf *buf)
{
    ...
    /*
     * Modify for feature support extend segment.
     * When an extended segment is used, the segment length is 230.
     * During TX allocation, the fragment length is set based on the length of the received packet.
     */
    uint8_t len = MIN(tx->seg_len, tx->len - (tx->seg_len * seg_o));
		    ...
		    uint8_t *ctl_op)
{
     ...
     * During TX allocation, the fragment length is set based on the length of the received packet.
     */
    if (bt_mesh_is_en_adv()) {
        tx->slab_buff = false;
        tx->seg_len = BT_MESH_APP_SEG_EXT_SDU_MAX;
    } else {
        tx->slab_buff = true;
        tx->seg_len = seg_len(!!ctl_op);
    }

    tx->src = net_tx->src;
	tx->dst = net_tx->ctx->addr;
	tx->seg_n = (sdu->om_len - 1) / tx->seg_len;
	tx->seg_o = 0;
	tx->len = sdu->om_len;
	...
	for (seg_o = 0; sdu->om_len; seg_o++) {
		...
		uint16_t len;
		int err;
        len = MIN(sdu->om_len, tx->seg_len);
        /*
         *  Modify for feature support extend segment.
         *  When an extended segment is used, the segment length is 230.
         *  During the memory allocation for the segment, the memory pool is not used,
         *  and system memory is directly allocated.
         */
        if (tx->slab_buff) {
            ...
            buf = osal_kmalloc(len, 0);
            if (buf == NULL) {
                BT_ERR("malloc failed, Out of segment buffers");
                seg_tx_reset(tx);
                return -ENOBUFS;
            }
        }
		...
		if (IS_ENABLED(CONFIG_BT_MESH_FRIEND)) {
			...
			struct os_mbuf *seg = NET_BUF_SIMPLE(len + 4);
			...
			if (bt_mesh_friend_enqueue_tx(
				 ...
				 * out through the Friend Queue.
				 * Modify for feature support extend segment.
				 * If tx->slab_buff is true, memory is allocated using the memory pool.
				 * If it is false, system memory is allocated, and osal_kfree is used for deallocation.
				 */
                if (tx->slab_buff) {
		       ...
		       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if ((tx == NULL) || (msg == NULL)) {
			BT_ERR("tx: %s, msg: %s", tx == NULL?"NULL":"OK", msg == NULL?"NULL":"OK");
            return -EINVAL;
	}
	if (msg->om_len < 1) {
	...
	if (tx->ctx->addr == BT_MESH_ADDR_UNASSIGNED ||
	    (!BT_MESH_ADDR_IS_UNICAST(tx->ctx->addr) &&
		 ...
		 !atomic_test_and_clear_bit(bt_mesh.flags, BT_MESH_NET_TEST))) {
			    ...
			    uint8_t aszmic)
{
	...
	for (i = 0; i <= rx->seg_n; i++) {
		net_buf_simple_add_mem(buf, rx->seg[i], MIN(rx->seg_len,
					   rx->len - (i * rx->seg_len)));
	}
...
int bt_mesh_register_ping_callbacks(bt_mesh_ping_callback_t *mesh_callback)
{
    (void)memcpy_s(&g_bt_mesh_ping_cbks, sizeof(bt_mesh_ping_callback_t),
        mesh_callback, sizeof(bt_mesh_ping_callback_t));
    return 0;
}

int bt_mesh_ping_req(struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
	struct bt_mesh_msg_ctx ctx = {
		.net_idx = rx->ctx.net_idx,
		.app_idx = BT_MESH_KEY_UNUSED,
		.addr = rx->ctx.addr,
		.send_ttl = rx->ctx.send_ttl,
	};
	struct bt_mesh_net_tx tx = {
		.sub = bt_mesh_subnet_get(rx->ctx.net_idx),
		.ctx = &ctx,
	...
	if (buf->om_len < 3) {
		BT_ERR("Too short ping message");
		return -EINVAL;
	}

	if (rx->ctx.recv_dst != 0xffff && rx->ctx.recv_dst != tx.src) {    // 不处理非自己消息
		return 0;
	}

	BT_INFO("src 0x%04x TTL %u ping messsage recv",
	       rx->ctx.addr, rx->ctx.recv_ttl);


	/* Do nothing if heartbeat publication is not enabled or the subnet is
	 * removed.
	 */
	if (!tx.sub || rx->ctx.addr == BT_MESH_ADDR_UNASSIGNED) {
		return 0;
	}
	return bt_mesh_ctl_send(&tx, TRANS_CTL_OP_PING_RSP, buf->om_data, buf->om_len,
				NULL, NULL);
}

int bt_mesh_ping_rsp(struct bt_mesh_net_rx *rx, struct os_mbuf *buf)
{
	if (buf->om_len < 3) {
		BT_ERR("Too short ping message");
		return -EINVAL;
	...
	if (rx->ctx.recv_dst != 0xffff && rx->ctx.recv_dst != bt_mesh_primary_addr()) {    // 不处理非自己消息
		return 0;
	}

	BT_INFO("src 0x%04x TTL %u ping messsage rsp",
	       rx->ctx.addr, rx->ctx.recv_ttl);
	if (g_bt_mesh_ping_cbks.mesh_ping_func != NULL) {
		g_bt_mesh_ping_cbks.mesh_ping_func(rx, buf);
	}
	return 0;
}
		    ...
		    struct os_mbuf *buf, uint64_t *seq_auth)
{
	...
	case TRANS_CTL_OP_PING_REQ:
		return bt_mesh_ping_req(rx, buf);
	case TRANS_CTL_OP_PING_RSP:
		return bt_mesh_ping_rsp(rx, buf);
	}
		       ...
		       uint64_t *seq_auth)
{
	...
	sdu = NET_BUF_SIMPLE(BT_MESH_SDU_UNSEG_MAX);
...
static void seg_rx_reset(struct seg_rx *rx, bool full_reset)
{
	...
	for (i = 0; i <= rx->seg_n; i++) {
        ...
        /*
         * Modify for feature support extend segment.
         * If tx->slab_buff is true, memory is allocated using the memory pool(segs).
         * If it is false, system memory is allocated, and osal_kfree is used for deallocation.
         */
        if (rx->slab_buff) {
		     ...
		     uint8_t *seg_count)
{
	...
	*seg_count = seg_n + 1;
    ...
    /*
     * Modify for feature support extend segment.
     * When an extended segment is used, the maximum segment length is 230.
     * If the length of the received fragment is greater than BT_MESH_APP_SEG_SDU_MAX,
     * it is considered that an extended segment is used.
     * In this case, the memory fragment uses the system memory instead of the memory pool(segs).
     */
    if (buf->om_len > BT_MESH_APP_SEG_EXT_SDU_MAX) {
        BT_WARN("invalid segmented length:%d", buf->om_len);
        return -ENOMEM;
    }
    if (buf->om_len > BT_MESH_APP_SEG_SDU_MAX) {
        rx->slab_buff = false;
        rx->seg_len = buf->om_len;
    } else {
        rx->slab_buff = true;
        rx->seg_len = seg_len(rx->ctl);
    }
...
found_rx:
	...
	if (seg_o == seg_n) {
		...
		rx->len = seg_n * rx->seg_len  + buf->om_len;
		BT_DBG("Target len %u * %u + %u = %u", seg_n, rx->seg_len,
		       buf->om_len, rx->len);
	...
	/* 
	 * Modify for feature support extend segment
	 * The memory fragment uses the system memory instead of the memory pool(segs).
	 */
    if (rx->slab_buff) {
        ...
        rx->seg[seg_o] = osal_kmalloc(buf->om_len, 0);
        if (rx->seg[seg_o] == NULL) {
            BT_WARN("Unable osal malloc buffer for Seg %u", seg_o);
            return -ENOBUFS;
        }
    }
...
{
#if MYNEWT_VAL(BLE_MESH_SETTINGS) || MYNEWT_VAL(BLE_MESH_SETTINGS_EXT)
...
void bt_mesh_trans_deinit(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(seg_tx); i++) {
        k_work_cancel_delayable_delete(&seg_tx[i].retransmit);
    }

    for (i = 0; i < ARRAY_SIZE(seg_rx); i++) {
        k_work_cancel_delayable_delete(&seg_rx[i].ack);
    }
    bt_mesh_trans_reset();
}
...
static int va_set(uint16_t index, struct va_val *va)
{
	if (va->ref == 0) {
		BT_WARN("Ignore Mesh Virtual Address ref = 0");
		return 0;
	...
	struct virtual_addr *lab = bt_mesh_va_get(index);
	if (lab == NULL) {
		BT_WARN("Out of labels buffers");
		return -ENOBUFS;
	}
	...
	BT_INFO("Restored Virtual Address, addr 0x%04x ref 0x%04x", lab->addr, lab->ref);
...
{
	struct virtual_addr *lab;
	struct va_val va[CONFIG_BT_MESH_LABEL_COUNT] = { 0 };
	uint16_t i;
	int err = 0;
	...
	for (i = 0; (lab = bt_mesh_va_get(i)) != NULL; i++) {
		...
		if (IS_VA_DEL(lab)) {
			memset(&va[i], 0x00, sizeof(struct va_val));
		} else {
			va[i].ref = lab->ref;
			va[i].addr = lab->addr;
			memcpy(va[i].uuid, lab->uuid, 16);
		}
	}
	err = ble_mesh_nv_push(BTH_BLE_MESH_VA_NV_ID, (uint8_t *)va, sizeof(va));
	if (err) {
		BT_ERR("ble_mesh_nv_push (err %d)", err);
	} else {
		BT_INFO("ble_mesh_nv_push success");
	}
}

void bt_mesh_va_recover(void)
{
	struct va_val va[CONFIG_BT_MESH_LABEL_COUNT] = { 0 };
	uint16_t i;
	uint16_t real_len = 0;
	errcode_t nv_err = 0;
	int err;
	uint16_t recover_num = 0;

	BT_DBG("load va");
	nv_err = ble_mesh_data_read(BTH_BLE_MESH_VA_NV_ID, sizeof(va), &real_len, (uint8_t *)va);
	if (nv_err) {
		BT_ERR("Failed load virtual addr, err: 0x%x", nv_err);
		return;
	}

	if (real_len != sizeof(va)) {
		BT_ERR("Failed to load virtual addr, unexpected value len real:%d: %d:", real_len, sizeof(va));
		return;
	}
	...
	for (i = 0; i < CONFIG_BT_MESH_LABEL_COUNT; i++) {
		...
		err = va_set(i, &va[i]);
		if (err) {
			BT_ERR("Failed to set virtual addr, addr:0x%04x, err:%d", va[i].addr, err);
		} else {
			recover_num++;
		}
	}
	BT_DBG("load va num:%d", recover_num);
}
#endif
 ...
 * Add for feature support extend segment.
 */
void bt_mesh_set_en_adv(bool flag)
{
    g_ext_adv_seg_enable = flag;
}