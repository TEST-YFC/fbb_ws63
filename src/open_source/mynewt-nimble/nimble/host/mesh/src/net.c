...
#define LOOPBACK_MAX_PDU_LEN (BT_MESH_NET_HDR_LEN + 251)
...
struct bt_mesh_net bt_mesh = {
    ...
    .priv_beacon_int = BT_MESH_PRIV_BEACON_INTERVAL,
#endif
};
...
static int loopback(const struct bt_mesh_net_tx *tx, struct os_mbuf *buf)
{
...
static bool net_decrypt(struct bt_mesh_net_rx *rx, struct os_mbuf *in,
			...
			const struct bt_mesh_net_cred *cred)
{
    if ((out == NULL) || (in == NULL) || (rx == NULL) || (cred == NULL)) {
        BT_ERR("in: %s, rx: %s, out: %s, cred: %s",
            in == NULL?"NULL": "OK", rx == NULL?"NULL": "OK", out == NULL?"NULL": "OK", cred == NULL?"NULL": "OK");
            return false;
	}
    ...
    if (rx->net_if != BT_MESH_NET_IF_LOCAL && bt_mesh_has_addr(rx->ctx.addr)) {
			      ...
			      struct bt_mesh_net_rx *rx)
{
	...
	/* 内存池使用过半时不进行中继转发 */
	if (adv_buf_mempool.mp_num_free < (adv_buf_mempool.mp_num_blocks / 2)) { // 2:half
		if (relay_drop_num > 0 && relay_drop_num % 100 == 0) {
			BT_INFO("Relay Insufficient resources, %u", adv_buf_mempool.mp_num_free);
		}
		relay_drop_num++;
		return;
	}
	 ...
	 */
	if (IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY) &&
        (rx->friend_cred || bt_mesh_gatt_proxy_get() == BT_MESH_GATT_PROXY_ENABLED ||
            bt_mesh_priv_gatt_proxy_get() == BT_MESH_GATT_PROXY_ENABLED)) {
...
{
    if ((in == NULL) || (rx == NULL) || (out == NULL)) {
		BT_ERR("in: %s, rx: %s, out: %s", in == NULL?"NULL": "OK", rx == NULL?"NULL": "OK", out == NULL?"NULL": "OK");
		return -1;
	}
		      ...
		      enum bt_mesh_net_if net_if)
{
	...
	if (!bt_mesh_is_provisioned()) {
		BT_DBG("Not provisioned; dropping packet");
		goto done;
	}
...
static void ivu_refresh(struct ble_npl_event *work)
{
	...
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS)) {
		bt_mesh_beacon_ivu_initiator(true);
		bt_mesh_net_iv_update(bt_mesh.iv_index, false);
	} else if (IS_ENABLED(CONFIG_BT_SETTINGS) || IS_ENABLED(CONFIG_BT_SETTINGS_EXT)) {
		store_iv(true);
	}
}
...
{
	struct os_mbuf *buf, *next;
	k_work_cancel_delayable_delete(&bt_mesh.ivu_timer);
	k_work_deinit_delete(&bt_mesh.local_work); 
	
	buf = net_buf_slist_get(&bt_mesh.local_queue);
    while (buf) {
        next = net_buf_slist_get(&bt_mesh.local_queue);
        net_buf_unref(buf);
        buf = next;
    }
}
...
static void clear_iv(void)
{
    struct iv_val_store iv;
    int err;

    iv.valid = false;
    memset(&iv.val, 0x00, sizeof(struct iv_val));

    err = ble_mesh_nv_push(BTH_BLE_MESH_IV_NV_ID, (uint8_t *)&iv, sizeof(iv));
    if (err) {
        BT_ERR("Failed to clear_iv IV");
    } else {
        BT_INFO("clear_iv IV");
    }
}
...
static void store_pending_iv(void)
{
    ...
    errcode_t err;
    iv.valid = true;
    iv.val.iv_index = bt_mesh.iv_index;
    iv.val.iv_update = atomic_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS);
    iv.val.iv_duration = bt_mesh.ivu_duration;
...
static void clear_net(void)
{
    ...
    err = ble_mesh_nv_push_encrypt(BTH_BLE_MESH_NET_ADDR_NV_ID, (uint8_t *)&net, sizeof(net));
    if (err) {
        BT_ERR("Failed to clear Network");
    } else {
        BT_DBG("Cleared Network");
...
static int net_set(struct net_val *net)
{
    if (!net || net->primary_addr == 0x00) {
        bt_mesh_comp_unprovision();
        memset(bt_mesh.dev_key, 0, sizeof(bt_mesh.dev_key));
        BT_ERR("load net addr no data, not provision", net->primary_addr);
        return 0;
    }
    ...
    BT_INFO("Provisioned with primary address 0x%04x", net->primary_addr);
    BT_DBG("Recovered DevKey %s", bt_hex(bt_mesh.dev_key, 16));

    return 0;
}

static int iv_set(struct iv_val_store *iv)
{
    if (!iv->valid) {
        bt_mesh.iv_index = 0U;
        atomic_clear_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS);
    ...
    bt_mesh.iv_index = iv->val.iv_index;
    atomic_set_bit_to(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS, iv->val.iv_update);
    bt_mesh.ivu_duration = iv->val.iv_duration;

    BT_INFO("IV Index 0x%04x (IV Update Flag %u) duration %u hours",
        (unsigned)iv->val.iv_index,
        iv->val.iv_update,
        iv->val.iv_duration);
...
static int seq_set(struct seq_val *seq)
{
    if (!seq) {
        bt_mesh.seq = 0;
        return 0;
    ...
    BT_INFO("Sequence Number 0x%06x", bt_mesh.seq);
...
void bt_mesh_net_pending_seq_store(void)
{
    ...
    err = ble_mesh_nv_push(BTH_BLE_MESH_SEQ_NV_ID, (uint8_t *)&seq, sizeof(seq));
    if (err) {
        BT_ERR("Failed to save seq:0x%06x", bt_mesh.seq);
    } else {
        BT_INFO("success to save seq:0x%06x", bt_mesh.seq);
    }
}

void bt_mesh_net_seq_recover(void)
{
    struct seq_val seq = { 0 };
    uint16_t real_len = 0;
    errcode_t err;
    
    err = ble_mesh_data_read(BTH_BLE_MESH_SEQ_NV_ID, sizeof(struct seq_val), &real_len, (uint8_t *)&seq);
    if (err) {
        BT_ERR("Failed to load seq number, err:0x%x", err);
        return;
    }
    if (real_len != sizeof(struct seq_val)) {
        BT_ERR("Failed to load seq number, unexpected value len, real:%d: %d:", real_len, sizeof(struct seq_val));
        return;
    }
	BT_DBG("load seq num:%d", sys_get_le24(seq.val));
    seq_set(&seq);
}

void bt_mesh_net_iv_recover(void)
{
    struct iv_val_store iv;
    uint16_t real_len = 0;
    errcode_t err;
    ...
    err = ble_mesh_data_read(BTH_BLE_MESH_IV_NV_ID, sizeof(struct iv_val_store), &real_len, (uint8_t *)&iv);
    if (err) {
        BT_ERR("Failed to load iv, err:0x%x", err);
        return;
    }
    if (real_len != sizeof(struct iv_val_store)) {
        BT_ERR("Failed to load iv, unexpected value len, real:%d: %d:", real_len, sizeof(struct iv_val_store));
        return;
    }
    iv_set(&iv);
}

int bt_mesh_net_recover(void)
{
    struct net_val net;
    uint16_t real_len = 0;
    errcode_t err;
    BT_INFO("load Network");
    err = ble_mesh_data_read(BTH_BLE_MESH_NET_ADDR_NV_ID, sizeof(net), &real_len, (uint8_t *)&net);
    if (err) {
        BT_ERR("Failed to load Network, err:0x%x", err);
        return err;
    }
    if (real_len != sizeof(net)) {
        BT_ERR("Failed to load Network, unexpected value len real:%d: %d:", real_len, sizeof(net));
        return -1;
    }
    net_set(&net);
	return 0;
