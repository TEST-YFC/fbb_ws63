...
static void bt_mesh_comp_page_process(uint8_t *page)
{
    if (*page >= 129U && atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY)) {
        *page = 129U;
    } else if (*page >= 128U && atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY)) {
        *page = 128U;
    } else if (*page >= 2U && atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY)) {
        *page = 2U;
    } else if (*page >= 1U && atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY)) {
        *page = 1U;
    } else {
        *page = 0;
    }
			     ...
			     struct os_mbuf *buf)
{
	...
	bt_mesh_comp_page_process(&page);

	bt_mesh_model_msg_init(sdu, OP_DEV_COMP_DATA_STATUS);
	net_buf_simple_add_u8(sdu, page);
#ifdef CONFIG_BLE_MESH_PTS_MODE
	if (atomic_test_bit(bt_mesh.flags, BT_MESH_COMP_DIRTY) && page == 0) {
		err = bt_mesh_comp_get_page_128(sdu);
	} else {
			 ...
			 uint8_t retransmit, bool store)
{
	...
	if (pub_addr == BT_MESH_ADDR_UNASSIGNED) {
		...
		if ((IS_ENABLED(CONFIG_BT_SETTINGS) || IS_ENABLED(CONFIG_BT_SETTINGS_EXT)) && store) {
...
static void mod_app_key_add(struct bt_mesh_model *mod,
			    struct bt_mesh_elem *elem, bool vnd, bool primary,
			    void *user_data)
{
	uint16_t *app_idx = user_data;
	for (int i = 0; i < ARRAY_SIZE(mod->keys); i++) {
		/* use dev key model not auto bind */
		if (mod->keys[i] == BT_MESH_KEY_DEV || mod->keys[i] == BT_MESH_KEY_DEV_REMOTE) {
			BT_INFO("mod:0x%04x use dev key not bind.", mod->id);
			return;
		}
	}
	mod_bind(mod, *app_idx);
}
			...
			enum bt_mesh_key_evt evt)
{
	...
	// feature auto bind
	if (IS_ENABLED(BLE_MESH_AUTO_MOD_BIND)) {
		if (evt == BT_MESH_KEY_ADDED) {
			bt_mesh_model_foreach(&mod_app_key_add, &app_idx);
		}
	}
}
			   ...
			   struct os_mbuf *buf)
{
    ...
    if (IS_ENABLED(BLE_MESH_PRIV_BEACONS) && buf->om_data[0]) {
...
{
	uint16_t elem_addr;
	uint16_t sub_addr = BT_MESH_ADDR_UNASSIGNED;
	struct bt_mesh_model *mod;
	struct bt_mesh_elem *elem;
	uint8_t *label_uuid;
	uint8_t *mod_id;
	...
	(void)bt_mesh_va_del(label_uuid, &sub_addr);
...
int bt_mesh_overwrite_mod_sub(struct bt_mesh_model *mod, uint16_t sub_addr)
{
    if (ARRAY_SIZE(mod->groups) > 0) {
        bt_mesh_model_extensions_walk(mod, mod_sub_clear_visitor, NULL);
...
int bt_mesh_del_mod_sub(struct bt_mesh_model *mod, uint16_t sub_addr)
{
    uint16_t *match;
...
int bt_mesh_add_mod_sub(struct bt_mesh_model *mod, uint16_t sub_addr)
{
    uint16_t *entry;
    ...
    *entry = sub_addr;
    ...
    if (IS_ENABLED(CONFIG_BT_SETTINGS) || IS_ENABLED(CONFIG_BT_SETTINGS_EXT)) {
        (void)bt_mesh_model_sub_store(mod);
    }
 
    if (IS_ENABLED(CONFIG_BT_MESH_LOW_POWER)) {
        (void)bt_mesh_lpn_group_add(sub_addr);
    }
