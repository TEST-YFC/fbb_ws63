...
int prov_ccc_write(uint16_t conn_handle, uint8_t type)
{
	if (cli == NULL) {
		cli = bt_mesh_proxy_role_setup(conn_handle, gatt_send, proxy_msg_recv);
		BT_ERR("No PB-GATT Client found, setup client. conn:%d", conn_handle);
	}
	
	if (cli == NULL || cli->conn_handle != conn_handle) {
...
static size_t gatt_prov_adv_create(struct bt_data *prov_sd)
{
	...
	if (prov_sd == NULL) {
		BT_WARN("sd null to fit advertising packet");
		return sd_len;
	}
...
{
	BT_DBG("");
	// feat: do not send gatt adv when prov link is active
	if (!service_registered || bt_mesh_is_provisioned() || bt_mesh_prov_active()
	...
	struct bt_data *prov_sd = sd_data;
#else
	struct bt_data *prov_sd = NULL;
#endif

	size_t prov_sd_len = 1;
	int err;
