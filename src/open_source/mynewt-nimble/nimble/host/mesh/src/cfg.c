...
int bt_mesh_priv_beacon_set(enum bt_mesh_feat_state priv_beacon)
{
    ...
    if (!IS_ENABLED(BLE_MESH_PRIV_BEACONS)) {
        return -ENOTSUP;
    }
    ...
    if ((IS_ENABLED(CONFIG_BT_SETTINGS) || IS_ENABLED(CONFIG_BT_SETTINGS_EXT)) && atomic_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
    #ifdef CONFIG_BLE_MESH_PRIV_BEACON_SRV
...
{
    int err;
	BT_INFO("priv_gatt_proxy: %u", priv_gatt_proxy);
...
bool bt_mesh_fixed_group_match(uint16_t addr)
{
	...
	case BT_MESH_ADDR_PROXIES:
        return (bt_mesh_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED ||
                bt_mesh_priv_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED);
...
static void store_pending_cfg(void)
{
	...
	val.private_gatt_proxy = bt_mesh_priv_gatt_proxy_get();
...
static int cfg_set(struct cfg_val *cfg)
{
    int len, err;

    if (!cfg->valid) {
        BT_INFO("Cleared configuration state");
        return 0;
    }
    ...
    bt_mesh_beacon_set(cfg->beacon);
    bt_mesh_gatt_proxy_set(cfg->gatt_proxy);
	bt_mesh_priv_gatt_proxy_set(cfg->private_gatt_proxy);
    bt_mesh_friend_set(cfg->frnd);
    bt_mesh_default_ttl_set(cfg->default_ttl);
...
void bt_mesh_cfg_pending_store(void)
{
    ...
    err = ble_mesh_nv_push(BTH_BLE_MESH_CFG_NV_ID, &val, sizeof(val));
    if (err) {
        BT_ERR("Failed to store configuration");
    } else {
        BT_DBG("Stored configuration");
...
void bt_mesh_cfg_recover(void)
{
    ...
    BT_DBG("load cfg");
    nv_err = ble_mesh_data_read(BTH_BLE_MESH_CFG_NV_ID, sizeof(val), &real_len, (uint8_t *)&val);
    if (nv_err) {
        BT_ERR("Failed to load configuration, err:0x%x", nv_err);
        return;
    }

    if (real_len != sizeof(val)) {
        BT_ERR("Failed to load net cfg, unexpected value len real:%d: %d:", real_len, sizeof(val));
        return;
    }
    (void)cfg_set(&val);
}
