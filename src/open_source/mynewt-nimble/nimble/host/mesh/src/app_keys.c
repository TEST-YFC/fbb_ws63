...
// app key持久化结构
struct app_key_store {
    uint16_t app_idx; /* app index */
    struct app_key_val val;
} __packed;

static struct app_key_store app_key_stores[CONFIG_BT_MESH_APP_KEY_COUNT] = {
    [0 ...(CONFIG_BT_MESH_APP_KEY_COUNT - 1)] = {
        .app_idx = BT_MESH_KEY_UNUSED,
    }};

#if MYNEWT_VAL(BLE_MESH_SETTINGS_EXT)
static int app_key_set(struct app_key_store *app)
{
    struct app_key_val *key = &app->val;
    uint16_t app_idx = app->app_idx;
    int err;

    err = bt_mesh_app_key_set(app_idx, key->net_idx, key->val[0], key->updated ? key->val[1] : NULL);
    if (err) {
        BT_ERR("Failed to set \'app-key\'");
        return err;
    }
...
void clear_app_key(uint16_t app_idx)
{
    unused(app_idx);
    int err;
    unsigned i;
    for (i = 0; i < ARRAY_SIZE(app_key_stores); i++) {
        struct app_key_store *store = &app_key_stores[i];
        if (store->app_idx == app_idx) {
            memset(store, 0x00, sizeof(struct app_key_store));
            store->app_idx = BT_MESH_KEY_UNUSED;
            break;
        }
    }
    ...
    err = ble_mesh_nv_push_encrypt(BTH_BLE_MESH_APP_KEY_NV_ID, (uint8_t *)app_key_stores, sizeof(app_key_stores));
    if (err) {
        BT_ERR("Failed to clear AppKeyIndex 0x%03x", app_idx);
    } else {
        BT_DBG("Cleared AppKeyIndex 0x%03x", app_idx);
...
void store_app_key(uint16_t app_idx)
{
    unused(app_idx);
    const struct app_key *app;
    struct app_key_val *key;
    ...
    int err;
    unsigned i;
    struct app_key_store *store = NULL;
    for (i = 0; i < ARRAY_SIZE(app_key_stores); i++) {
        if (app_key_stores[i].app_idx == BT_MESH_KEY_UNUSED && store == NULL) {
            store = &app_key_stores[i];
            continue;
        }

        if (app_key_stores[i].app_idx == app_idx) {
            store = &app_key_stores[i];
            break;
        }
    }
    ...
    if (!store) {
        BT_WARN("ApKeyIndex 0x%03x store pos not found", app_idx);
        return;
    }
    ...
    err = ble_mesh_nv_push_encrypt(BTH_BLE_MESH_APP_KEY_NV_ID, (uint8_t *)app_key_stores, sizeof(app_key_stores));
    if (err) {
        BT_ERR("Failed to store AppKey: 0x%03x, error:0x%x", app_idx, err);
    } else {
        BT_INFO("Stored AppKey: 0x%03x", app_idx);
    }
}

void bt_mesh_app_key_recover(void)
{
    // int i;
    uint16_t real_len = 0;
    int err;
    ...
    BT_DBG("load app key");
    nv_err = ble_mesh_data_read(BTH_BLE_MESH_APP_KEY_NV_ID, sizeof(app_key_stores), &real_len, (uint8_t *)app_key_stores);
    if (nv_err) {
        BT_ERR("Failed to load appkey, err: 0x:%x", nv_err);
        return;
    }

    if (real_len != sizeof(app_key_stores)) {
        BT_ERR("Failed to load appkey, unexpected value len real: %d :%d", real_len, sizeof(app_key_stores));
        return;
    }

    for (uint8_t i = 0; i < ARRAY_SIZE(app_key_stores); i++) {
        if (app_key_stores[i].app_idx == BT_MESH_KEY_UNUSED) {
            continue;
        }
        err = app_key_set(&app_key_stores[i]);
        if (err) {
            BT_ERR("Failed to set appkey, err: %d", nv_err);
        }
    }
}
#endif