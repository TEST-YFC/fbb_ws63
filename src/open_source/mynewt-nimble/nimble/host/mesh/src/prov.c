...
static bool g_prov_is_sig_mesh_mode = true;
...
int bt_mesh_prov_reset_state(void)
{
	...
	// add for custom prov
	if (g_prov_is_sig_mesh_mode) {
        atomic_set_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
    } else {
        atomic_clear_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
    }
...
int bt_mesh_input_number(uint32_t num)
{
	...
	if (bt_mesh_prov_link.role == NULL) {
		BT_ERR("role is null");
		return -EINVAL;
	}
		      		  ...
		      		  struct os_mbuf *buf)
{
	...
	// for provisioner resend prov data
	bt_mesh_prov_link.cur_pdu_type = type;
...
static void prov_link_opened(const struct prov_bearer *bearer, void *cb_data)
{
	...
	if (bt_mesh_prov_link.role && bt_mesh_prov_link.role->link_opened) {
...
{
	if (bt_mesh_prov_link.role && bt_mesh_prov_link.role->link_closed) {
    ...
    // after link close, recover user set prov mode
    if (g_prov_is_sig_mesh_mode) {
        atomic_set_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
    } else {
        atomic_clear_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
...
static void prov_bearer_error(const struct prov_bearer *bearer, void *cb_data,
			      uint8_t err)
{
	if (bt_mesh_prov_link.role && bt_mesh_prov_link.role->error) {
...
{
    return atomic_test_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
}

void bt_mesh_pb_set_sig_mesh_prov(bool flag)
{
    g_prov_is_sig_mesh_mode = flag;
    if (g_prov_is_sig_mesh_mode) {
        atomic_set_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
    } else {
        atomic_clear_bit(bt_mesh_prov_link.flags, PROV_SIG_MESH);
