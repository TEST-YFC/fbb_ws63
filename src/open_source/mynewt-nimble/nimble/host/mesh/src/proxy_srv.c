...
static void proxy_msg_recv(struct bt_mesh_proxy_role *role)
{
	...
	case BT_MESH_PROXY_BEACON:
		...
		bt_mesh_beacon_recv(NULL, 0xff, role->buf);
		break;
...
{
	struct os_mbuf *buf = NET_BUF_SIMPLE(BEACON_BUF_SIMPLE_LEN);
	int rc;

	net_buf_simple_init(buf, 1);
#if defined(CONFIG_BT_MESH_PRIV_BEACONS)
    rc = bt_mesh_beacon_create(sub, buf, client->privacy);
#else
    rc = bt_mesh_beacon_create(sub, buf, false);
#endif
    if (rc) {
        os_mbuf_free_chain(buf);
        return rc;
...
static void private_node_id_start(struct bt_mesh_subnet *sub)
{
    ...
    sub->priv_beacon_ctx.node_id = BT_MESH_NODE_IDENTITY_RUNNING;
    sub->priv_beacon_ctx.node_id_start = k_uptime_get_32();
}
...
void bt_mesh_proxy_identity_stop(struct bt_mesh_subnet *sub, bool private)
{
    if (private) {
        sub->priv_beacon_ctx.node_id = BT_MESH_NODE_IDENTITY_STOPPED;
        sub->priv_beacon_ctx.node_id_start = 0U;
    } else {
...
#define ID_TYPE_PRIV_NET 0x02
#define ID_TYPE_PRIV_NODE 0x03

#define NET_ID_LEN   11
#define NODE_ID_LEN  19
#define PRIV_NET_LEN  19
#define PRIV_NODE_LEN  19
#define NET_ID_LEN   11
...
static const struct bt_data priv_node_id_ad[] = {
	BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
	BT_DATA_BYTES(BT_DATA_UUID16_ALL,
		      BT_UUID_16_ENCODE(BT_UUID_MESH_PROXY_VAL)),
		      BT_DATA(BT_DATA_SVC_DATA16, proxy_svc_data, PRIV_NODE_LEN),
};
...
static const struct bt_data priv_net_identity_ad[] = {
	BT_DATA_BYTES(BT_DATA_FLAGS, (BT_LE_AD_GENERAL | BT_LE_AD_NO_BREDR)),
	BT_DATA_BYTES(BT_DATA_UUID16_ALL,
		      BT_UUID_16_ENCODE(BT_UUID_MESH_PROXY_VAL)),
		      BT_DATA(BT_DATA_SVC_DATA16, proxy_svc_data, PRIV_NET_LEN),
};

static int node_identity_adv(struct bt_mesh_subnet *sub, int32_t duration)
{
...
static int priv_net_identity_adv(struct bt_mesh_subnet *sub, int32_t duration)
{
    struct ble_gap_adv_params fast_adv_param = {ADV_OPT_PROXY ADV_FAST_INT};
#if ADV_OPT_USE_NAME
    const char *name = CONFIG_BT_DEVICE_NAME;
    size_t name_len = strlen(name);
    struct bt_data sd_data = {.type = BT_DATA_NAME_COMPLETE, .data_len = name_len, .data = (void *)name};
    struct bt_data *sd = &sd_data;
#else
    struct bt_data *sd = NULL;
#endif
    uint8_t tmp[16] = {0};  // 16 size
    int err;

    proxy_svc_data[2] = ID_TYPE_PRIV_NET;  // 2 index

    err = bt_rand(proxy_svc_data + 11, 8);  // 11 index,8 size
    if (err) {
        return err;
    }

    memcpy(tmp, sub->keys[SUBNET_KEY_TX_IDX(sub)].net_id, 8);  // 8: size
    memcpy(tmp + 8, proxy_svc_data + 11, 8);                   // 8,11: offset, 8:size
    err = bt_encrypt_be(sub->keys[SUBNET_KEY_TX_IDX(sub)].identity, tmp, tmp);
    if (err) {
        return err;
    }
    memcpy(proxy_svc_data + 3, tmp + 8, 8);  // 3,8: offset, 8:size

    err = bt_mesh_adv_start(
        &fast_adv_param, duration, priv_net_identity_ad, ARRAY_SIZE(priv_net_identity_ad), sd, 1);  // 1 size
    if (err) {
        BT_WARN("Failed to advertise using Node ID (err %d)", err);
        return err;
    }
...
static int priv_node_identity_adv(struct bt_mesh_subnet *sub, int32_t duration)
{
    struct ble_gap_adv_params fast_adv_param = {ADV_OPT_PROXY ADV_FAST_INT};
#if ADV_OPT_USE_NAME
    const char *name = CONFIG_BT_DEVICE_NAME;
    ...
    proxy_svc_data[2] = ID_TYPE_PRIV_NODE;  // 2 index
    ...
    tmp[5] = 0x03;                            // 5: index, 0x03: Private Node Identity type
    memcpy(tmp + 6, proxy_svc_data + 11, 8);  // 6,8,11: offset
    sys_put_be16(bt_mesh_primary_addr(), tmp + 14);
    err = bt_encrypt_be(sub->keys[SUBNET_KEY_TX_IDX(sub)].identity, tmp, tmp);
    if (err) {
        return err;
    ...
    err = bt_mesh_adv_start(&fast_adv_param, duration, priv_node_id_ad, ARRAY_SIZE(priv_node_id_ad), sd, 1);  // 1 size
    if (err) {
        BT_WARN("Failed to advertise using Node ID (err %d)", err);
        return err;
    }
...
static int gatt_proxy_advertise(struct bt_mesh_subnet *sub)
{
    ...
    if (bt_mesh_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED) {
        BT_DBG("Network ID active for %d ms remaining", remaining);
    ...
    if (sub->node_id == BT_MESH_NODE_IDENTITY_RUNNING) {
        ...
        if (active < NODE_ID_TIMEOUT) {
            ...
            BT_DBG("Node Identity active for %u ms, %d ms remaining", active, remaining);
            err = node_identity_adv(sub, remaining);
        } else {
            bt_mesh_proxy_identity_stop(sub, false);
            BT_DBG("Node Identity stopped");
        }
    }

    if (bt_mesh_priv_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED) {
        BT_DBG("Private Network Identity active for %d ms remaining", remaining);
        err = priv_net_identity_adv(sub, remaining);
        BT_DBG("err: %d", err);
    }

    if (sub->priv_beacon_ctx.node_id == BT_MESH_NODE_IDENTITY_RUNNING) {
        uint32_t active = k_uptime_get_32() - sub->priv_beacon_ctx.node_id_start;

        if (active < NODE_ID_TIMEOUT) {
            remaining = NODE_ID_TIMEOUT - active;
            BT_DBG("Private Node Identity active for %u ms, %d ms remaining", active, remaining);
            err = priv_node_identity_adv(sub, remaining);
        } else {
            bt_mesh_proxy_identity_stop(sub, true);
            BT_DBG("Private Node Identity stopped");
        }
    }
...
static void proxy_ccc_write(uint16_t conn_handle)
{
    ...
    client = find_client(conn_handle);
    if (client == NULL) {
        BT_ERR("client not setup, conn:%d", conn_handle);
        return;
    }
...
int bt_mesh_proxy_gatt_disable(void)
{
	...
	/* FIXME: figure out end handle */
	ble_svc_gatt_changed(svc_handles.proxy_h, 0xffff);
	svc_handles.proxy_data_out_h = 0;
...
static void gatt_connected(uint16_t conn_handle)
{
...
#if defined(CONFIG_BT_MESH_PRIV_BEACONS)
	/** When either the GATT Proxy state or the Node Identity state is enabled, the Proxy Privacy parameter 
	(see Section 6.5) for the connection shall be Disabled.
    ...
    enum bt_mesh_feat_state node_identity_state, private_node_identity_state;
    bt_mesh_subnet_node_id_get(beacon_sub->net_idx, &node_identity_state);
    bt_mesh_subnet_priv_node_id_get(beacon_sub->net_idx, &private_node_identity_state);
    if (bt_mesh_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED || node_identity_state == BT_MESH_FEATURE_ENABLED) {
        client->privacy = false;
    } else {
        client->privacy = (bt_mesh_priv_gatt_proxy_get() == BT_MESH_FEATURE_ENABLED) ||
                          (private_node_identity_state == BT_MESH_FEATURE_ENABLED);
    }
#else
    client->privacy = false;
#endif
	BT_DBG("privacy: %d", client->privacy);
...
int ble_mesh_proxy_gap_event(struct ble_gap_event *event, void *arg)
{
	if ((event->type == BLE_GAP_EVENT_CONNECT) ||
		...
		if (event->subscribe.attr_handle == svc_handles.proxy_data_out_h) {
			...
			BT_WARN("handle not match att_handle:0x%x proxy:0x%x prov:0x%x", event->subscribe.attr_handle,
					 svc_handles.proxy_data_out_h, svc_handles.prov_data_out_h);
		}
	} else {
		BT_WARN("event not match:0x%x ", event->type);
	}
...
#if (MYNEWT_VAL(BLE_MESH_GATT_PROXY))
		k_work_deinit_delete(&clients[i].send_beacons);
#endif
...
#if (MYNEWT_VAL(BLE_MESH_GATT_PROXY))
	if (!bt_mesh_subnet_cb_list[5]) {
		bt_mesh_subnet_cb_list[5] = subnet_evt;
	}
#endif
