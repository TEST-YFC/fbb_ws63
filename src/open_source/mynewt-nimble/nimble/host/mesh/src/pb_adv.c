...
static struct prov_rx_exp g_prov_exp = {
	.is_filter = false,
	.link_id = 0,
	.tx_id = XACT_ID_MAX
};
...
// feature: user define prov
static pb_adv_recv g_pb_adv_recv = NULL;
...
static uint8_t last_seg(uint8_t len)
{
	...
	return (len / CONT_PAYLOAD_MAX) + (len % CONT_PAYLOAD_MAX ? 1 : 0);
}
...
static struct os_mbuf *adv_buf_create(uint8_t retransmits)
{
	...
	buf = bt_mesh_adv_create(BT_MESH_ADV_PROV,
				 BT_MESH_TRANSMIT(retransmits, 10),
				 BUF_TIMEOUT);
	if (!buf) {
...
static void gen_prov_cont(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	if (XACT_SEG_OFFSET(seg) + buf->om_len > RX_BUFFER_MAX) {
		BT_WARN("Rx buffer overflow. Malformed generic prov frame? off:%d, len:%d",
				XACT_SEG_OFFSET(seg), buf->om_len);
		return;
	}
	...
	if (!link.rx.seg) {
		// feature: user define prov
		if (g_pb_adv_recv != NULL) {
			if (!g_prov_exp.is_filter || g_prov_exp.link_id == rx->link_id) {
				recv_ret = g_pb_adv_recv(&rx->addr, rx->rssi, rx->link_id, link.rx.buf);
			}
		}
...
static void gen_prov_start(struct prov_rx *rx, struct os_mbuf *buf)
{
    ...
    if (rx->xact_id == link.rx.id) {
        if (!bt_mesh_pb_is_sig_mesh_prov() && ((type == PROV_DATA || type == PROV_COMPLETE) && g_pb_adv_recv != NULL)) {
            BT_DBG("Resending prov data, user define process");
        } else {
            if (!link.rx.seg) {
                if (!ack_pending()) {
                    BT_DBG("Resending ack");
        ...
        if (!bt_mesh_pb_is_sig_mesh_prov() && (type == PROV_DATA || type == PROV_COMPLETE)) {
            BT_DBG("user define process, act id not start from 0");
        } else {
            BT_WARN("Unexpected xact 0x%x, expected 0x%x", rx->xact_id, next_transaction_id(link.rx.id));
            return;
        }
    }
...
static void gen_prov_recv(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	// feature: support user define prov
	if (!atomic_test_bit(link.flags, ADV_LINK_ACTIVE) &&
...
{
	BT_DBG("retry for act:%d", link.tx.pending_ack);
	...
	if (k_uptime_get() - link.tx.start > TRANSACTION_TIMEOUT) {
		BT_WARN("xact_id: 0x%x start:%d", link.tx.id, link.tx.start);
...
// feature: user define prov
static int prov_adv_to_tx_buff(struct os_mbuf *msg,
			 prov_bearer_send_complete_t cb, void *cb_data)
{
	...
	BT_DBG("xact_id: 0x%x len: %u start:%d", link.tx.id, msg->om_len, link.tx.start);
			 ...
			 prov_bearer_send_complete_t cb, void *cb_data)
{
	...
	ret = prov_adv_to_tx_buff(msg, cb, cb_data);
	if (ret != 0) {
		return ret;
	}
...
void bt_mesh_pb_adv_recv(const bt_addr_le_t *addr, int8_t rssi, struct os_mbuf *buf)
{
	...
	if (!link.cb && !g_pb_adv_recv) {
		return;
	}

    BT_DBG("bt_mesh_prov_active before:%d", atomic_test_bit(link.flags, PROVISIONER));
    ...
    if (!atomic_test_bit(link.flags, PROVISIONER)) {
        if (!bt_mesh_prov_active() && bt_mesh_is_provisioned()) {
            if (bt_mesh_pb_is_sig_mesh_prov() || (type != PROV_DATA && type != PROV_COMPLETE)) {
    ...
    // feature: user define prov
	rx.rssi = rssi;
	rx.addr.type = addr->type;
	(void)memcpy_s(&rx.addr.val, sizeof(rx.addr.val), &addr->val, sizeof(addr->val));
...
{
	k_work_cancel_delayable_delete(&link.prot_timer);
    k_work_cancel_delayable_delete(&link.tx.retransmit);
...
{
	g_prov_exp.link_id = link_id;
	g_prov_exp.is_filter = true;
...
{
	g_prov_exp.link_id = link_id;
	g_prov_exp.is_filter = is_filter;
...
// unreliable send not start timer
int bt_mesh_prov_send_adv(struct os_mbuf *msg)
{
	int ret = 0;
	if (atomic_test_bit(link.flags, ADV_LINK_ACTIVE)) {
		return -EBUSY;
	...
	ret = prov_adv_to_tx_buff(msg, NULL, NULL);
	if (ret != 0) {
		return ret;
	}
