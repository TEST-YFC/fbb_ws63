...
struct bt_mesh_beacon {
    ...
    uint8_t  last;        // Number of beacons during last observation window
    uint8_t  cur;         // Number of beacons observed during currently ongoing window.
    uint8_t  cache[26];   // Cached last beacon auth value
...
struct bt_mesh_subnet {
    ...
    struct bt_mesh_beacon priv_beacon_date;
    struct {
        uint16_t idx;        /* Private beacon random index */
        uint8_t node_id;        /* Private Node Identity enabled */
	    uint32_t node_id_start; /* Private Node Identity started timestamp */
	...
	struct bt_mesh_subnet_keys {
        ...
        uint8_t priv_beacon[16];
	} keys[2];
};

#ifdef CONFIG_BT_MESH_GATT_PROXY
extern void (*bt_mesh_subnet_cb_list[6]) (struct bt_mesh_subnet *sub,
