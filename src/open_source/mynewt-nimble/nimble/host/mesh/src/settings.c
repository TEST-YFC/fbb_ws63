...
{
    k_work_cancel_delayable_delete(&pending_store);
}
...
void bt_mesh_settings_conf_load(void)
{
    // 网络信息恢复
    BT_INFO("Begin to recover.");
    int err = bt_mesh_net_recover();
	if (err != 0) {
		BT_INFO("not provisioned, do not recover other.");
		return;
	}
    bt_mesh_net_iv_recover();
    bt_mesh_net_seq_recover();
    bt_mesh_subnet_recover();
    bt_mesh_app_key_recover();
    bt_mesh_hb_pub_recover();
    bt_mesh_cfg_recover();
    bt_mesh_va_recover();
    bt_mesh_model_recover();
    mesh_commit();
}
#endif

#endif /* MYNEWT_VAL(BLE_MESH_SETTINGS) || MYNEWT_VAL(BLE_MESH_SETTINGS_EXT) */