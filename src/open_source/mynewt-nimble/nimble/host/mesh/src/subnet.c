...
void (*bt_mesh_subnet_cb_list[6]) (struct bt_mesh_subnet *sub,
...
void clear_net_key(uint16_t net_idx)
{
	char path[20];
	int err;
...
#if MYNEWT_VAL(BLE_MESH_SETTINGS)
void store_subnet(uint16_t net_idx)
{
			   ...
			   const uint8_t key[16])
{
    ...
    BT_DBG("PrivateBeaconKey %s", bt_hex(&keys->priv_beacon, PRI_BEACON_MSG_LEN));
#endif
...
#if defined(CONFIG_BT_MESH_PRIV_BEACONS)
    for (int i = 0; i < ARRAY_SIZE(subnets); i++) {
        if (subnets[i].net_idx != BT_MESH_KEY_UNUSED && subnets[i].node_id == BT_MESH_FEATURE_ENABLED &&
            subnets[i].priv_beacon_ctx.node_id) {
            bt_mesh_proxy_identity_stop(&subnets[i], false);
        }
    }
#endif
...
uint8_t bt_mesh_subnet_priv_node_id_set(uint16_t net_idx, enum bt_mesh_feat_state priv_node_id)
{
    ...
    if (!IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY) || !IS_ENABLED(BLE_MESH_PRIV_BEACONS)) {
        return STATUS_FEAT_NOT_SUPP;
    }
...
struct net_key_store {
    uint16_t net_idx;            /* NetKeyIndex */
    struct net_key_val val;
} __packed;

static struct net_key_store net_key_stores[CONFIG_BT_MESH_SUBNET_COUNT]  = {
	[0 ... (CONFIG_BT_MESH_SUBNET_COUNT - 1)] = {
		.net_idx = BT_MESH_KEY_UNUSED,
	},
};;

static int net_key_set(struct net_key_store *net)
{
    struct net_key_val *key = &net->val;
    int len;

    BT_DBG("NetKeyIndex 0x%03x recovered from storage", net->net_idx);

    return bt_mesh_subnet_set(
        net->net_idx, key->kr_phase, key->val[0], (key->kr_phase != BT_MESH_KR_NORMAL) ? key->val[1] : NULL);
}
...
void clear_net_key(uint16_t net_idx)
{
    int err = 0;
    unsigned i;
    for (i = 0; i < ARRAY_SIZE(net_key_stores); i++) {
        struct net_key_store *store = &net_key_stores[i];
        if (store->net_idx == net_idx) {
            memset(store, 0x00, sizeof(*store));
            store->net_idx = BT_MESH_KEY_UNUSED;
            break;
        }
    }

    err = ble_mesh_nv_push_encrypt(BTH_BLE_MESH_NET_KEY_NV_ID, (uint8_t *)net_key_stores, sizeof(net_key_stores));
    if (err) {
        BT_ERR("Failed to clear NetKeyIndex 0x%03x", net_idx);
    } else {
        BT_DBG("Cleared NetKeyIndex 0x%03x", net_idx);
...
void store_subnet(uint16_t net_idx)
{
    ...
    int err;
    unsigned i;
    struct net_key_store *store = NULL;
    for (i = 0; i < ARRAY_SIZE(net_key_stores); i++) {
        if (net_key_stores[i].net_idx == BT_MESH_KEY_UNUSED && store == NULL) {
            store = &net_key_stores[i];
            continue;
        }

        if (net_key_stores[i].net_idx == net_idx) {
            store = &net_key_stores[i];
            break;
        }
    }
    ...
    BT_INFO("NetKeyIndex 0x%03x", net_idx);
    key = &store->val;
    ...
    if (err) {
        ...
        BT_INFO("Stored NetKey");
    }
}

void bt_mesh_subnet_recover(void)
{
    errcode_t nv_err;
    int err;
    uint16_t real_len = 0;
    ...
    BT_DBG("load NetKey");
    nv_err = ble_mesh_data_read(BTH_BLE_MESH_NET_KEY_NV_ID, sizeof(net_key_stores), &real_len, (uint8_t *)net_key_stores);
    if (nv_err) {
        BT_ERR("Failed to load NetKey err: 0x:%x", nv_err);
        return;
    }

    for (uint8_t i = 0; i < ARRAY_SIZE(net_key_stores); i++) {
        if (net_key_stores[i].net_idx == BT_MESH_KEY_UNUSED) {
            continue;
        }
        err = net_key_set(&net_key_stores[i]);
        if (err) {
            BT_ERR("Failed to set net key, err:%d", nv_err);
        }
    }
}
#endif