...
int bt_dh_key_gen(const uint8_t *pub_key, const uint8_t *priv_key, uint8_t *dhkey);
...
int bt_le_adv_start(const struct ble_gap_adv_params *param,
                    ...
                    const struct bt_data *sd, size_t sd_len, int *adv_id);
...
int k_work_cancel_delayable_delete(struct k_work_delayable *w);
...
static inline void sys_memcpy_swap(uint8_t *dst, const uint8_t *src, size_t length)
{
...
#define CONFIG_BT_MESH_EN_SEG_ADV_SUPPORT     BLE_MESH_EN_SEG_ADV_SUPPORT
...
#define CONFIG_BT_MESH_PRIV_BEACONS              MYNEWT_VAL(BLE_MESH_PRIV_BEACONS)
