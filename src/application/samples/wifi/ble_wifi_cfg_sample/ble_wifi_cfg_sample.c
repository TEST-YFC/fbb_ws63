...
static int8_t get_wifi_errcode(void)           { return g_errcode; }
uint8_t get_wifi_cfg_info_flag(void)           { return g_wifi_cfg_info_flag; }
void    set_wifi_cfg_info_flag(uint8_t flag)   { g_wifi_cfg_info_flag = flag; }
uint8_t get_wifi_list_req_flag(void)           { return g_wifi_list_req_flag; }
void    set_wifi_list_req_flag(uint8_t flag)   { g_wifi_list_req_flag = flag; }
...
int bgwc_wifi_list_resp_send(uint16_t handle)
{
    ...
    uint8_t result = 0x01;
    errcode_t ret = ble_wifi_cfg_server_send_report_by_handle(handle,
        (const uint8_t *)&result, sizeof(uint8_t));
...
static td_s32 example_get_match_network(wifi_sta_config_stru *expected_bss)
{
    ...
    memcpy_s(key, WIFI_CONFIG_INFO_SSID_LEN, g_data + WIFI_CONFIG_INFO_SSID_LEN,
             WIFI_CONFIG_INFO_KEY_LEN);
    PRINT("%s expected_ssid :%s\r\n", BGLE_WIFI_CFG_LOG, expected_ssid);

    for (bss_index = 0; bss_index < num; bss_index++) {
        if (strlen(expected_ssid) == strlen(result[bss_index].ssid)) {
            if (memcmp(expected_ssid, result[bss_index].ssid,
                       strlen(expected_ssid)) == 0) {
                find_ap = TD_TRUE;
                break;
            }
        }
    ...
    if (memcpy_s(expected_bss->ssid, WIFI_MAX_SSID_LEN, expected_ssid,
                 strlen(expected_ssid)) != 0) { osal_kfree(result); return -1; }
    if (memcpy_s(expected_bss->bssid, WIFI_MAC_LEN, result[bss_index].bssid,
                 WIFI_MAC_LEN) != 0) { osal_kfree(result); return -1; }
    expected_bss->security_type = result[bss_index].security_type;
    if (memcpy_s(expected_bss->pre_shared_key, WIFI_MAX_SSID_LEN, key,
                 strlen(key)) != 0) { osal_kfree(result); return -1; }
...
static td_bool __attribute__((unused)) example_check_connect_status(td_void)
{
    ...
    for (index = 0; index < 5; index++) {
        (td_void)osDelay(50);
        memset_s(&wifi_status, sizeof(wifi_linked_info_stru), 0,
                 sizeof(wifi_linked_info_stru));
...
static td_void bgwc_get_ap_list_info(wifi_scan_info_stru *scan_ret,
                                     uint32_t real_ap_number, uint8_t *report_data)
{
    bgwc_wifi_bss bss = { 0 };
    ...
    for (uint32_t idx = 0; idx < real_ap_number; idx++) {
        if (strlen(scan_ret[idx].ssid) == 0) { continue; }
        if (memcpy_s(bss.ssid, sizeof(bss.ssid), scan_ret[idx].ssid,
                     sizeof(scan_ret[idx].ssid) - 1) != EOK) { return; }
        bss.rssi = (int8_t)scan_ret[idx].rssi;
        if (memcpy_s(bss_data, sizeof(bgwc_wifi_bss), &bss,
                     sizeof(bss)) != EOK) { return; }
        PRINT("%d:ssid[%s]\trssi[%d] \t\r\n", count, bss.ssid, bss.rssi);
        bss_data += sizeof(bgwc_wifi_bss);
        count++;
        if (count >= WIFI_AP_LIST_MAX_NUM) { break; }
    }
...
static td_void bgwc_scan_state_changed(td_s32 state, td_s32 size)
{
    ...
    if ((get_wifi_list_req_flag() == 0) || (size <= 0)) { return; }

    scan_ret = (wifi_scan_info_stru *)malloc(sizeof(wifi_scan_info_stru) * real_ap_number);
    if (scan_ret == NULL) { return; }
    memset_s(scan_ret, sizeof(wifi_scan_info_stru) * real_ap_number, 0,
             sizeof(wifi_scan_info_stru) * real_ap_number);
    ...
    if (report_data == NULL) { free(scan_ret); return; }
...
static void bgwc_wifi_reason_code(td_s32 reason_code, int8_t *err_code)
{
    ...
    if (disconn_by_ap == 1) {
        switch (reason_code) {
        case 0:                              *err_code = WIFI_ERRCODE_NONE;          break;
        ...
        case WLAN_REASON_GROUP_KEY_UPDATE_TIMEOUT:
                                             *err_code = WIFI_ERRCODE_PWD_ERROR;      break;
        default:                             *err_code = WIFI_ERRCODE_OTHERS;         break;
        }
        return;
    }
    if (reason_code >= MAC_STATUS_MAX) { reason_code -= MAC_STATUS_MAX; }
    ...
    if (reason_code == WLAN_DISASOC_MISC_LINKLOSS) {
        ...
        *err_code = WIFI_ERRCODE_SSID_NOT_FOUND;
        ...
        switch (reason_code) {
        case WLAN_STATUS_CHALLENGE_FAIL:     *err_code = WIFI_ERRCODE_PWD_ERROR;      break;
        default:                             *err_code = WIFI_ERRCODE_OTHERS;         break;
        }
    }
}

static td_void bgwc_connection_changed(td_s32 state, const wifi_linked_info_stru *info,
                                       td_s32 reason_code)
{
    UNUSED(state);
...
static int bgwc_nv_quick_connect(void)
{
    ...
    (void)memcpy_s(g_data + WIFI_CONFIG_INFO_SSID_LEN, WIFI_CONFIG_INFO_KEY_LEN,
...
static int ble_wifi_cfg_example_task(const char *arg)
{
    ...
    (td_void)osDelay(200);   /* wait for system init */
...
#define CONFIG_BLE_PROV_MAX_RETRIES 3
    ...
    for (int attempt = 0; attempt < CONFIG_BLE_PROV_MAX_RETRIES; attempt++) {
        ...
        /* Run one provisioning round */
        if (attempt == 0 && !use_ble) {
            /* NV path: WiFi already initialised by caller? No.
             * bgwc_wifi_start() needs to be called first. */
            PRINT("%s NV connect attempt...\r\n", BGLE_WIFI_CFG_LOG);
            (void)bgwc_wifi_start();
            last_errcode = prov_wifi_scan_connect_dhcp();
        } else {
            /* BLE path */
            if (!use_ble) {
                /* NV path failed; switch to BLE for remaining retries */
                use_ble = true;
                PRINT("%s NV failed, switching to BLE provisioning.\r\n",
                      BGLE_WIFI_CFG_LOG);
            }
            last_errcode = prov_run_one_round(attempt == 0 || !use_ble);
        }

        /* Report status to phone (BLE path only) */
        result[1] = (uint8_t)last_errcode;
        if (use_ble) {
            ble_wifi_cfg_server_send_report_by_uuid((const uint8_t *)result,
                                                     sizeof(result));
        }
        ...
        if (last_errcode == (int8_t)WIFI_ERRCODE_NONE) {
            ...
            if (use_ble) { bgwc_ble_stop(); }

#ifdef CONFIG_BLE_PROV_NV_ENABLE
            /* Only persist credentials from a fresh BLE provisioning,
             * not from NV auto-reconnect (avoids unnecessary NV wear). */
            if (use_ble) {
                char ssid_buf[WIFI_CONFIG_INFO_SSID_LEN];
                char pwd_buf[WIFI_CONFIG_INFO_KEY_LEN];
                (void)memcpy_s(ssid_buf, sizeof(ssid_buf), g_data,
                               WIFI_CONFIG_INFO_SSID_LEN);
                (void)memcpy_s(pwd_buf, sizeof(pwd_buf),
                               g_data + WIFI_CONFIG_INFO_SSID_LEN,
                               WIFI_CONFIG_INFO_KEY_LEN);
        ...
        PRINT("%s FAILED attempt %d/%d, errcode=%d.\r\n",
              BGLE_WIFI_CFG_LOG, attempt + 1, CONFIG_BLE_PROV_MAX_RETRIES, last_errcode);
...
static void bgle_wifi_cfg_entry(void)
{
    ...
    osal_kthread_lock();
    osal_task *g_wifi_cfg_task = osal_kthread_create(
