...
#define RADAR_DEFAULT_ANT_SPACE_AMP_COEF    10000
#define RADAR_POWER_ON_DELAY                150 // 1.5s
#define RADAR_DEFAULT_STA_CREATE_RNG_THRES  0   // 0m
#define RADAR_DEFAULT_ANGLE_BOUNDARY        60
#define RADAR_DEFAULT_ANTI_MULTIPATH_MODE   0
#define RADAR_AI_SWITCH                     0
#define RADAR_DEFAULT_SENSITIVITY_PARA_LEN  46  // 对应45*0.15=6.75m
#define RADAR_DEFAULT_DYN_SENSITIVITY_PARA0 60  // 动检测0灵敏度默认值
#define RADAR_DEFAULT_DYN_SENSITIVITY_PARA1 60  // 动检测1灵敏度默认值
#define RADAR_DEFAULT_STA_SENSITIVITY_PARA0 90  // 静检测0灵敏度默认值
#define RADAR_DEFAULT_TX_POWER_IDX          {34}  // 对应发射功率-3dBm
#define RADAR_ANTCODE_TABLE                 {6, 2, 4, 0}
#define SLEEP_DATA_CH_NUM                   3
#define SLEEP_DATA_BIN_NUM                  40

static complex_short_t g_radar_sleep_raw_data[SLEEP_DATA_CH_NUM][SLEEP_DATA_BIN_NUM] = {0};

// 雷达结果回调函数实现样例
static void radar_print_res(const radar_result_t *result)
{
    osal_printk("[SLP_RADAR]");
    for (uint32_t i = 0; i < RADAR_MAX_RPT_TARGET_NUM; i++) {
        osal_printk("%1u,%3d,%5d,%4d,", result->target_info[i].id, result->target_info[i].rng,
...
// 雷达睡眠数据回调函数实现样例
static void radar_copy_sleep_raw_data(radar_raw_data_msg_t *data_msg)
{
    // 此回调函数内只完成数据的拷贝或存放，不要进行其他复杂处理阻塞slp_radar_client线程
    (void)memcpy_s(g_radar_sleep_raw_data, sizeof(complex_short_t) * SLEEP_DATA_CH_NUM * SLEEP_DATA_BIN_NUM,
                   data_msg->data, data_msg->data_len);
    // 向算法处理线程发送消息进行数据处理
}
...
static void register_radar_cb(void)
{
    ...
    ret = uapi_radar_register_raw_data_cb(radar_copy_sleep_raw_data);
    if (ret != ERRCODE_RC_SUCCESS) {
        osal_printk("[SLP RADAR][SAMPLE]uapi_radar_register_raw_data_cb failed, ret: 0x%x.\r\n", ret);
    }
    // 注册设置芯片上下电状态回调函数
...
static void radar_init_alg_para(radar_alg_para_t *alg_para)
{
    // 算法基础参数
    ...
    alg_para->basic_para.ant_space_amp_coef = RADAR_DEFAULT_ANT_SPACE_AMP_COEF;
    alg_para->basic_para.static_create_range = RADAR_DEFAULT_STA_CREATE_RNG_THRES;
    alg_para->basic_para.is_wire_down = 0;
    alg_para->basic_para.angle_boundary = RADAR_DEFAULT_ANGLE_BOUNDARY;
    alg_para->basic_para.anti_multipath_mode = RADAR_ANTI_MULTIPATH_DEFAULT_MODE;
    alg_para->basic_para.cali_res_save_mode = RADAR_CALI_RES_SAVE_IN_NV;
    // AI补偿参数
    alg_para->ai_para.ai_status = RADAR_AI_SWITCH;
    ...
    // 灵敏度
    alg_para->sensitivity_para.para_len = RADAR_DEFAULT_SENSITIVITY_PARA_LEN;
    for (uint32_t i = 0; i < alg_para->sensitivity_para.para_len; i++) {
        alg_para->sensitivity_para.dyn_sensitivity_0[i] = RADAR_DEFAULT_DYN_SENSITIVITY_PARA0;
        alg_para->sensitivity_para.dyn_sensitivity_1[i] = RADAR_DEFAULT_DYN_SENSITIVITY_PARA1;
        alg_para->sensitivity_para.sta_sensitivity_0[i] = RADAR_DEFAULT_STA_SENSITIVITY_PARA0;
    }
}
...
static void radar_function_start(void)
{
    ...
    // 2. 硬件参数
    radar_hardware_para_t hardware_para = {
        .rf_para = {
            .tx_power_idx = RADAR_DEFAULT_TX_POWER_IDX, // 只初始化首个元素值即可
            .ant_code = RADAR_ANTCODE_TABLE,    // SP31C只用到了ant_code数组的前四个元素，radar_hardware_para_t结构体中的其他成员无效
        },
    };
    ret = uapi_radar_set_hardware_para(&hardware_para);
    if (ret != ERRCODE_RC_SUCCESS) {
        osal_printk("[SLP RADAR][SAMPLE]uapi_radar_set_hardware_para failed, ret: 0x%x.\r\n", ret);
        return;
    }

    // 3. 算法参数
    radar_alg_para_t alg_para = {0};
    radar_init_alg_para(&alg_para);
    ret = uapi_radar_set_alg_para(&alg_para, 0);
    if (ret != ERRCODE_RC_SUCCESS) {
