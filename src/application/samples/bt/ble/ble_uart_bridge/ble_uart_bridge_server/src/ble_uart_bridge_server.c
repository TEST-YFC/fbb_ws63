...
static void ble_uart_bridge_handle_cccd_write(uint8_t server_id, uint16_t conn_id, gatts_req_write_cb_t *request)
{
    ...
    osal_printk("%s UART RX notify CCCD %s\r\n", BLE_UART_BRIDGE_SERVER_LOG,
                                             ...
                                             errcode_t status)
{
    ...
    if (response_status == GATT_STATUS_SUCCESS) {
        ...
        osal_printk("%s BLE write forwarded to UART TX: %.*s\r\n", BLE_UART_BRIDGE_SERVER_LOG, g_property_value_len,
                    g_property_value);
        (void)uapi_uart_write(BLE_UART_BRIDGE_UART_BUS, g_property_value, g_property_value_len, 0);
                                                         ...
                                                         const char *name)
{
    ...
    if (ret == ERRCODE_BT_SUCCESS) {
        osal_printk("%s %s sent: %.*s\r\n", BLE_UART_BRIDGE_SERVER_LOG, name, len, data);
    } else {
