...
#define BLE_UART_BRIDGE_SERVICE_UUID 0x3333
#define BLE_UART_BRIDGE_DATA_UUID 0x3434
#define BLE_UART_BRIDGE_NOTIFY_UUID 0x3435
...
static const uint8_t TARGET_NAME[] = "uart_bridge";
static const uint8_t DEFAULT_VALUE[] = "uart_ready";
static const uint8_t NEW_VALUE[] = "uart_from_central";
static bt_uuid_t g_client_app_uuid = {BLE_UART_BRIDGE_UUID_LEN, {0x33, 0x33}};
...
{
    bt_uuid_t uuid = {0};
    ble_uart_bridge_uuid16(BLE_UART_BRIDGE_SERVICE_UUID, &uuid);
                                            ...
                                            errcode_t status)
{
    ...
    osal_printk("%s BLE notify forwarded to UART TX: %.*s\r\n", BLE_UART_BRIDGE_CLIENT_LOG, data->data_len, data->data);
    (void)uapi_uart_write(BLE_UART_BRIDGE_UART_BUS, data->data, data->data_len, 0);
    if (!g_read_started) {
        g_read_started = true;
...
errcode_t ble_uart_bridge_client_send_write(const uint8_t *data, uint16_t length)
{
    ...
    return gattc_write_req(g_client_id, g_conn_id, &write_value);
}
