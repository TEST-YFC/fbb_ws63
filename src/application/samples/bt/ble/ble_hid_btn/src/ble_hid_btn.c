...
static const uint8_t g_hid_info[] = { 0x11, 0x01, 0x00, 0x03 };
...
static const uint8_t g_report_map[] = {
...
static void cbk_read_request(uint8_t server_id, uint16_t conn_id,
                              gatts_req_read_cb_t *para, errcode_t status)
{
    unused(server_id); unused(conn_id);
    osal_printk("%s read req: hdl=%d offset=%d status=%d\r\n",
                BLE_HID_TAG, para->handle, para->offset, status);
}

static void cbk_write_request(uint8_t server_id, uint16_t conn_id,
                               gatts_req_write_cb_t *para, errcode_t status)
{
    unused(server_id); unused(conn_id);
...
static void cbk_mtu_changed(uint8_t server_id, uint16_t conn_id,
                             uint16_t mtu, errcode_t status)
{
...
static void cbk_conn_state_change(uint16_t conn_id, bd_addr_t *addr,
 ...
 */
static errcode_t add_chara(uint16_t uuid, uint8_t props, uint8_t perms,
                            const uint8_t *value, uint16_t value_len,
                            uint16_t *out_val_handle)
{
    gatts_add_chara_info_t ch = { 0 };
    gatts_add_character_result_t res = { 0 };
    uuid16_to_bt(uuid, &ch.chara_uuid);
    ch.properties  = props;
    ch.permissions = perms;
    ch.value_len   = value_len;
    ch.value       = (uint8_t *)value;   /* const cast — stack won't mutate */
    errcode_t ret = gatts_add_characteristic_sync(g_server_id, g_hid_srvc_handle,
                                                   &ch, &res);
    if (out_val_handle) { *out_val_handle = res.value_handle; }
    osal_printk("%s chara uuid=0x%04X val_hdl=%d ret=%d\r\n",
                BLE_HID_TAG, uuid, res.value_handle, ret);
    return ret;
}
 ...
 */
static errcode_t add_chara_with_cccd(uint16_t uuid, uint8_t props, uint8_t perms,
                                      const uint8_t *value, uint16_t value_len,
                                      uint16_t *out_val_handle,
                                      uint16_t *out_cccd_handle)
{
    errcode_t ret = add_chara(uuid, props, perms, value, value_len, out_val_handle);
    if (ret != ERRCODE_BT_SUCCESS) { return ret; }
    ...
    uint16_t dummy;
    ret = gatts_add_descriptor_sync(g_server_id, g_hid_srvc_handle,
                                     &desc, (out_cccd_handle ? out_cccd_handle : &dummy));
    return ret;
}
...
static errcode_t build_hid_service(void)
{
    ...
    /* 1. Add primary service 0x1812 */
    bt_uuid_t svc_uuid;
    uuid16_to_bt(BLE_UUID_HID_SERVICE, &svc_uuid);
    ret = gatts_add_service_sync(HID_SERVER_ID, &svc_uuid, true, &g_hid_srvc_handle);
    if (ret != ERRCODE_BT_SUCCESS) {
    ...
    /* 2. Protocol Mode 0x2A4E (Read + Write No Response) */
    ret = add_chara(BLE_UUID_PROTOCOL_MODE,
                     ...
                     &g_protocol_mode, 1, NULL);
    if (ret) { return ret; }

    /* 3. Report Map 0x2A4B (Read only) */
    ret = add_chara(BLE_UUID_REPORT_MAP,
                     GATT_CHARACTER_PROPERTY_BIT_READ,
                     GATT_ATTRIBUTE_PERMISSION_READ,
                     g_report_map, sizeof(g_report_map), NULL);
    if (ret) { return ret; }

    /* 4. Boot Keyboard Input 0x2A22 (Read + Notify, with CCCD) */
    uint8_t zero_report[8] = { 0 };
    ret = add_chara_with_cccd(BLE_UUID_BOOT_KB_INPUT,
                               GATT_CHARACTER_PROPERTY_BIT_READ | GATT_CHARACTER_PROPERTY_BIT_NOTIFY,
                               GATT_ATTRIBUTE_PERMISSION_READ,
                               zero_report, sizeof(zero_report),
                               &g_input_val_handle, NULL);
    if (ret) { return ret; }

    /* 5. Boot Keyboard Output 0x2A32 (Read + Write + Write No Response) */
    ret = add_chara(BLE_UUID_BOOT_KB_OUTPUT,
                     GATT_CHARACTER_PROPERTY_BIT_READ | GATT_CHARACTER_PROPERTY_BIT_WRITE | GATT_CHARACTER_PROPERTY_BIT_WRITE_NO_RSP,
                     GATT_ATTRIBUTE_PERMISSION_READ | GATT_ATTRIBUTE_PERMISSION_WRITE,
                     zero_report, 1, NULL);
    if (ret) { return ret; }

    /* 6. HID Information 0x2A4A (Read only, 4 bytes) */
    ret = add_chara(BLE_UUID_HID_INFORMATION,
                     GATT_CHARACTER_PROPERTY_BIT_READ,
                     GATT_ATTRIBUTE_PERMISSION_READ,
                     g_hid_info, sizeof(g_hid_info), NULL);
    if (ret) { return ret; }

    /* 7. HID Control Point 0x2A4C (Write No Response, optional) */
    uint8_t cp_val = 0x00;
    ret = add_chara(BLE_UUID_HID_CONTROL_POINT,
...
errcode_t ble_hid_btn_init(void)
{
    ...
    if (register_callbacks() != ERRCODE_BT_SUCCESS) { return ERRCODE_FAIL; }

    osal_printk("%s enabling BLE\r\n", BLE_HID_TAG);
    enable_ble();

    osal_printk("%s set name '%s'\r\n", BLE_HID_TAG, CONFIG_BLE_HID_DEVICE_NAME);
    gap_ble_set_local_name((const uint8_t *)CONFIG_BLE_HID_DEVICE_NAME,
                           sizeof(CONFIG_BLE_HID_DEVICE_NAME) - 1);
