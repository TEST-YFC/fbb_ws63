...
{
    errcode_t ret;
    uint8_t announce_data_len;
    uint8_t seek_data_len;
    sle_announce_data_t data = {0};
    uint8_t adv_handle = SLE_ADV_HANDLE_DEFAULT;
    uint8_t announce_data[SLE_ADV_DATA_LEN_MAX] = {0};
    uint8_t seek_rsp_data[SLE_ADV_DATA_LEN_MAX] = {0};

    announce_data_len = (uint8_t)sle_set_adv_data(announce_data);
    data.announce_data = announce_data;
    data.announce_data_len = announce_data_len;

    seek_data_len = (uint8_t)sle_set_scan_response_data(seek_rsp_data);
    ...
    if (ret == ERRCODE_SLE_SUCCESS) {
        ...
        osal_printk("%s set announce data fail: 0x%x\r\n", SENSOR_SERVER_LOG, ret);
    }
...
errcode_t sle_sensor_report_announce_register_cbks(void)
{
    ...
    if (ret != ERRCODE_SLE_SUCCESS) {
        osal_printk("%s register announce callbacks fail: 0x%x\r\n", SENSOR_SERVER_LOG, ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}
