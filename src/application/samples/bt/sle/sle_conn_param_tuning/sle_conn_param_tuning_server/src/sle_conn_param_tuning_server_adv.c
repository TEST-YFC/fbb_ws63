...
enum {
    ...
    SLE_CONN_PARAM_DATA_DISCOVERY_LEVEL = 0x01,
    SLE_CONN_PARAM_DATA_ACCESS_MODE = 0x02,
    SLE_CONN_PARAM_DATA_COMPLETE_LOCAL_NAME = 0x0B,
...
static uint16_t sle_conn_param_set_announce_data(uint8_t *data)
{
    sle_conn_param_adv_common_value_t discovery = {
        .length = sizeof(discovery) - 1,
        .type = SLE_CONN_PARAM_DATA_DISCOVERY_LEVEL,
        .value = SLE_ANNOUNCE_LEVEL_NORMAL,
    };
    sle_conn_param_adv_common_value_t access_mode = {
        .length = sizeof(access_mode) - 1,
        .type = SLE_CONN_PARAM_DATA_ACCESS_MODE,
        .value = 0,
    };
    uint16_t index = 0;
...
static uint16_t sle_conn_param_set_seek_response(uint8_t *data)
{
    sle_conn_param_adv_common_value_t tx_power = {
        .length = sizeof(tx_power) - 1,
        .type = SLE_CONN_PARAM_DATA_TX_POWER_LEVEL,
        .value = SLE_CONN_PARAM_ADV_TX_POWER,
    };
    uint16_t index = 0;
    uint16_t name_len;

    if (memcpy_s(data, SLE_CONN_PARAM_ADV_DATA_MAX_LEN, &tx_power, sizeof(tx_power)) != EOK) {
        return 0;
    }
    index += sizeof(tx_power);
    name_len = sle_conn_param_set_local_name(&data[index], SLE_CONN_PARAM_ADV_DATA_MAX_LEN - index);
...
static errcode_t sle_conn_param_set_announce_param(void)
{
    ...
    const uint8_t local_addr[SLE_ADDR_LEN] = {0x21, 0x22, 0x23, 0x24, 0x25, 0x26};

    param.announce_mode = SLE_ANNOUNCE_MODE_CONNECTABLE_SCANABLE;
    param.announce_handle = SLE_CONN_PARAM_ADV_HANDLE;
    param.announce_gt_role = SLE_ANNOUNCE_ROLE_T_CAN_NEGO;
    param.announce_level = SLE_ANNOUNCE_LEVEL_NORMAL;
    param.announce_channel_map = SLE_CONN_PARAM_ADV_CHANNEL_MAP_DEFAULT;
    param.announce_interval_min = SLE_CONN_PARAM_ADV_INTERVAL;
    param.announce_interval_max = SLE_CONN_PARAM_ADV_INTERVAL;
    param.conn_interval_min = SLE_CONN_PARAM_DEFAULT_INTERVAL;
...
static errcode_t sle_conn_param_set_announce_payload(void)
{
    ...
    uint8_t announce_data[SLE_CONN_PARAM_ADV_DATA_MAX_LEN] = {0};
    uint8_t seek_rsp_data[SLE_CONN_PARAM_ADV_DATA_MAX_LEN] = {0};

    payload.announce_data_len = sle_conn_param_set_announce_data(announce_data);
    payload.seek_rsp_data_len = sle_conn_param_set_seek_response(seek_rsp_data);
...
errcode_t sle_conn_param_tuning_announce_register_callbacks(void)
{
    ...
    callbacks.announce_enable_cb = sle_conn_param_announce_enable_cb;
    callbacks.announce_disable_cb = sle_conn_param_announce_disable_cb;
...
errcode_t sle_conn_param_tuning_server_announce_start(void)
{
    ...
    ret = sle_conn_param_set_announce_payload();
    if (ret != ERRCODE_SLE_SUCCESS) {
