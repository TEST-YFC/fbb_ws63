...
{
    uint64_t timer_cycles = (uint64_t)OSAL_INTERRUPT_INTERVAL_US *
        (CONFIG_TIMER_CLOCK_VALUE / OSAL_INTERRUPT_US_PER_SECOND);
 ...
 * @endif
 */
static int osal_interrupt_worker_handler(void *data)
{
    unused(data);
...
static bool osal_interrupt_start_worker(void)
{
    ...
    osal_kthread_lock();
    worker_task = osal_kthread_create((osal_kthread_handler)osal_interrupt_worker_handler, NULL,
        "InterruptWorker", OSAL_INTERRUPT_TASK_STACK_SIZE);
    if (worker_task != NULL) {
 ...
 * @endif
 */
static int osal_interrupt_task_handler(void *data)
{
    bool semaphore_initialized = false;
    bool timer_hal_registered = false;
    ...
    osal_ret = osal_sem_binary_sem_init(&g_osal_interrupt_sem, 0);
    ...
    osal_ret = osal_irq_request(OSAL_INTERRUPT_IRQ_NUMBER, osal_interrupt_handler, NULL,
        "OsalInterrupt", NULL);
    if (osal_ret != OSAL_SUCCESS) {
        osal_printk("%s irq request failed irq=%u\r\n", OSAL_INTERRUPT_LOG,
            OSAL_INTERRUPT_IRQ_NUMBER);
        goto test_exit;
    }
    irq_requested = true;
    ...
    osal_printk("%s registered irq=%u priority=%u\r\n", OSAL_INTERRUPT_LOG,
        OSAL_INTERRUPT_IRQ_NUMBER, OSAL_INTERRUPT_IRQ_PRIORITY);

    g_osal_interrupt_worker_running = true;
    worker_started = osal_interrupt_start_worker();
    if (!worker_started) {
    ...
    while (reported_count < OSAL_INTERRUPT_EXPECTED_COUNT) {
        if (reported_count >= g_osal_interrupt_count) {
            if (osal_sem_down(&g_osal_interrupt_sem) != OSAL_SUCCESS) {
        ...
        while (reported_count < received_count) {
            ...
            osal_printk("%s irq=%u context=%s interrupted_progress=%u delta=%u\r\n",
                OSAL_INTERRUPT_LOG, reported_count + 1U,
                g_osal_interrupt_context_snapshot[reported_count] ? "ISR" : "TASK",
    ...
    test_passed = (g_osal_interrupt_count == OSAL_INTERRUPT_EXPECTED_COUNT) &&
...
test_exit:
    ...
    osal_printk("%s cleanup disable=PASS free=%s\r\n", OSAL_INTERRUPT_LOG,
        irq_requested ? "PASS" : "SKIP");
    osal_printk("%s summary irq=%u context_fail=%u progress_fail=%u\r\n", OSAL_INTERRUPT_LOG,
        g_osal_interrupt_count, g_osal_interrupt_context_failures, progress_failures);
    osal_printk("%s %s\r\n", OSAL_INTERRUPT_LOG,
        test_passed ? "ALL TESTS PASS" : "TESTS FAILED");
...
static void osal_interrupt_entry(void)
{
    ...
    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)osal_interrupt_task_handler, NULL,
        "OsalInterruptTask", OSAL_INTERRUPT_TASK_STACK_SIZE);
    if (task_handle != NULL) {
