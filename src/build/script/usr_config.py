...
def mconfig(command, chip, core, target, output, assignments = None, root=""):
    ...
    try:
        ...
        project_target = os.environ.get(
            "FBB_PROJECT_TARGET", "").replace('-', '_')
        project_conf = os.environ.get("FBB_KCONFIG_CONFIG")
        if project_conf and project_target == target:
            target_conf = project_conf
        else:
            target_conf = os.path.join(
                root, "build", "config", "target_config", chip,
                "menuconfig", core, "%s.config" % target)
